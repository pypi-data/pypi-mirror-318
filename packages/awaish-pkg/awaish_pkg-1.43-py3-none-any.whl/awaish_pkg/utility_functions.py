class UtilityFunctions:
    ...
    def palindrome_number_pattern(n: int) -> None:
        for i in range(1, n + 1):  # outer loop -->  for rows
            for j in range(1, (n - i) + 1):  # inner loop --> spaces
                print(" ", end=' ')
            for k in range(i, 0, -1):  # inner loop --> for printing numbers (value of k) in descending order
                print(k, end=' ')
            for l in range(2, i + 1):  # inner loop --> for printing numbers (value of l) in ascending order
                print(l, end=' ')
            print('\n')  # for next line
