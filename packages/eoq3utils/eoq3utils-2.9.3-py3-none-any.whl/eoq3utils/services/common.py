...
from ..cli.common import RegisterCliArgument, GetInModuleFileAbsPath
# type annotations
...
# Config files
RegisterCliArgument('config',                 "Config",               typ=str, default=None, description='Path to a config file', isConfigFile=True)
RegisterCliArgument('configout',              "Config out",            typ=str, default=None, description='If given, a config file with the current settings is written')
# Remote connection
...
RegisterCliArgument('sslCertificatePem',      "SSL certificate",      typ=str, default=GetInModuleFileAbsPath(SERVICES_MODULE_NAME, CONFIG_PATH, 'sslCertificate_DO_NOT_USE_IN_PRODUCTION.pem'), description='Path to the certificate pem file to be used for the SSL server')
RegisterCliArgument('sslCertificateKeyPem',   "SSL key",              typ=str, default=GetInModuleFileAbsPath(SERVICES_MODULE_NAME, CONFIG_PATH, 'sslKey_DO_NOT_USE_IN_PRODUCTION.pem'), description='Path to the private key pem file of the SSL server')
RegisterCliArgument('sslCertificatePassword', "SSL cert PW",          typ=str, default='ILS-admin', description='Pass phrase used for the certificate or key pem', isSecret=True)
# TCP connection
...
RegisterCliArgument('tcpPort',                "TCP port",             typ=int, default=5161, description='The EOQ TCP port')
# Domain type
...
RegisterCliArgument('usersFile',              "Users",                typ=str, default=GetInModuleFileAbsPath(SERVICES_MODULE_NAME, CONFIG_PATH, 'users_DO_NOT_USE_IN_PRODUCTION.json'), description='The path to the user definition JSON file')
RegisterCliArgument('permissionsFile',        "Permissions",          typ=str, default=GetInModuleFileAbsPath(SERVICES_MODULE_NAME, CONFIG_PATH, 'permissions_DO_NOT_USE_IN_PRODUCTION.json'), description='The path to the generic permission definitions JSON file')
...
RegisterCliArgument('accessControllerUser',   "Access ctrl. user",    typ=str, default='aco', description='The user for the access controller. Is required if interactive user management is desired')
RegisterCliArgument('accessControllerPw',     "Access ctrl. user PW", typ=str, default='aha1289?', description='The password of the access controller. Is required if interactive user management is desired', isSecret=True)
