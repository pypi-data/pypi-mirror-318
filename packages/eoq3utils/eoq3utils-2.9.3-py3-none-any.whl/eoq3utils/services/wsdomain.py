...
from eoq3autobahnws.util import CreateSelfSignedServerSslContext
# eoq cli commons
from ..cli.common import PrintCliHeader, GetCliPredefinedArguments, PrintCliArgument, ArgparseToEoqConfig
...
def ServerSslFactory(sslFactoryArgs:Dict[str,Any]):
    return CreateSelfSignedServerSslContext(sslFactoryArgs["sslCertificatePem"], sslFactoryArgs["sslCertificateKeyPem"], sslFactoryArgs["sslCertificatePassword"])
...
def PyEoq3WebSocketDomain(argv:List[Any])->int:
    ...
    # print header
    if(args.printHeader):
        PrintCliHeader("eoq3utils.services.wsdomain",version,{"eoq3":eoqVersion, "eoq3pyecoremdb":pyEcoreMdbVersion,"eoq3autobahnws":autobahnWsVersion, "eoq3pyaccesscontroller":pyAccessControllerVersion})
    #print args
    if(args.printArgs):
        for a in argDefs.values():
    ...
    try:
        ...
        if(args.enableSsl):
            sslContextFactory = ServerSslFactory
            sslContextFactoryArgs = {"sslCertificatePem" : args.sslCertificatePem,
                                     "sslCertificateKeyPem" : args.sslCertificateKeyPem,
                                     "sslCertificatePassword" : args.sslCertificatePassword}
...
if __name__ == "__main__":
    MODULE_NAME = "eoq3utils.services.wsdomain"
    code = PyEoq3WebSocketDomain(sys.argv[1:])
    os._exit(code)
