...
from .wsdomain import PyEoq3WebSocketDomain
from .wsmpl import PyEoq3WebSocketMpl
from .wsactionmanager import PyEoq3WebSocketActionManager
from .wsactionhandler import PyEoq3WebSocketActionHandler
# eoq cli commons
from ..cli.common import PrintCliHeader, GetCliPredefinedArguments, PrintCliArgument, ArgparseToEoqConfig, CliArgument
from .common import PrintServiceReadyMessage, ServiceMenuLoop, ShallRun, ConfigFileOpenFunc, SERVICE_READY_MESSAGE
...
'''            
def PyEoq3WebSocketServer(argv:List[Any]):
    ...
    # print header
    if(args.printHeader):
        PrintCliHeader("eoq3utils.services.wsserver", version,None)
    # print args
    if(args.printArgs):
        for a in argDefs.values():
    ...
    try:
        ...
        if(args.enableDomain):
            ...
            domMonitor.Start(PyEoq3WebSocketDomain,['--config',args.domainConfig])
        ...
        if(args.enableMpl):
            ...
            mplMonitor.Start(PyEoq3WebSocketMpl,['--config',args.mplConfig])
        ...
        if(args.enableActionManager):
            ...
            amaMonitor.Start(PyEoq3WebSocketActionManager,['--config',args.actionManagerConfig])
        ...
        if(args.enableActionHandler):
            ...
            ahaMonitor.Start(PyEoq3WebSocketActionHandler,['--config',args.actionHandlerConfig])
...
if __name__ == "__main__":
    code = PyEoq3WebSocketServer(sys.argv[1:])
    os._exit(code)
