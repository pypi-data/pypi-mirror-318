...
def filter_fealtures(final_mbl):
    final_mbl_filtered = {
        key: final_mbl[key] for key in customsAssessment_features if key in final_mbl
    }
    return final_mbl_filtered


async def process_customs_assessment(file_content, input_doc_type, llm_client):
...
async def extract_customs_assessment_from_pdf_w_llm(
    input_doc_type, file_content, llm_client
):
    """Extract data from the PDF file."""
    result = await process_customs_assessment(file_content, input_doc_type, llm_client)
