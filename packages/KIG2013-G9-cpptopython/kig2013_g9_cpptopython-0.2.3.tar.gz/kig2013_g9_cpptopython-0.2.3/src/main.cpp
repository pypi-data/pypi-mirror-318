...
vector<double> eqn4(double sx, double sy, double v, double t, double residual) {
    double iter = 0;
    vector<double> vct = {sx, sy, v, t, iter};
    vector<int> imissing;
    for (int i = 0; i < 4; i++) {
        if (vct.at(i) == 0) {
            imissing.push_back(i);
    ...
    if (imissing.size() == 1) {
        if (imissing.at(0) == 2) {
            tuple<vector<int>, vector<double>> result = numerical_analysis1(sx, sy, t, residual, 2.0);
            if (get<1>(result).size() == 0) {
                v = -1;
            } else {
                v = get<1>(result)[get<1>(result).size() - 1];
                iter = get<0>(result)[get<0>(result).size() - 1];
            }
        } else if (imissing.at(0) == 3) {
            tuple<vector<int>, vector<double>> result = numerical_analysis2(sx, sy, v, residual, v + 1);
            if (get<1>(result).size() == 0) {
                t = -1;
            } else {
...
vector<double> calculate(vector<double> inputs, double residual) {
        ...
        if (zerocount == 2) {
            ...
            if (output1.empty()) {
                ...
                if (output2.empty()) {
                    ...
                    if (output3.empty()) {
                        vector<double> output4 = eqn4(inputs.at(0), inputs.at(1), inputs.at(2), inputs.at(4), residual);
            ...
            if (fraud == 0) {
                vector<double> outputa = {output1.at(0), output2.at(0), output1.at(1), output1.at(2), output1.at(3), output1.at(4)};
                vector<double> outputb = {output1.at(0), output2.at(0), output2.at(1), output2.at(2), output2.at(3), output2.at(4)};
                for (int i = 0; i < 6; i++) {
...
tuple<vector<double>, vector<double>, vector<double>, vector<double>, vector<double>, vector<double>, vector<double>, int> forcefield(string funcF_X, string funcF_Y, string funcF_Z, double R_X, double R_Y, double R_Z, double V_X, double V_Y, double V_Z, double m, double dt, double tlim, double iterlim) {
    vector<string> testX = evalfunc(funcF_X, R_X, R_Y, R_Z);
    vector<string> testY = evalfunc(funcF_Y, R_X, R_Y, R_Z);
    vector<string> testZ = evalfunc(funcF_Z, R_X, R_Y, R_Z);
    vector<int> test = {stoi(testX[1]), stoi(testY[1]), stoi(testZ[1])};

    int fraud = 0;

    for (double status : test) {
        if (status != 0) {
            fraud += 1;
        }
    }
    ...
    if (fraud == 0) {
        vector<double> vcttime = {0};
        vector<double> vctx = {R_X};
        vector<double> vcty = {R_Y};
        vector<double> vctz = {R_Z};
        vector<double> vctfx = {stod(evalfunc(funcF_X, R_X, R_Y, R_Z)[0])};
        vector<double> vctfy = {stod(evalfunc(funcF_Y, R_X, R_Y, R_Z)[0])};
        vector<double> vctfz = {stod(evalfunc(funcF_Z, R_X, R_Y, R_Z)[0])};
        ...
        while (iter < iterlim && time < tlim - dt) {
            iter += 1;
            time += dt;
            vcttime.push_back(time);

            if (stoi((evalfunc(funcF_X, r_X, r_Y, r_Z))[1]) == 0) {
                F_X = stod((evalfunc(funcF_X, r_X, r_Y, r_Z))[0]);
                v_X = v_X + (F_X/m)*dt;
                r_X = r_X + v_X*dt;
                vctx.push_back(r_X);
                vctfx.push_back(F_X);
            ...
            if (stoi((evalfunc(funcF_Y, r_X, r_Y, r_Z))[1]) == 0) {
                F_Y = stod((evalfunc(funcF_Y, r_X, r_Y, r_Z))[0]);
                v_Y = v_Y + (F_Y/m)*dt;
                r_Y = r_Y + v_Y*dt;
                vcty.push_back(r_Y);
                vctfy.push_back(F_Y);
            } else {
                cout << stoi((evalfunc(funcF_Y, r_X, r_Y, r_Z))[1]);
                fail = true;
                break;
            }

            if (stoi((evalfunc(funcF_Z, r_X, r_Y, r_Z))[1]) == 0) {
                F_Z = stod((evalfunc(funcF_Z, r_X, r_Y, r_Z))[0]);
                v_Z = v_Z + (F_Z/m)*dt;
                r_Z = r_Z + v_Z*dt;
                vctz.push_back(r_Z);
                vctfz.push_back(F_Z);
            } else {
                cout << stoi((evalfunc(funcF_Z, r_X, r_Y, r_Z))[1]);
                fail = true;
                break;
            }
        }
        ...
        if (fail == false) {
            return make_tuple(vcttime, vctx, vcty, vctz, vctfx, vctfy, vctfz, 0);
        } else {
            return make_tuple(vcttime, vctx, vcty, vctz, vctfx, vctfy, vctfz, iter);
        }

    } else {
        vector<double> vcttime = {0};
        vector<double> vctx = {static_cast<double>(test[0])};
        vector<double> vcty = {static_cast<double>(test[1])};
        vector<double> vctz = {static_cast<double>(test[2])};
        vector<double> vctfx = {0};
        vector<double> vctfy = {0};
        vector<double> vctfz = {0};
        return make_tuple(vcttime, vctx, vcty, vctz, vctfx, vctfy, vctfz, -1);
    }
}

void seeforcefieldoutput(string funcF_X, string funcF_Y, string funcF_Z, double R_X, double R_Y, double R_Z, double V_X, double V_Y, double V_Z, double m, double dt, double tlim, double iterlim, int printoutput)
{
    tuple<vector<double>, vector<double>, vector<double>, vector<double>, vector<double>, vector<double>, vector<double>, int> result = forcefield(funcF_X, funcF_Y, funcF_Z, R_X, R_Y, R_Z, V_X, V_Y, V_Z, m, dt, tlim, iterlim);
    if (get<7>(result) == -1) {
        cout << "There is a problem with input functions." << endl << "Status for X: " << get<1>(result)[0] << endl << "Status for Y: " << get<2>(result)[0] << endl << "Status for Z: " << get<3>(result)[0] << endl;
    } else if (get<7>(result) > 0) {
        if (printoutput == 1) {
            for (int i = 0; i < static_cast<int>((get<0>(result)).size()); i++) {
                cout << "t = " << get<0>(result)[i] << ", X = " << get<1>(result)[i] << ", Y = " << get<2>(result)[i] << ", Z = " << get<3>(result)[i] << ", F_X = " << get<4>(result)[i] << ", F_Y = " << get<5>(result)[i] << ", F_Z = " << get<6>(result)[i] << endl;
            }
        }
        cout << "An error occurred when processing iteration " << get<7>(result) << "." << endl;
    } else {
        if (printoutput == 1) {
            for (int i = 0; i < static_cast<int>((get<0>(result)).size()); i++) {
                cout << "t = " << get<0>(result)[i] << ", X = " << get<1>(result)[i] << ", Y = " << get<2>(result)[i] << ", Z = " << get<3>(result)[i] << ", F_X = " << get<4>(result)[i] << ", F_Y = " << get<5>(result)[i] << ", F_Z = " << get<6>(result)[i] << endl;
...
PYBIND11_MODULE(KIG2013_G9_cpptopython, m) {
    ...
    m.def("forcefield", &forcefield, "Evaluates the trajectory of a spherical basketball in a force field.");
    m.def("seeforcefieldoutput", &seeforcefieldoutput, "See the output of the force field evaluator in text form.");
}