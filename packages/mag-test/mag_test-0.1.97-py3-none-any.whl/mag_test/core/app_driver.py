...
class AppDriver(webdriver.Remote):
    ...
    def switch_to_window_by_title(self, title: str):
        # 检查缓存中是否存在目标窗口的句柄
        if title in self.__window_cache:
            self.switch_to.window(self.__window_cache[title])
            return True
        return False

    def __cache_window(self):
        try:
            tp = TimeProbe('缓存窗口')
            initial_handles_num = len(self.window_handles)
            print(f"Initial handles count: {initial_handles_num}")  # 添加调试信息
            WebDriverWait(self, 60).until(lambda d: len(d.window_handles) > initial_handles_num)

            for window in self.window_handles:
                self.switch_to.window(window)
                self.__window_cache[window.title] = window
                print(f"Cached window: {self.title}")  # 添加调试信息
            tp.check('缓存完毕')
            tp.write_log()
        except (NoSuchElementException, TimeoutException, WebDriverException, Exception) as e:
            Logger.error(f'缓存窗口出错{str(e)}')
