...
class ElementFinder:
    ...
    #         element = ElementFinderUtils.find_element_by_type(combo_box, element_info.child_name, ControlType.LIST_ITEM)
    #     else:
    #         element = combo_box
    #
    #     return ElementFinder.__find_context_menu(driver, element, element_info.action, element_info.menu_item)
    ...
    @staticmethod
    def __find_combox_listbox_listview_treeview(driver:AppDriver, element_info:ElementInfo, child_type: ControlType, parent:WebElement=None):
        """
            查找树视图
            参数：
        ...
        if element_info.child_name:
            if element_info.init_status == InitStatus.EXPANDED:
                TreeUtils.expand_all(driver, main_element)

            element = ElementFinderUtils.find_element_by_type(main_element, element_info.child_name, child_type)
        else:
    ...
    #         element = tree_view
    #
    #     if element_info.action == ActionType.MENU_ITEM:
    #         element = ElementFinder.__find_context_menu(driver, element, element_info.menu_item)
    #
    #     if element is not None:
    #         Logger.debug(LogType.FRAME, f"控件或弹出菜单项为：{element}")
    #
    #     return element
    ...
    def __find_table(driver:AppDriver, element_info:ElementInfo, parent:WebElement=None):
        ...
        return ElementFinder.__find_context_menu(driver, element, element_info.menu_item)
    ...
    @staticmethod
    def __find_context_menu(driver:AppDriver, element:WebElement, action, menu_item_name:str):
        if action == ActionType.MENU_ITEM and menu_item_name:
            actions = ActionChains(driver)
