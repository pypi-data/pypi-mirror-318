...
class TableUtils:
    ...
    @staticmethod
    def set_table(table:WebElement, data:List[List[str]], init_status:InitStatus=None):
        table_items = ElementFinderUtils.find_elements_by_type(table, None, ControlType.DATA_ITEM)
        row_num, column_num = TableUtils.get_size(table)

        if init_status == InitStatus.T:
    ...
    @staticmethod
    def get_table_data(table:WebElement, init_status:InitStatus=None) -> List[List[str]]:
        table_data = [[]]
        table_items = ElementFinderUtils.find_elements_by_type(table, None, ControlType.DATA_ITEM)
        row_num, column_num = TableUtils.get_size(table)
