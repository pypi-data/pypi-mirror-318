...
class Client(Singleton):
    ...
    def _node_wait_lock(self, timeout):
        t = time.time()
    ...
    def _check_backends(self):
        return len([n for n in self.nodes if n['minio'] and n['connected'] and n['status']['disk']['free'] > 1]) > 0
    
    def _random_pick_node(self):
        if self._node_wait_lock():
            matched_nodes = [n for n in self.nodes if n['minio'] and n['connected'] and n['status']['disk']['free'] > 1]
