...
class WindowQueryHelper:
    ...
    def minimum_window_size(self, min_width:int=None, key="min_width_window", on_change=None, args=None, kwargs=None, default=None):
        ...
        while (value is None or key not in st.session_state or st.session_state[key] is None) and time.time() < timeout:
            time.sleep(0.1)
            value = _st_window_query_size(mediaMatchQ=query_result, on_change=on_change, args=args, kwargs=kwargs, key=key, default=default)

        if time.time() >= timeout:
            st.warning("Timeout reached while waiting for window size information.")
            return default

        return value
    
    def minimum_window_size_2(self, min_width: int = None, key="min_width_window", on_change=None, args=None, kwargs=None, default=None):
        if min_width is None or not isinstance(min_width, int):
            return default
        ...
        return self.wait_for_value(get_value, default=default)

    def wait_for_value(self, value_getter, timeout=5, interval=0.1, default=None):
