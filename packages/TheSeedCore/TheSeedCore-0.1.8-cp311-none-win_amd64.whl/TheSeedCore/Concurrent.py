...
class _ConfigManager:
    def __init__(self, shared_manager: Optional[multiprocessing.Manager] = None, **config: Unpack[_Config]):
        ...
        self._MultiProcessMode = False if shared_manager is None else True
    ...
    def MainProcessPriority(self) -> str | multiprocessing.Manager.Value:
        return self._MainProcessPriority.value if self._MultiProcessMode else self._MainProcessPriority

    @property
    def CoreProcessCount(self) -> int | multiprocessing.Manager.Value:
        return self._CoreProcessCount.value if self._MultiProcessMode else self._CoreProcessCount

    @property
    def CoreThreadCount(self) -> int | multiprocessing.Manager.Value:
        return self._CoreThreadCount.value if self._MultiProcessMode else self._CoreThreadCount

    @property
    def MaximumProcessCount(self) -> int | multiprocessing.Manager.Value:
        return self._MaximumProcessCount.value if self._MultiProcessMode else self._MaximumProcessCount

    @property
    def MaximumThreadCount(self) -> int | multiprocessing.Manager.Value:
        return self._MaximumThreadCount.value if self._MultiProcessMode else self._MaximumThreadCount

    @property
    def IdleCleanupThreshold(self) -> int | multiprocessing.Manager.Value:
        return self._IdleCleanupThreshold.value if self._MultiProcessMode else self._IdleCleanupThreshold

    @property
    def TaskThreshold(self) -> int | multiprocessing.Manager.Value:
        return self._TaskThreshold.value if self._MultiProcessMode else self._TaskThreshold

    @property
    def GlobalTaskThreshold(self) -> int | multiprocessing.Manager.Value:
        return self._GlobalTaskThreshold.value if self._MultiProcessMode else self._GlobalTaskThreshold

    @property
    def SubProcessPriority(self) -> str | multiprocessing.Manager.Value:
        return self._SubProcessPriority.value if self._MultiProcessMode else self._SubProcessPriority

    @property
    def ExpandPolicy(self) -> str | multiprocessing.Manager.Value:
        return self._ExpandPolicy.value if self._MultiProcessMode else self._ExpandPolicy

    @property
    def ShrinkagePolicy(self) -> str | multiprocessing.Manager.Value:
        return self._ShrinkagePolicy.value if self._MultiProcessMode else self._ShrinkagePolicy

    @property
    def ShrinkagePolicyTimeout(self) -> int | multiprocessing.Manager.Value:
        return self._ShrinkagePolicyTimeout.value if self._MultiProcessMode else self._ShrinkagePolicyTimeout

    @property
    def PerformanceReport(self) -> bool:
        return self._PerformanceReport.value if self._MultiProcessMode else self._PerformanceReport

    def _validateMainProcessPriority(self, shared_manager: Optional[multiprocessing.Manager], priority: ProcessPriority) -> str | multiprocessing.Manager.Value:
        allowed_values = ["IDLE", "BELOW_NORMAL", "NORMAL", "ABOVE_NORMAL", "HIGH", "REALTIME"]
        is_allowed_value = True if priority in allowed_values else False

        if not self._MultiProcessMode:
            if is_allowed_value:
                return priority
