    ...
    async def stream_text_logic(self, web_search=False, max_wait_time=300):
        ...
        while True:
            try:
                ...
                if current_length > last_yield_position:
                    # Split into words to process in chunks
                    words = current_text[last_yield_position:].split(" ")

                    if words:
                        # Process first half of new content
                        mid_point = len(words) // 2
                        first_half = ' '.join(words[:mid_point])

                        if first_half:
                            # Update last yield position to track where we left off
                            last_yield_position = len(current_text[:last_yield_position] + first_half)
                            yield first_half + " "
                ...
                if current_length == last_length:
                    ...
                    if unchanged_count >= 2:
                        # Yield any remaining text we haven't processed yet
                        if last_yield_position < current_length:
                            remaining_text = current_text[last_yield_position:]
                            if remaining_text:
                ...
                await asyncio.sleep(0.1)  # Reduced sleep time for more responsive streaming
            except Exception as e:
