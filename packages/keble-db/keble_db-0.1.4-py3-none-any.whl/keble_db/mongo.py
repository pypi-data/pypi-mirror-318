...
def merge_mongo_queries(token, *filters):
    ...
    for f in filters:
        if f is None:
            ...
            assert isinstance(f[token], list), "[Db] Expected f[token] to be a list in mongo queries"
            q = f[token]
...
def build_mongo_find_query(nested_dict: dict) -> dict:
    ...
    def _build(may_be_dict: dict, prefix=None):
        ...
        if type(may_be_dict) == list and len(may_be_dict) >0 and isinstance(may_be_dict[0], dict):
            for item in may_be_dict:
                child_key_val_paris = _build(item, prefix)
                key_val_pairs += child_key_val_paris
            return key_val_pairs

        if type(may_be_dict) != dict: return may_be_dict
    ...
    def build_key_val_pairs(may_be_tuple_list) -> dict:
        ...
        if isinstance(may_be_tuple_list, tuple):
            if isinstance(may_be_tuple_list[1], list):
                _dict[may_be_tuple_list[0]] = [build_key_val_pairs(i) for i in may_be_tuple_list[1]]
            else:
            ...
            for item in may_be_tuple_list:
                ...
                for k, v in new_dict.items():
                    if k in _dict and str(_dict[k]) != str(v):
                        raise AssertionError(f"[Db] Looks like you have a value contradiction on key: {k}")
                    _dict[k] = v
            return _dict
        else:
