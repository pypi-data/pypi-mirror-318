...
from typing import Generic, List, Optional, Type, TypeVar, Sequence, Tuple
...
class QdrantCRUDBase(Generic[ModelType, VectorModelType]):
    ...
    def __convert_record(self, record: qdrant_types.Record) -> Record[VectorModelType]:
        assert record.vector is None or isinstance(record.vector, dict), f"[Db] Expected record.vector to be a dict or None, but got {record.vector}"
    ...
    def search(self, client: QdrantClient, *, vector: List[float], vector_key: str, query: Optional[QueryBase | dict] = None,
        ...
        if query is not None and query.offset is not None:
            assert isinstance(query.offset, int), f"[Db] Expected search in qdrant with int or None type offset, but got {query.offset}"
            offset = query.offset
        scored_points: List[qdrant_types.ScoredPoint] = client.search(
            self.collection,
            query_filter=filter_,
            query_vector=(vector_key, vector),
            limit=query.limit if query is not None and query.limit is not None else 100, # by default 100
            offset=offset,
            score_threshold=score_threshold,
            **kwargs
        )
        return [Record(**sp.model_dump(include={"score", "shard_key", "id"}),
                       payload=self.model(**sp.payload) if sp.payload is not None else None,
                       vector=self.vector_model(**sp.vector)  # type: ignore
                       if sp.vector is not None else None) for sp in
                scored_points]

    def scroll_records(self, client: QdrantClient, *, query: QueryBase |dict| None, **kwargs) -> Tuple[
        List[Record], Optional[qdrant_types.PointId]]:
        if isinstance(query, dict):
            query = QueryBase(**query)
        filter_ = cleanse_query_for_search(query, offset_type="str")
        assert query is None or query.limit is not None, "[Db] You need to provide limit to perform a scroll in Qdrant"
        offset = None
        if query is not None and query.offset is not None:
            assert isinstance(query.offset, str), f"[Db] Expected str or None type query offset when scrolling records in qdrant, but got {query.offset}"
            offset = query.offset
    ...
    def scroll(self, client: QdrantClient, *, query: QueryBase | dict | None, **kwargs) -> Tuple[
        List[ModelType], Optional[qdrant_types.PointId]]:
        records, next_point_id = self.scroll_records(client, query=query, **kwargs)
        return [record.payload for record in records if record.payload is not None], next_point_id
    ...
    def delete_multi(self, client: QdrantClient, *, query: QueryBase | dict | None, **kwargs):
        if query is None: 
            query_dict: dict = {}
        else:
            if isinstance(query, dict):
        ...
        assert filters_exist or ids_exist, "[Db] you must set query.ids or query.filters to delete_multi in qdrant"
        if ids_exist:
            ...
            raise Exception("[Db] you must set query.ids or query.filters to delete_multi in qdrant")
    ...
    def overwrite_payload(self, client: QdrantClient, *, _id: qdrant_types.PointId, payload: ModelType, **kwargs):
