...
def latlon_to_crop_xy(lat, lon, world_w, world_h, nw_min_lat, nw_max_lat, nw_min_lon, nw_max_lon, crop_width, crop_height):
    """
...
###############################################################################
# 5. Main Function: Argument Parsing and Execution
###############################################################################
def main():
    parser = argparse.ArgumentParser(description="Generate typhoon season animation video.")
    parser.add_argument('--csv', type=str, required=True, help='Path to the typhoon data CSV file.')
    parser.add_argument('--map', type=str, required=True, help='Path to the world map image file.')
    parser.add_argument('--output', type=str, default=DEFAULT_OUTPUT_VIDEO, help='Name of the output video file.')
    parser.add_argument('--nw_min_lat', type=float, default=NW_MIN_LAT, help='NW Pacific minimum latitude.')
    parser.add_argument('--nw_max_lat', type=float, default=NW_MAX_LAT, help='NW Pacific maximum latitude.')
    parser.add_argument('--nw_min_lon', type=float, default=NW_MIN_LON, help='NW Pacific minimum longitude.')
    parser.add_argument('--nw_max_lon', type=float, default=NW_MAX_LON, help='NW Pacific maximum longitude.')
    parser.add_argument('--crop_width', type=int, default=CROP_WIDTH, help='Output canvas width.')
    parser.add_argument('--crop_height', type=int, default=CROP_HEIGHT, help='Output canvas height.')
    parser.add_argument('--frames_per_3h', type=int, default=FRAMES_PER_3H, help='Frames per 3-hour interpolation.')
    parser.add_argument('--fps', type=int, default=FPS, help='Video frame rate.')
    parser.add_argument('--ty_icon_radius', type=int, default=TY_ICON_RADIUS, help='Typhoon icon radius.')
    parser.add_argument('--ex_threshold_lat', type=float, default=EX_THRESHOLD_LAT, help='Latitude threshold to change category to EX.')
    parser.add_argument('--empty_speedup_factor', type=int, default=EMPTY_SPEEDUP_FACTOR, help='Speedup factor when no typhoon is present.')
    parser.add_argument('--static_frames_after_end', type=int, default=STATIC_FRAMES_AFTER_END, help='Number of static frames after the video ends.')
    ...
    generate_typhoon_animation(
        csv_file=args.csv,
        map_file=args.map,
        output_file=args.output,
        nw_min_lat=args.nw_min_lat,
        nw_max_lat=args.nw_max_lat,
        nw_min_lon=args.nw_min_lon,
        nw_max_lon=args.nw_max_lon,
        crop_width=args.crop_width,
        crop_height=args.crop_height,
        frames_per_3h=args.frames_per_3h,
        fps=args.fps,
        ty_icon_radius=args.ty_icon_radius,
        ex_threshold_lat=args.ex_threshold_lat,
        empty_speedup_factor=args.empty_speedup_factor,
        static_frames_after_end=args.static_frames_after_end
    )
