...
def split_into_files(diff_content: str) -> Dict[str, str]:
    ...
    while True:
        ...
        file_diff = diff_content[match.start() : end_pos]
        if file_diff:
            files[filename] = file_diff + "\n"
...
def process_hunk(hunk: str, include_context: bool) -> Tuple[str, str]:
    ...
    lines = HUNK_HEADER_PATTERN.split(hunk)[-1].splitlines(keepends=False)
