...
def get_git_diff(original: str, modified: str, file_path: str) -> str:
    """Get a proper git diff between original and modified content."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp_path = Path(tmpdir)
...
def simplify_diff(diff: str) -> str:
    """Simplify a git diff by removing unnecessary headers."""
    # Keep only the file path, hunk headers, and content lines
...
def test_compare_with_git_apply(test_case: dict) -> None:
    ...
    with runner.isolated_filesystem():
        # Get git diff and simplify it
        git_diff = get_git_diff(
            test_case["original"], test_case["modified"], "target.txt"
        )
        diff = simplify_diff(git_diff)
        print(f"\nTest case: {test_case['description']}")
        print(f"Original content: {test_case['original']!r}")
        print(f"Modified content: {test_case['modified']!r}")
        print(f"Git diff:\n{git_diff}")
        print(f"Simplified diff:\n{diff}")

        # Write target and diff files
        target_file = Path("target.txt")
        target_file.write_text(test_case["original"])
        ...
        assert our_result == test_case["modified"], (
            f"Test case '{test_case['description']}' failed:\n"
            f"Expected: {test_case['modified']!r}\n"
            f"Got: {our_result!r}\n"
            f"Diff:\n{diff}"
        )
...
def test_random_modifications(test_files: dict[str, Path], seed: int) -> None:
    ...
    for file_path in test_files.values():
        with runner.isolated_filesystem():
            ...
            # Get git diff and simplify it
            git_diff = get_git_diff(original, modified, file_path.name)
            diff = simplify_diff(git_diff)
