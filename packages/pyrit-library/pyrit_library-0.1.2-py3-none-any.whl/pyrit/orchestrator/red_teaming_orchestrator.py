...
class RedTeamingOrchestrator(Orchestrator):
    ...
    def __init__(
        ...
        *,
        ...
        self._scorer = scorer
        if scorer is not None and scorer.scorer_type != "true_false":
    ...
    async def check_conversation_complete_async(self) -> Union[Score, None]:
        ...
        if self._scorer:
            ...
            if score.score_type != "true_false":
                raise ValueError(f"The scorer must return a true/false score. The score type is {score.score_type}.")
            return score
        return None
    
    async def apply_attack_strategy_until_completion_async(self, *, max_turns: int = 5) -> Optional[Score]:
        """
        ...
        with open("target_responses.json", "w") as file:
            json.dump([], file)
        ...
        while turn <= max_turns:
            ...
            logging.info(f"Response from the prompt target: {response}")

            responses.append({
            "turn": turn,
            "response": response.converted_value
            })
    ...
    async def send_prompt_async(
        ...
        if prompt:
            ...
            logging.info(f"Prompt generated for the prompt target: {prompt}")

            self.prompt_responses_log.append({
                "red_teaming_response": prompt,
            })

        with open("red_teaming_responses.json", "w") as f:
            json.dump(self.prompt_responses_log, f, indent=4)
