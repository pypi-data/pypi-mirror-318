...
class ThsQuote:
    ...
    def wencai(self, condition: str):
        """
       问财API

       :param condition: 问财条件
       :return:  list 股票代码。
       """
        url_str = "https://apigate.10jqka.com.cn/d/platform/dynamicplate/stocks/self/v3/select"
        params = {'num': '7000', 'query': condition}
        url_with_params = url_str + '?' + urllib.parse.urlencode(params)
        ...
        datas = []
        for code in result.get('data', {}).get('codes', []):
            pre = market_str(code['market'])
            if pre:
                datas.append(pre + code['code'])
