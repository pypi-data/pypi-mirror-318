    ...
    return {
        ...
        f"{pool_name}UserPoolClient": {
            ...
            "Properties": {
                ...
                "UserPoolId": {"Ref": f"{pool_name}UserPool"},
        ...
        f"{pool_name}IdentityPool": {
            ...
            "Properties": {
                ...
                "CognitoIdentityProviders": [{
                    "ClientId": {"Ref": f"{pool_name}UserPoolClient"},
                    "ProviderName": {"Fn::GetAtt": [f"{pool_name}UserPool", "ProviderName"]},
                    "ServerSideTokenCheck": False
                }]
            }
        },
        f"{pool_name}IdentityPoolRoleAttachment": {
            "Type": "AWS::Cognito::IdentityPoolRoleAttachment",
            "Properties": {
                "IdentityPoolId": {"Ref": f"{pool_name}IdentityPool"},
                "Roles": {
                    "authenticated": {"Fn::GetAtt": [f"{pool_name}AuthRole", "Arn"]}
                }
            }
        },
        f"{pool_name}AuthRole": {
            "Type": "AWS::IAM::Role",
            "Properties": {
                "AssumeRolePolicyDocument": {
                    ...
                    "Statement": [{
                        ...
                        "Condition": {
                            "StringEquals": {
                                "cognito-identity.amazonaws.com:aud": {"Ref": f"{pool_name}IdentityPool"}
                            },
        ...
        f"{pool_name}UserGroup": {
            "Type": "AWS::Cognito::UserPoolGroup",
            "Properties": {
                "GroupName": f"{pool_name}-users",
                "UserPoolId": {"Ref": f"{pool_name}UserPool"}
            }
        },
        # SSM Parameters
        f"{pool_name}CognitoPoolIDStore": {
            "Type": "AWS::SSM::Parameter",
            "Properties": {
                "Name": f"{pool_name.upper()}_COGNITO_USER_POOL_ID",
                "Type": "String",
                "Value": {"Ref": f"{pool_name}UserPool"}
            }
        },
        f"{pool_name}CognitoClientIDStore": {
            "Type": "AWS::SSM::Parameter",
            "Properties": {
                "Name": f"{pool_name.upper()}_COGNITO_CLIENT_ID",
                "Type": "String",
                "Value": {"Ref": f"{pool_name}UserPoolClient"}
            }
        },
        f"{pool_name}CognitoIdentityPoolIDStore": {
            "Type": "AWS::SSM::Parameter",
            "Properties": {
                "Name": f"{pool_name.upper()}_COGNITO_IDENTITY_POOL_ID",
                "Type": "String",
                "Value": {"Ref": f"{pool_name}IdentityPool"}
            }
        },
        f"{pool_name}CognitoPoolARNStore": {
            "Type": "AWS::SSM::Parameter",
            "Properties": {
                "Name": f"{pool_name.upper()}_COGNITO_AUTH_ARN",
                "Type": "String",
                "Value": {"Fn::GetAtt": [f"{pool_name}UserPool", "Arn"]}
            }
        }
    }
