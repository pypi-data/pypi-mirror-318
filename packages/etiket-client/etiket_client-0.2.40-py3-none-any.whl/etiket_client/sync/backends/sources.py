...
def remove_sync_source(name : str):
    with Session() as session:
        ...
        try:
            src_loaded = SyncSource.init_from_sql(source)
            if issubclass(src_loaded.sync_class, SyncSourceDatabaseBase):
                manifest_manager.delete_manifest(name)
        except Exception as e:
            print(f"Failed to delete manifest for source {name}. Error: {e}")
