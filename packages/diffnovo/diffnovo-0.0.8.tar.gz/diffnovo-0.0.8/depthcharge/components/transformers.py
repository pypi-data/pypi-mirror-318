...
from .encoders import MassEncoder, PeakEncoder, PositionalEncoder, MultiScalePeakEmbedding
...
class SpectrumEncoder(torch.nn.Module):
    ...
    def forward(self, spectra, rt):
        ...
        spectra : torch.Tensor of shape (n_spectra, n_peaks, 2)
            ...
            are the same length.
        rt: torch.Tensor of shape (n_spectra)
        ...
        #print(rt)
        peaks = self.peak_encoder(spectra, rt)
...
class _PeptideTransformer(nn.Module):
    ...
    def detokenize(self, tokens):
        ...
        """
        sequence = [self._idx2aa.get(i.item(), "") for i in tokens if i]
        if "$" in sequence:
            idx = sequence.index("$")
...
class PeptideDecoder(_PeptideTransformer):
    ...
    def __init__(
        ...
        """Initialize a PeptideDecoder and Pogfuse"""
    ...
    def forward(self, sequences, time_, precursors, memory, memory_key_padding_mask):
        ...
        sequences : list of str or list of torch.Tensor
            ...
            amino acid. Optionally, these may be the token indices instead
            of a string.
        time: long tesnor of timestep indices
