...
def predict(
    ...
    """
    Predict peptide sequences with a trained diffnovo model.
...
def evaluate(
    peak_path: str, model_filename: str, config: Dict[str, Any], out_writer
) -> None:
    """
    Evaluate peptide sequence predictions from a trained diffnovo model.
    ...
    config : Dict[str, Any]
        The configuration options.
    """
    _execute_existing(peak_path, model_filename, config, True, out_writer)
...
def _execute_existing(
    ...
    """
    Predict peptide sequences with a trained diffnovo model with/without
    evaluation.
