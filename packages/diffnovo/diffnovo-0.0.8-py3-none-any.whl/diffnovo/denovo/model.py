...
logger = logging.getLogger("fnovo")
...
class Spec2Pep(pl.LightningModule, ModelMixin):
    ...
    **kwargs : Dict
    ...
    def reverse_denoising(self, x_t, spectra, precursors,noise_levels=None):
        rt = precursors[:, -1]
        precursors = precursors[:, :-1]
        batch_size = spectra.shape[0]
        for t in reversed(range(self.time_steps)):
            times = (t * torch.ones((batch_size,))).long().to(spectra.device)
            scores, tokens = self.decoder(x_t, times, precursors, *self.encoder(spectra, rt))
    ...
    def forward(
        ...
        diff_scores, diff_tokens = self.reverse_denoising(beam_tokens, spectra, precursors)
    ...
    def beam_search_decode(
        ...
        memories, mem_masks = self.encoder(spectra, precursors[:,-1])
        precursors = precursors[:, :-1]
    ...
    def _forward_step(
        ...
        tokens : torch.Tensor of shape (n_spectra, length)
            The predicted tokens for each spectrum.
        """
        # called second druing the training
        batch_size = spectra.shape[0]
        ...
        pred, truth = self.decoder(sequences, time, precursors[:, :-1], *self.encoder(spectra, precursors[:,-1]))
        return pred, truth
    ...
    def training_step(
        ...
        *args,
        ...
        """
        #called first during the training

        pred, truth = self._forward_step(*batch)
        pred = pred[:, :-1, :].reshape(-1, self.decoder.vocab_size + 1)
        #spectra, precursor = batch[0], batch[1]
        losses = []
        loss = self.celoss(pred, truth.flatten())
    
        #it might be required to change the loss function here
        self.log(
            "CELoss",
            {mode: loss.detach()},
            on_step=False,
    ...
    def num_matched_aa(self, s: str, l: List[str]):
        # chek the number of matched chars of l in s:
        return sum(1 for c in l if c in s)
    ...
    def validation_step(
        ...
        beam_pred_peps, diff_pred_peps, post_peptides, post_pep_scores, diff_aa_scores, beam_aa_scores = self.forward(batch[0], batch[1])

        beam_pred_peps_lst, diff_pred_peps_lst, peptides_true = [], [], []

        for beam_pred_pep, peptide_true, diff_pred_pep, post_pep, post_score_pep,beam_aa_score, diff_aa_score, precursor in zip(beam_pred_peps, batch[2], diff_pred_peps,post_peptides, post_pep_scores, beam_aa_scores, diff_aa_scores, batch[1]):     #print(f'beam_aa_score_shape={beam_aa_scores.shape}, diff_aa_scores ={len(diff_aa_scores)}')

            if len(beam_pred_pep) > 0:
                if beam_pred_pep[0] == "$":
                    beam_pred_pep = beam_pred_pep[1:]  # Remove stop token.
                if "$" not in beam_pred_pep and len(beam_pred_pep) > 0:
                    beam_pred_peps_lst.append(beam_pred_pep)
                    peptides_true.append(peptide_true)
                    beam_peptide, aa_token, beam_peptide_score, beam_aa_score = self._get_output_peptide_and_scores(
        ...
        beam_aa_precision, beam_aa_recall, beam_pep_recall = evaluate.aa_match_metrics(
            *evaluate.aa_match_batch(
                beam_pred_peps_lst, peptides_true, self.decoder._peptide_mass.masses
            )
        )
        post_aa_precision, post_aa_recall, post_pep_recall = evaluate.aa_match_metrics(
            *evaluate.aa_match_batch(
                post_peptides, peptides_true, self.decoder._peptide_mass.masses
            )
        )
        log_args = dict(on_step=False, on_epoch=True, sync_dist=True)
        self.log("aa_precision", {"valid": max(beam_aa_precision, post_aa_precision)}, **log_args)
        self.log("aa_recall", {"valid": max(beam_aa_recall, post_aa_recall)}, **log_args)
        self.log("pep_recall", {"valid": max(beam_pep_recall, post_aa_recall)}, **log_args)
    ...
    def predict_step(
        ...
        aa_scores : torch.Tensor of shape (n_spectra, length, n_amino_acids)
            The individual amino acid scores for each prediction.
        """
        beam_peptides, diff_peptides, post_peptides, post_pep_scores, diff_aa_scores, beam_aa_scores = self.forward(batch[0], batch[1])

        return batch[2], batch[1], beam_peptides, diff_peptides, post_peptides, post_pep_scores, beam_aa_scores
    ...
    def on_predict_epoch_end(
        ...
        for batch in results:
            for step in batch:
                for spectrum_i, precursor,  beam_aa_tokens, diff_aa_tokens, post_aa_tokens, post_score, beam_aa_scores in zip(*step):
                    ...
                    if not is_within_precursor_mz_tol:
                        beam_peptide_score -= 1
                    self.out_writer.psms.append(
                        (
                            spectrum_i,
                            beam_peptide,
                            beam_peptide_score,  #post_score[:len(post_pep)].tolist(),
                        ),
                    )

    def _get_output_peptide_and_scores(
        self, aa_tokens: List[str], aa_scores: torch.Tensor, alg_name=None
        ...
        if alg_name == 'diff':
            top_aa_scores = [
                aa_scores[self.decoder._aa2idx[aa_token]]
                for aa_token in aa_tokens[::step]
                ][::step]
        else:
            top_aa_scores = [
