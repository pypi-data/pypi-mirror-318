...
### MAIN CLASS     
class PyEcoreWorkspaceMpl(Mpl):
    def __init__(self, baseDir:str, config:Config=EOQ_DEFAULT_CONFIG):
        super().__init__()
    ...
    #@Override
    def Connect(self,domain:Domain, target:Qry=None, featureName:str=None, sessionId:str=None, trackDomainChanges:bool=False, progressCallback:Callable[[int],None]=None)->Obj:
        self.domain = domain
        ...
        self.logger.Info('ok (%.5f s)'%(end-start))
        if(progressCallback): progressCallback(self.__CalcProgress(1, 1, 0, 10))
        ...
        for mm in uploadedMms:
            ...
            self.logger.Info('ok (%.5f s)'%(end-start))
        if(progressCallback): progressCallback(self.__CalcProgress(1, 1, 10, 10))
        ...
        for r in self.workspaceModel.eAllContents():
            if(Resource == type(r) and r.isLoaded):
                ...
                if(not isinstance(m,EPackage)): #upload any user model
                    ...
                    self.logger.Info('ok (%.5f s)'%(end-start))
            i = i+1
            if(progressCallback): progressCallback(self.__CalcProgress(nModels, i, 20, 60))
        ...
        for m in uploadedModels:
            ...
            self.logger.Info('ok (%.5f s)'%(end-start))
            i = i+1
            if(progressCallback): progressCallback(self.__CalcProgress(nModels, i, 60, 100))
