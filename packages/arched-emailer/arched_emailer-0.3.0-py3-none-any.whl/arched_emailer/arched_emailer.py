...
class ArchedEmailer:
        ...
        if intercept_errors:
            ...
            def update_visited_urls():
                ...
                if "static" not in current_url and ".js" not in current_url and ".css" not in current_url:
                    ...
                    session['visited_urls'] = visited_urls[-5:]
            ...
            def teardown_request(exception):
                if exception:
                    self.log_exception(exception, add_current_user)
    ...
    def _cleanup_error_log(self, max_age_minutes: int = 2880):  # 1 day
        ...
        keys_to_delete = [k for k, v in self.errors_name_time.items()
                          if (current_time - v).total_seconds() / 60 > max_age_minutes]
        for k in keys_to_delete:
            del self.errors_name_time[k]
        self._save_error_log()

    def log_exception(self, exception, add_current_user):
        ...
        if (request.content_type in ["application/xml", "text/xml"]) and request.method.lower() == "post":
        ...
        elif request.method.lower() == "post" and request.json:
            ...
            error_text += f"<br><strong style='font-size:larger'>Request Form</strong>:<br>{format_dict_for_email(dict(request.form))}<br>"

        self.send_error_email(["lewis@arched.dev"], error_text=error_text, exception=exception, allowed_minutes=60)
