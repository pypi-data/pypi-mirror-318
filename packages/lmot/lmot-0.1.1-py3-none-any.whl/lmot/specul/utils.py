...
def update_inference_inputs(
    ...
    tgt_len = len(select_indices)
    cache_max_len = past_key_values.get_seq_length()
    if tgt_len > 1:
        #Update the past key values based on the selected tokens
        for idx in range(len(past_key_values)):
            past_key_values.key_cache[idx][..., prev_input_len: prev_input_len + tgt_len, :] = past_key_values.key_cache[idx][..., select_indices, :]
            past_key_values.value_cache[idx][..., prev_input_len: prev_input_len + tgt_len, :] = past_key_values.value_cache[idx][..., select_indices, :]
    #clear post cache
    past_key_values.crop((prev_input_len+tgt_len) - cache_max_len)

    # Update the current length tensor (currently only support batch size is 1)
    current_length_data.fill_(prev_input_len + tgt_len)
