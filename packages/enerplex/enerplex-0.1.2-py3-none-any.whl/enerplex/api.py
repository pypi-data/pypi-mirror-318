...
# Load credentials for the API client from environment (or .env file in development mode)
load_dotenv()
_API_URL: str = os.environ.get("ENERPLEX_API_URL")
...
def _get_auth_token(force_refresh: bool = False) -> str:
    ...
    raw_res = requests.post(f"{_API_URL}/auth/login", data={
...
def get_target_proteins() -> list[DBTargetProtein]:
    ...
    res = _interfere_errors(DataResponse(**requests.get(f"{_API_URL}/data/target-proteins", headers=headers).json()))
...
def get_embeddings() -> list[DBTargetProtein]:
    ...
    """
    headers = {"Authorization": _get_auth_token()}
    res = _interfere_errors(DataResponse(**requests.get(f"{_API_URL}/data/embeddings", headers=headers).json()))
...
def download_target_protein_file(target: DBTargetProtein, path: Path, exists_ok: bool = True) -> None:
    ...
    headers = {"Authorization": _get_auth_token()}
    download_file(
        f"{_API_URL}/data/target-protein/{target.id}/file",
        headers,
        path=path,
        exist_ok=exists_ok
    )
...
def download_ligand_file(target: DBProteinLigandComplex, path: Path, exists_ok: bool = True) -> None:
    ...
    headers = {"Authorization": _get_auth_token()}
    download_file(
        f"{_API_URL}/data/ligand/{target.id}/file",
        headers,
        path=path,
        exist_ok=exists_ok
    )
...
def download_embedding_file(target: DBProteinLigandComplexEmbedding, path: Path, exists_ok: bool = True) -> None:
    ...
    headers = {"Authorization": _get_auth_token()}
    download_file(
        f"{_API_URL}/data/embedding/{target.id}/file",
        headers,
        path=path,
        exist_ok=exists_ok
    )
...
def upload_ligand(
    ...
    res = _interfere_errors(DataResponse(**requests.post(f"{_API_URL}/data/ligand", data=data, files=files, headers=headers).json()))
