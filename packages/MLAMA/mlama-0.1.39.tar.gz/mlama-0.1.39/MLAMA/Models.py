...
def gen_ml(wave,test_length,parameter_length,lag_reserve, predictions, resid):#generate train, test from whole data
...
def arima_res_xgb(ARMAmodel,max_prediction_length, data,train,test,fit_length,cv,training_period, overlap,confidence_interval, lag_reserve, resid,booster,learning_rate, n_estimators):
    test_length = len(test)
    ...
    finaldf_train_resid_x,finaldf_test_resid_x,finaldf_train_resid_y,finaldf_test_resid_y, train_index, test_index = gen_ml(residual,test_length,cv, lag_reserve, max_prediction_length, resid)#I should pass the lag reserve amount here
