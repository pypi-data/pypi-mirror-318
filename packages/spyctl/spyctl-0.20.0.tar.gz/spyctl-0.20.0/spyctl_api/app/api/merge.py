...
class MergeHandlerInput(BaseModel):
    object: PrimaryObject = Field(title="The primary object of the merge")
    merge_objects: List[MergeObject] = Field(
...
def merge(
    ...
    merge_objects = [
        json.loads(obj.model_dump_json(by_alias=True, exclude_unset=True))
        for obj in i.merge_objects
    ]
    cmd_input = cmd_merge.MergeInput(
        json.loads(
            i.object.model_dump_json(by_alias=True, exclude_unset=True)
        ),
        merge_objects,
        i.org_uid,
        i.api_key,
