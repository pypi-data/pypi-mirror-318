...
class Agent_destilation:
    ...
    def DestilationResponseAgent(input, output, instructionsassistant, nameassistant):                        
        ...
        output_path_json2 = os.path.abspath(os.path.join(os.path.dirname(__file__), f'../Destilation/{nameassistant}/ListJson/DestilationAgent{date}'))
        os.makedirs(output_path_json, exist_ok=True)
        os.makedirs(output_path_json2, exist_ok=True)
        ...
        if os.path.exists(output_path_json2):
            with open(output_path_json2, 'r', encoding='utf-8') as json_file:
                try:
                    datasetjson2 = json.load(json_file) 
        ...
        finaloutputjson2 = os.path.join(output_path_json2, f"DestilationDateTime_{date.replace('-', '_').replace(':', '_')}.json")
        with open(finaloutputjson2, 'w', encoding='utf-8') as json_file:
