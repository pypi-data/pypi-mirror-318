...
class SoftwareDevelopment:
    ...
    def QuantumCoreUpdate(
        ...
        main = get_file_content(repo_name, f"{onlyrepo_name}/main.py", branch_name)
        Changelog = get_file_content(repo_name, f"{onlyrepo_name}/Changelog.env", branch_name)
        ...
        if UseVectorstoreToGenerateFiles == True:
            file_paths = [os.getenv("PATH_ANALISE_ENV"), os.getenv("PATH_NAME_DOC_PRE_PROJETO_ENV"), os.getenv("PATH_ROADMAP_ENV"), os.getenv("PATH_PLANILHA_PROJETO_ENV"), os.getenv("PATH_NOME_DO_CRONOGRAMA_ENV")]
            AI_QuantumCore = Agent_files_update.del_all_and_upload_files_in_vectorstore(appfb, client, AI_QuantumCore, "QuantumCore_Work_Environment", file_paths)
            mensaxgem = f"""pense e crie melhorias a main.py do repositorio {repo_name} \n
            com base arquivos `doc.txt`, `Roadmap.txt`, `AnalysisRequirements.txt`, `Schedule.txt`, `Spreadsheet.txt` \n
            que estao armazenados em QuantumCore_Work_Environment melhore com os requisitos funcionais e objetivos \n
            """
        ...
        mensaxgem = f"""Com base nas melhorias que ja pensamos vamos implementar as melhorias no codigo
        codigo:
        {main}
        """
