...
def create_github_repo_and_upload(repo_name: str, 
    ...
    def upload_codes_to_github(directory):
        for dirpath, dirnames, filenames in os.walk(directory):
            for filename in filenames:
                ...
                try:
                    print(f"Falha ao fazer upload do arquivo {file_name}. Status: {response.status_code} {response.content}")
                except:
                    try:
                        print(f"Falha ao fazer upload do arquivo {file_name}. Status: {response.status_code} {response.json()}")
                    except:
                        print(f"Falha ao fazer upload do arquivo {file_name}. Status: {response.status_code} {response.json}")
    ...
    # Upload do arquivo PATH_SOFTWARE_DEVELOPMENT_utils_file_utils_ENV
