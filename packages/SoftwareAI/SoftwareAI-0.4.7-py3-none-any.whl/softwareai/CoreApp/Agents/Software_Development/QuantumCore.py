...
class SoftwareDevelopment:
    ...
    def QuantumCoreUpdate(
        ...
        config = get_file_content(repo_name, f"{onlyrepo_name}/config.py", branch_name)
        setup = get_file_content(repo_name, f"setup.py", branch_name)
        pyproject = get_file_content(repo_name, f"pyproject.toml", branch_name)
        ...
        python_functions.save_TXT(allenvdic, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../", f"environment.txt")), 'x')
        ...
        if UseVectorstoreToGenerateFiles == True:
            ...
            AI_QuantumCore = Agent_files_update.del_all_and_upload_files_in_vectorstore(appfb, client, AI_QuantumCore, "QuantumCore_Work_Environment", file_paths)
            mensaxgem = f"""crie e salve utilizando autosave as melhorias para main.py, config.py, setup.py, pyproject.toml do repositorio {repo_name} \n
