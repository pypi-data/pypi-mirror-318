...
class SimQ(object):
    ...
    def __init__(
        ...
        构造参数:
            db(Redis): redis连接
            connection_pool(ConnectionPool): redis连接池
            prefix(str): 数据前缀
    ...
    async def hset(self, key, data, db=None):
        db = db or self.db or Redis(connection_pool=self.connection_pool)
        mapping = {}
        for item, value in data.items():
            mapping[item] = yaml.safe_dump(value)
        await db.hset(key, mapping=mapping)

    async def hgetall(self, key, db=None):
        db = db or self.db or Redis(connection_pool=self.connection_pool)
        data = {}
        mapping = await db.hgetall(key)
    ...
    async def get_running_keys(self, channel=None, db=None):
        db = db or self.db or Redis(connection_pool=self.connection_pool)
        if channel:
            rkeys = [
                TEXT(x) for x in await db.keys(f"{self.prefix}:running:{channel}:*")
            ]
        else:
            rkeys = [TEXT(x) for x in await db.keys(f"{self.prefix}:running:*")]
        return set(rkeys)

    async def get_id_from_running_key(self, rkey, db=None):
        db = db or self.db or Redis(connection_pool=self.connection_pool)
        ids = await db.lrange(rkey, 0, -1)
        if ids:
            return TEXT(ids[0])
        else:
            return None
    ...
    async def rpush(
        ...
        返回值:
            str: 消息编号。
        """
        db = self.db or Redis(connection_pool=self.connection_pool)
        nowtime = time.time()
        ...
        await self.hset(ikey, msg, db=db)
        await db.rpush(qkey, id)
        return id
    ...
    async def pop(
        ...
        id = await db.brpoplpush(qkey, rkey, timeout=timeout)
        if id is None:
            return None
        id = TEXT(id)
        ikey = self.get_item_key(id=id)
        msg = await self.hgetall(key=ikey, db=db)
        if not msg:
            return None
        # 处理被取消的消息
        if msg.get("cancel_flag", False):
            ...
            rkey = self.get_running_key(channel=channel, running_id=running_id)
            await db.delete(rkey)
            dkey = self.get_done_key(id=msg["id"])
            await db.lpush(dkey, msg["id"])
            await db.expire(dkey, self.ack_event_expire)
            await db.expire(ikey, self.done_item_expire)
            return None
        # 更新消息状态
        update_msg = {
        ...
        await self.hset(ikey, update_msg, db=db)
        return msg
    ...
    async def ack(
        ...
        await db.lpush(dkey, id)
        await db.expire(dkey, self.ack_event_expire)
        await db.expire(ikey, self.done_item_expire)
        return True

    async def drop(self, id: str, db=None):
        """消息丢弃。
    ...
    async def query(
        ...
        ikey = self.get_item_key(id=id)
        if timeout == 0:
            msg = await self.hgetall(ikey, db=db)
            if not msg:
                return None
            return msg
        else:
            dkey = self.get_done_key(id=id)
            await db.brpop(dkey, timeout=timeout)
            msg = await self.hgetall(ikey, db=db)
            if not msg:
                return None
            return msg
    ...
    async def update_worker_status(
        ...
        info = await self.hgetall(wkey, db=db)
        if not info:
            await self.hset(
                wkey,
                {
    ...
    async def recovery(
        ...
        current_rkeys = await self.get_running_keys(channel=channel, db=db)
        ...
        if not last_running_rkeys:
            ...
            for rkey in rkeys_to_check:
                id = await self.get_id_from_running_key(rkey=rkey, db=db)
                ikey = self.get_item_key(id=id)
                msg = await self.hgetall(ikey, db=db)
                # running记录中指定了无效的消息
                # 删除running记录，并记录异常
    ...
    async def running_timeout_handler(
        ...
        if action == "drop":
            ...
            await self.ret(id=id, return_reason=running_timoeut_reason, db=db)
            return True
