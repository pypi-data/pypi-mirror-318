...
class SimQ(object):
    ...
    def hset(self, key, data, db=None):
    ...
    def get_running_keys(self, channel=None, db=None):
        db = db or self.db or Redis(connection_pool=self.connection_pool)
        if channel:
            rkeys = [TEXT(x) for x in db.keys(f"{self.prefix}:running:{channel}:*")]
        else:
            rkeys = [TEXT(x) for x in db.keys(f"{self.prefix}:running:*")]
        return set(rkeys)

    def get_id_from_running_key(self, rkey, db=None):
        db = db or self.db or Redis(connection_pool=self.connection_pool)
        ids = db.lrange(rkey, 0, -1)
        if ids:
            return TEXT(ids[0])
        else:
            return None
    ...
    def pop(
        ...
        id = db.brpoplpush(qkey, rkey, timeout=timeout)
        if id is None:
            return None
        id = TEXT(id)
        ikey = self.get_item_key(id=id)
        msg = self.hgetall(key=ikey, db=db)
        if not msg:
            return None
        # 处理被取消的消息
        if msg.get("cancel_flag", False):
            ...
            db.expire(dkey, self.ack_event_expire)
            db.expire(ikey, self.done_item_expire)
            return None
        # 更新消息状态
        update_msg = {
        ...
        self.hset(ikey, update_msg, db=db)
        return msg
    ...
    def recovery(
        ...
        current_rkeys = self.get_running_keys(channel=channel, db=db)
        ...
        if not last_running_rkeys:
            ...
            for rkey in rkeys_to_check:
                id = self.get_id_from_running_key(rkey=rkey, db=db)
                ikey = self.get_item_key(id=id)
                msg = self.hgetall(ikey, db=db)
                # running记录中指定了无效的消息
                # 删除running记录，并记录异常
    ...
    def running_timeout_handler(
        ...
        if action == "drop":
            ...
            self.ret(id=id, return_reason=running_timoeut_reason, db=db)
            return True
