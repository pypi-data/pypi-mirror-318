...
class Buffer:
    ...
    def dumpStartElement(self, elem, attrs={}, ignoreAttrs=(), hook=False,
        ...
        #       time ;
        # [px]  an Attribute object, representing a special HTML attribute like
        #       "checked" or "selected", that, if the tied expression returns
        #       False, must not be dumped at all. In this case, p_hook must be a
        #       tuple (s_attrName, s_expr). Note that, in this case, p_hook can
        #       also be a list of such tuples.
        ...
        if hook:
            if self.pod:
                ...
                if isinstance(hook, list):
                    for h in hook: self.addAttribute(*h)
                else:
...
class MemoryBuffer(Buffer):
    ...
    def addAttributes(self):
        '''pod-only: adds an Attributes object into this buffer'''
    ...
    def addAttribute(self, name, expr):
        '''px-only: adds an Attribute object into this buffer'''
