...
def is_converged(x_success, z_success, runs, code_params, precission):
    x_cond = _check_convergence(
        x_success, runs, code_params, precission_cutoff=precission
    )
...
#     json_files = [f for f in file_list if f.startswith('per')]

#     # Open the output file for writing
#     with open(directory_path + 'merged_results.json', 'w') as outfile:
#         # Loop through the JSON files and write their contents to the output file
#         for file in json_files:
#             with open(os.path.join(directory_path, file), 'r') as infile:
#                 data = json.load(infile)
#                 json.dump(data, outfile, ensure_ascii=False, indent=4)
#                 outfile.write(",")

#     print(f"All JSON files in {directory_path} have been merged into merged_file.json")
...
def merge_datasets(datasets: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    x_ler, x_ler_eb, x_wer, x_wer_eb = _update_error_rates(
        x_success_cnt, runs, merged_data["code_K"]
    )
    z_ler, z_ler_eb, z_wer, z_wer_eb = _update_error_rates(
        z_success_cnt, runs, merged_data["code_K"]
    )
...
def _combine_xz_data(xdata: Union[dict, None], zdata: Union[dict, None]) -> dict:
    """
