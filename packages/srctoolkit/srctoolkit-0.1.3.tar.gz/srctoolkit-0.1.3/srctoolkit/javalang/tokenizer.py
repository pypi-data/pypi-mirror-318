...
class JavaToken(object):
    ...
    def end_pos(self):
        ...
        line_no = self.position.line
        column_no = self.position.column + len(self.value) - 1
        for line in lines[1:]:
            line_no += 1
            column_no = len(line)
        return Position(line_no, column_no)

    def __repr__(self):
        if self.position:
            return '%s "%s" begin (%d, %d) - end (%d, %d)' % (
                self.__class__.__name__, self.value, self.begin_pos[0], self.begin_pos[1], self.end_pos[0],
                self.end_pos[1]
            )
        else:
