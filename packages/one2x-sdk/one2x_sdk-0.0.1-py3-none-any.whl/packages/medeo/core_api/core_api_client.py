...
from packages.utils.logger import get_default_logger
...
class CoreApiClient:
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(CoreApiClient, cls).__new__(cls, *args, **kwargs)
    ...
    def _build_headers(self):
        headers = {"Cookie": f"medeo-service-auth-token={self.token}"}
        return headers
    ...
    def request(self, method, api_path, params=None, data=None, json=None):
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Response is not in JSON format: {result.text}")
                return result.text
