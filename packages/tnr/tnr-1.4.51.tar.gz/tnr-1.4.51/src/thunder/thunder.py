...
def wait_and_configure_ssh(instance_id, token):
    ...
    and adds it to SSH config when ready.
    """
    ...
    while attempt < max_attempts:
        ...
        if instance["status"] == "RUNNING" and instance.get("ip"):
            ...
            try:
                ...
                # Now that we have SSH access, do all configuration at once
                ...
                _, stdout, stderr = ssh.exec_command("pip install --no-warn-script-location --upgrade tnr && tnr status")
                _ = stdout.read()
