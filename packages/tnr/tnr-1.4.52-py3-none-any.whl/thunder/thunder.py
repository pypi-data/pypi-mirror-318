...
def create(cores, template):
    with DelayedProgress(
        ...
        token = get_token()
        success, error, instance_id = utils.create_instance(token, cores, template)
...
def delete(instance_id):
    with DelayedProgress(
        ...
        progress.add_task("Loading", total=None)  # No description text
        token = get_token()
        get_instances_success, _, instances = utils.get_instances(token, use_cache=False)
        # Get current device IP
        if get_instances_success and instances:
...
def wait_and_configure_ssh(instance_id, token):
    ...
    # First, let's download the binary locally
    binary = get_latest("client", "~/.thunder/libthunder.so", "linux")
    if binary == None:
        logger.error("Failed to download required binary")
        return
    ...
    while attempt < max_attempts:
        ...
        if instance["status"] == "RUNNING" and instance.get("ip"):
            ...
            try:
                ...
                # Now that we have SSH access, transfer the binary via SFTP
                try:
                    sftp = ssh.open_sftp()
                    remote_path = '/home/ubuntu/.thunder/libthunder.so'
                    
                    # Ensure the remote directory exists
                    ssh.exec_command('mkdir -p /home/ubuntu/.thunder')
                    
                    # Transfer the binary
                    sftp.put(binary, remote_path)
                    
                    # Set correct permissions
                    ssh.exec_command(f'chmod 755 {remote_path}')
                    
                    logger.info("Successfully transferred binary to remote instance")
                    sftp.close()
                except Exception as e:
                    logger.error(f"Failed to transfer binary: {e}")
                    return
                ...
                if not stdout.read():
                    cmd = f"""sed -i '/# start tnr setup/i export TNR_API_TOKEN={token}' /home/ubuntu/.bashrc"""
                    ssh.exec_command(cmd)
                    logger.info(f"Added TNR_API_TOKEN to .bashrc for {instance_id}")
                
                # Update TNR
                _, stdout, stderr = ssh.exec_command("pip install --no-warn-script-location --upgrade tnr")
                _ = stdout.read()
                stderr_content = stderr.read()
                if stderr_content:
                    logger.warning(f"TNR upgrade produced warnings: {stderr_content.decode()}")
                logger.info("Upgraded TNR package to the latest version")
                ...
                if error:
                    ...
                    try:
                        with ssh.open_sftp() as sftp:
                            # Define where we want to put the config file
                ...
                # Add TNR run to shell
                cmd = """export TNR_API_TOKEN={token} && /home/ubuntu/.local/bin/tnr run nvidia-smi > /dev/null && . /home/ubuntu/.thunder/setup.sh && tnr activate"""
                ssh.exec_command(cmd)
                _ = stdout.read()
                stderr_content = stderr.read()
                if stderr_content:
                    logger.warning(f"TNR run interactive shell modification produced warnings: {stderr_content.decode()}")
                logger.info("TNR run interactive shell modified")
...
def connect(tunnel, instance_id=None, mount=None, nomount=False):
    ...
    # Check if TNR_API_TOKEN is present
    _, stdout, _ = ssh.exec_command("grep 'export TNR_API_TOKEN' /home/ubuntu/.bashrc")
    if not stdout.read():
        # Add token to .bashrc above the TNR setup section
        cmd = f"""
        sed -i '/# start tnr setup/i export TNR_API_TOKEN={token}' /home/ubuntu/.bashrc
        """
        ssh.exec_command(cmd)
    ...
    env_cmd = f"export TNR_API_TOKEN={token};"
