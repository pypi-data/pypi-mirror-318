...
class SRGAN(ModelTraining):
    ...
    def __init__(
        ...
        print(f"SRGAN training initialized with generator: {self.generator.name}, discriminator: {self.discriminator.name}")
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            train_gen_loss = train_dis_loss = train_cnt_loss =  train_prc_loss = train_mse_loss = train_mae_loss = 0
            ...
            if val_data is not None:
                ...
                val_gen_loss = val_dis_loss = val_cnt_loss = val_prc_loss = val_mse_loss = val_mae_loss = 0
                # Initialize counters for early stopping and learning rate reduction
            ...
            if (epoch + 1) % ckpt_interval == 0:
                ...
                self.generator.save(f"{self.save_path}/{self.expname}_ckpts/generator_e{epoch+1:03d}.keras")
                print(f"\tGenerator checkpoint saved at epoch {epoch + 1}")
                self.discriminator.save(f"{self.save_path}/{self.expname}_ckpts/discriminator_e{epoch+1:03d}.keras")
