...
class ModelTraining:
    ...
    def __init__(
        ...
        self.best_gen_suffix, self.best_dis_suffix= 'ckpt_best_generator', 'ckpt_best_discriminator'
        self.trainlogs = None
    ...
    def train_step(self, input_image, target_image):
        with tf.GradientTape() as tape:  # Start gradient tape for tracking the gradients
            ...
            loss = self.loss_fn(target_image, predictions)  # Compute training loss (tracked by tape)
            mse = self.mse_calc(target_image, predictions)  # Compute Mean Squared Error (tracked by tape)
            mae = self.mae_calc(target_image, predictions)  # Compute Mean Absolute Error (tracked by tape)
        ...
        return loss, mse, mae  # Return the computed loss and metrics
    
    def val_step(self, input_image, target_image):
        predictions = self.generator(input_image, training=False)             
        loss = self.loss_fn(target_image, predictions)
        mse = self.mse_calc(target_image, predictions)
        mae = self.mae_calc(target_image, predictions)
        return loss, mse, mae
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for input_image, target_image in train_dataset.take(train_steps):
                gen_loss, mse_loss, mae_loss = self.train_step(input_image, target_image)
            ...
            if val_data is not None:
                ...
                for input_image, target_image in val_dataset.take(val_steps):
                    gen_loss, mse_loss, mae_loss = self.val_step(input_image, target_image)
                ...
                }.get(monitor, None) # Determine monitored metric

                if monitored_metric is None:
                    valid_options = ["val_loss", "val_mean_absolute_error", "val_mean_squared_error"]
                    raise ValueError(f"Unsupported monitor value: {monitor}. Available options: {', '.join(valid_options)}")
            ...
            if save_ckpt:
                if (epoch + 1) % ckpt_interval == 0:
                    ...
                    self.generator.save(f"{self.save_path}/{self.expname}_ckpt/generator_e{epoch+1:03d}.keras")
    ...
    def train_by_fit(
        ...
        if save_ckpt:
            ...
            model_save_dir = f"{self.save_path}/{self.expname}_ckpt/generator_e{{epoch:03d}}.keras"
    ...
    def generate_data_and_builf_netcdf(
            ...
            X_test, 
            model_path: str = None, 
            refd_path: str = '/nlsasfs/home/precipitation/midhunm/AI4KLIM/DATASET/DATA_P05A/LSM_100_GRDMSK.nc', 
            isgammaloss = False, 
            gamma_thres = 0.5, 
            batch_size = 32, 
            save_raw_npy = False, 
        ...
        if model_path is None:
            ...
            if FLAG:
                print(f"\nGenerating best model test data for {self.expname}...")
            ...
            print(f"\nGenerating test data from {model_path}...")
