...
class MiniRPACore:
    ...
    def get_from_or_update_data(self, file_path: str, file_name: str, sheet_name: str, process_number: int, has_file_condition: bool,
                                flag: str = 'update_file') -> tuple[pd.DataFrame, dict]:
        ...
        if len(target.columns) == 0:
            ...
            target = pd.read_excel(file_obj, sheet_name=sheet_name, dtype=dtype_dict, parse_dates=parse_date_list, date_parser=self.custom_date_parser)
            for date_column in parse_date_list:
                target[date_column] = pd.to_datetime(target[date_column]).dt.date
