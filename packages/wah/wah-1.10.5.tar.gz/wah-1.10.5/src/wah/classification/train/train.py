...
class Wrapper(L.LightningModule):
    ...
    def on_train_epoch_end(self) -> None:
        ...
        # log: eval
        idx: List[Tensor] = self.all_gather(self._train_idx)
        gt: List[Tensor] = self.all_gather(self._train_gt)
        pred: List[Tensor] = self.all_gather(self._train_pred)
        loss: List[Tensor] = self.all_gather(self._train_loss)
        conf: List[Tensor] = self.all_gather(self._train_conf)
        gt_conf: List[Tensor] = self.all_gather(self._train_gt_conf)
        print(len(idx), idx[0].shape)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # log: eval
        idx: List[Tensor] = self.all_gather(self._val_idx)
        gt: List[Tensor] = self.all_gather(self._val_gt)
        pred: List[Tensor] = self.all_gather(self._val_pred)
        loss: List[Tensor] = self.all_gather(self._val_loss)
        conf: List[Tensor] = self.all_gather(self._val_conf)
        gt_conf: List[Tensor] = self.all_gather(self._val_gt_conf)
        idx = process_gathered_data(idx, 2, 1, (1, 0))
        gt = process_gathered_data(gt, 2, 1, (1, 0))
        pred = process_gathered_data(pred, 2, 1, (1, 0))
        loss = process_gathered_data(loss, 2, 1, (1, 0))
