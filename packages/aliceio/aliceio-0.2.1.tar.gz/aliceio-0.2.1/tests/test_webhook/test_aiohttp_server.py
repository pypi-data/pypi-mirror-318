...
class TestOneSkillAiohttpRequestHandler:
    ...
    async def test_verify_skill_id_in_webhook_request(
        ...
        resp = await self.make_request(client=client, command="test", skill_id=skill.id)
        assert resp.status == 200
        assert resp.content_type == "application/json"

        resp = await self.make_request(client=client, command="test", skill_id="kaboom")
    ...
    async def test_reply_into_webhook_unhandled(
        ...
        resp = await self.make_request(client=client, command="spam", skill_id=skill.id)
