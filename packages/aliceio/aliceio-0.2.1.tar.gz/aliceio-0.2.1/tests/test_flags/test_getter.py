...
from aliceio.dispatcher.flags import (
    check_flags,
    extract_flags,
...
class TestGetters:
    def test_extract_flags_from_object(self):
        ...
        func.aliceio_flag = {"test": True}
        assert extract_flags_from_object(func) == func.aliceio_flag
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [{"handler": None}, {}],
            [
                {"handler": HandlerObject(lambda: True, flags={"test": True})},
                {"test": True},
            ],
        ],
    )
    ...
    def test_check_flag(self, flags, magic, result):
        with patch("aliceio.dispatcher.flags.extract_flags", return_value=flags):
