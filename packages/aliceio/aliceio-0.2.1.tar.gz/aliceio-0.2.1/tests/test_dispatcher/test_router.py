...
from aliceio.dispatcher.event.alice import AliceEventObserver
from aliceio.dispatcher.event.bases import UNHANDLED, SkipHandler, skip
...
class TestRouter:
    ...
    def test_observers_config(self):
        ...
        assert (
            router.observers["account_linking_complete_event"]
    ...
    async def test_global_filter_in_nested_router(self):
        ...
        assert await r1.propagate_event(event_type="message", event=None) is UNHANDLED
    ...
    async def test_custom_evenv_nested_router(self):
        ...
        r2 = Router()
        r3 = Router()
        r3.observers["custom-event"] = AliceEventObserver(r3, event_name="custom-event")
        ...
        assert await r1.propagate_event(event_type="custom-event", event=None) is None
        assert (
            await r2.propagate_event(event_type="custom-event", event=None) is UNHANDLED
        )
        assert await r3.propagate_event(event_type="custom-event", event=None) is None
