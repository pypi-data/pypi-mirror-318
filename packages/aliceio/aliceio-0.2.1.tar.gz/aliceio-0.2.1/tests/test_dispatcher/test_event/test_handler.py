...
from aliceio.dispatcher.event.handler import CallableObject, FilterObject, HandlerObject
...
class TestHandlerObject:
    ...
    async def test_check_with_combined_result(self):
        handler = HandlerObject(
            simple_handler,
            [
                FilterObject(lambda value: True),
                FilterObject(lambda value: {"test": value}),
            ],
        )
        result, data = await handler.check(42, foo=True)
        assert result
    ...
    async def test_class_based_handler(self, update: Update):
        class MyHandler(BaseHandler):
            ...
            async def handle(self) -> Any:
                return self.event.session.session_id
        ...
        assert result == update.session.session_id
