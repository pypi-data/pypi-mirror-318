...
from aliceio.dispatcher.event.bases import UNHANDLED, SkipHandler
...
from aliceio.filters import Filter
...
class TestAliceEventObserver:
    def test_register(self) -> None:
        ...
        assert any(
            isinstance(item.callback, MyFilter1)
            for item in observer.handlers[2].filters
        )
    ...
    def test_register_filters_via_decorator(
        self,
        count: int,
    ...
    async def test_trigger_right_context_in_handlers(self) -> None:
        router = Router()
        observer = router.message
    ...
    def test_register_middleware(self, middleware_type):
        event_observer = AliceEventObserver(Router(), "test")

        middlewares = getattr(event_observer, middleware_type)

        @middlewares
        async def my_middleware1(handler, event, data) -> None:
            pass
        ...
        @middlewares()
        async def my_middleware2(handler, event, data) -> None:
            pass
        ...
        async def my_middleware3(handler, event, data) -> None:
            pass
