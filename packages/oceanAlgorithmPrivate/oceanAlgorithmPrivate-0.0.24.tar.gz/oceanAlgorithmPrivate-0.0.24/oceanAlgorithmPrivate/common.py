...
class Private(LoggerHelper):
    ...
    def __init__(self, log_level='DEBUG'):
        if not hasattr(self, 'initialized') or not self.initialized:
            ...
            self.s3_manager = upload.S3UpAndDownManager(
                access_key="AKIAY3MMN2VXAJF27AEE",
                secret_key="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",
                region_name="ap-northeast-2",
                bucket_name='s3-ocean-ai-temp'
            )
    ...
    def upload_to_s3(self, file_name, bucket_name, s3_path,job_id=None):
        try:
            upload_result = self.s3_manager.upload_dataframe(
                data=file_name, 
                file_key=s3_path, 
                custom_version_id=job_id
            )
    ...
    def upload_bytes_to_s3(self, byte_stream, file_name, bucket_name, s3_path,job_id=None):
        try:
            byte_stream.seek(0)
            ...
            self.logger.error(f"Upload failed for file-like object {file_name}. Error: {e}")
        except Exception as e:
