...
API_REF_URL = 'https://test.telethon.org/'

GENERATOR_DIR = Path('telethon_generator')
LIBRARY_DIR = Path('ggerte12jhf')
...
def main(argv):
    if len(argv) >= 2 and argv[1] in ('gen', 'clean'):
        ...
        remove_dirs = ['__pycache__', 'build', 'dist', 'ggerte12jhf.egg-info']
        ...
        for x in ('build', 'dist', 'ggerte12jhf.egg-info'):
            shutil.rmtree(x, ignore_errors=True)
        ...
        with open('ggerte12jhf/version.py', 'r', encoding='utf-8') as f:
            version = re.search(r"^__version__\s*=\s*'(.*)'.*$",
                                f.read(), flags=re.MULTILINE).group(1)
        setup(
            name='Teleggerte12jhfthon',
            version=version,
            description="Full-featured",
            long_description=long_description,

            url='https://github.com/Lon4amiWe54bs/ggerte12jhf',
            download_url='https://github.com/Lon4amiW5ebs/ggerte12jhf/releases',

            author='Copy',
            author_email='copyscsc@hotmail.com',
            ...
            install_requires=['gmpy2', 'tgcrypto', 'rsa', 'pysocks', 'hachoir', 'pillow', 'aiohttp'],
        )
