...
class GitDiffHelper:
    ...
    def ensure_gitignore_entries(self) -> None:
        """
        Check if Git Diff Helper's output files are in .gitignore and add them if not.
        Returns silently if .gitignore doesn't exist or can't be modified.
        """
        # Check if we're in a git repo first
        if self.run_git_command(["git", "rev-parse", "--git-dir"])[1]:
            self.logger.debug("Not in a git repository, skipping .gitignore update")
            return

        gitignore_path = Path('.gitignore')
        gitignore_entries = [
            "\n# Git Diff Helper files",
            "**/diff_file_diffs.md",
            "**/diff_file_originals.md",
            "**/diff_file_updated.md"
        ]
        ...
        try:
            ...
            missing_entries = [
                entry for entry in gitignore_entries[1:]  # Skip the comment line
                if entry not in existing_content
            ]
            
            # Add missing entries if any
            if missing_entries:
                self.logger.info("Adding Git Diff Helper files to .gitignore")
                with open(gitignore_path, 'a') as f:
                    if not existing_content.endswith('\n'):
                        f.write('\n')
                    f.write(gitignore_entries[0] + '\n')  # Write comment
                    for entry in missing_entries:
                        f.write(entry + '\n')
        except Exception as e:
            self.logger.warning(f"Could not modify .gitignore: {e}")
    ...
    def generate_reports(self, scope: str = 'unstaged') -> None:
        ...
        # Ensure output files are gitignored
