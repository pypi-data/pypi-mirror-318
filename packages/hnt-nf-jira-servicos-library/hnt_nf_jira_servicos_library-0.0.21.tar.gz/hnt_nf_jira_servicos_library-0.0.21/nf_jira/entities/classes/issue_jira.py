...
class IssueJira:
    def get_issue_jql(self, jql):
        ...
        while True:
            fields='["key"]'
            params=f"jql={jql}&fields={fields}&startAt={start_at}&maxResults={PAGE_SIZE}"
            url=f"{API_ISSUE_URL}/search?{params}"
            ...
            if len(issues) >= data.get("total", 0):
                break
            start_at += PAGE_SIZE
        result = [issue['key'] for issue in issues]
        logger.info(f"Enter execute get_issue_jql result:{str(result)}")
        return result
