...
from llama_server_client.schema import ChatCompletionRequest, Message, MessageRole, ChatCompletion, ChatCompletionChunk
...
async def send_conversation(client: AsyncLlamaClient, country: str):
    ...
    answer, session_uuid = await collect_stream_response(client, request)
...
async def collect_stream_response(client, request):
    answer_text = ""
    session_uuid = None
    async for part_response in await client.send_chat_completion_request(request):
...
async def main():
    ...
    for _ in range(4):
        client = AsyncLlamaClient('tcp://localhost:5555',timeout=60000)
