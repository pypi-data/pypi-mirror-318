...
def print_ttd(doc_id, selected_printer, company_code):
    ...
    for result in json_obj['dataDetail']:
        nilai = int(result['TTD_NILAI'])  # Ambil nilai sebagai integer
        total_nilai += nilai  # Tambahkan nilai ke total
        barang.rows.append([
            result['TTD_NO'],
            result['TTD_DOKUMEN'],
            result['TTD_DOKUMEN_KETERANGAN'],
            format(nilai, ',d') if nilai != 0 else ""  # Tulis kosong jika nilai == 0
        ])

    barang.rows.append(["", "", "Total", format(total_nilai, ',d')])
