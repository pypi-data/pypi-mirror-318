...
def print_surat_jalan_ttbk(doc_id, selected_printer, company_code):
    ...
    for result in json_obj['barang']:
        barang_ttbk.rows.append([
            result['barangSuratJalan']['MASTER_BARANG_NAMA'],
            ""
        ])
    ...
    ttd.rows.append(["\n", "\n", "\n"])
    ttd.rows.append(["                         ", "                         ", "                         "])
    ttd.rows.append([f"{relasi_name} {supplier_name}".strip(), driver_name,f"{json_obj['user']['USER_NAMA']}"])
