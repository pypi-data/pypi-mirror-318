...
class Tap(ConfigMixin):
    ...
    def _get_client_for_address(self, address: str, use_local_encryption_config: bool=False) -> Client:
        ...
        """
        client = Client()
        if not re.match(r"\w+://", address) and self.remote_scheme is not None:
            # Relative path, use the remote overseer host details
            client.scheme = self.remote_scheme

            # Get overseer path, strip final element
            if self.remote_path:
                overseer_root_path = "/".join(self.remote_path.strip("/").split("/")[:-1])
            else:
                overseer_root_path = ""
            if overseer_root_path:
                client.path = f"/{overseer_root_path}/{address}"
            elif not address.startswith("/"):
                client.path = f"/{address}"
            else:
                client.path = address
