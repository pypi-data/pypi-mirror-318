from .extension import Component, Model, SMatrix, register_model_class
    ...
    def start(self, component: Component, frequencies: Sequence[float], **kwargs) -> ModelResult:
        ...
        if self.ports is None:
            ...
            if not all(name in component.ports for name in names):
                raise RuntimeError(
                    f"Not all port names defined in DataModel match the port names in component "
                    f"'{component.name}'."
                )
        ...
        if self.keys is None:
            ports = tuple(
                f"{name}@{mode}"
                for name in names
                for mode in range(component.ports[name].num_modes)
            )
            ...
            for i, port_in in enumerate(names):
                for j, port_out in enumerate(names):
                    for mode_in in range(component.ports[port_in].num_modes):
                        for mode_out in range(component.ports[port_out].num_modes):
