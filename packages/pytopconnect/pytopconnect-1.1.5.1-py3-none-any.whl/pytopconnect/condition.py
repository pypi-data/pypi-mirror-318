...
class Like:
	def __init__(self, column:pd.Series, params:str, operator:str='', _not:bool=False):
...
class WhereHaving:
	...
	def __init__(self, col:pd.Series, params:str='', like:Like=None, _not:bool=False, **kwargs):
	...
	def having(self,items:dict,options:list=[]):
		...
		for key, value in items.items():
			if self.table is not None:
				...
				key = self.table.get_column(key)
			val = value(key,sql=True)
			if not isinstance(val,Having):
				raise QueryException(f'The data type must be "{type(Having)}"')
			self.parameters += val.params
			func_where += val.func
			if len(options)>0:
				self.parameters += ' '+options[0].upper()+' '
