                ...
                if img_url:
                    ...
                    if annotations:
                        for ann in annotations[0].get("result", []):
                            ...
                            xmin.text = str(ann["value"]["x"])
                            ymin.text = str(ann["value"]["y"])
                            xmax.text = str(ann["value"]["x"] + ann["value"]["width"])
                            ymax.text = str(ann["value"]["y"] + ann["value"]["height"])
                    
                     # 保存XML文件
                    label_filename = f"{img_name}.xml"
                    tree = ET.ElementTree(annotation)
                    tree.write(os.path.join(label_path, label_filename))
                else:
                    print(f"No image found in task {task_id}")
        else:
