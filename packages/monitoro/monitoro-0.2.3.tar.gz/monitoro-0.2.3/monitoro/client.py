        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if response.status_code == 200:
                    ...
                    error = f"Bad request: {response.content}"
                    print(f"Error: {error}")
                    raise BadRequestError(error)
                elif response.status_code == 404:
                    error = f"Monitor or template not found: {response.content}"
                    print(f"Error: {error}")
                    raise MonitorNotFoundError(error)
                elif response.status_code == 500:
                    error = f"Server error: {response.content}"
                    print(f"Error: {error}")
                    raise ServerError(error)
                else:
                    error = f"API request failed with status code {response.status_code}: {response.content}"
                    print(f"Error: {error}")
                    raise MonitoroAPIError(error)
...
class Monitoro(BaseMonitoro):
    def extract(
        ...
        if sum(1 for param in (monitor, template, selectors) if param is not None) != 1:
            raise ValueError(
                "Exactly one of monitor, template, or selectors must be provided"
            )
...
class MonitoroSwarm(BaseMonitoro):
    ...
    def _extract_multiple_swarm(self, endpoint, payload, urls):
        ...
        with ThreadPoolExecutor(max_workers=len(self.api_tokens)) as executor:
            futures = [
                executor.submit(
                    self._extract_single_swarm, (endpoint, payload.copy(), url)
                )
                for url in urls
            ]
