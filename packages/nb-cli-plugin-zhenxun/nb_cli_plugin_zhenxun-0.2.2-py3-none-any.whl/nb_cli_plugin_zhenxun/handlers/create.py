...
async def install_dependencies(
    ...
    click.secho("开始在依赖环境中安装nb-cli等依赖...", fg="yellow")
    proc = await install_package(
        project_path, python_path, "nb-cli nb-cli-plugin-zhenxun poetry", pip_args
    )
    await proc.wait()
    click.secho("依赖环境中nb-cli等依赖安装完成！", fg="yellow")
