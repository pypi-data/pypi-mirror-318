...
class TaskLogger:
    ...
    def log(self, append_log: str):
        ...
        # print(call_info)
        self._log_detail(call_info, append_log, False)
    ...
    def error(self, e: Exception):
        """
            ...
            """
        call_info = self._get_filename_function_line(limit=2)
        self._log_detail(call_info, str(e) if e is not None else "", True)

    def _log_detail(self, call_info, append_log, is_error):
        """
        把东西写入文件中
        :param call_info:
        :param append_log:
        ...
        # 根据最新的要求,无论如何 都要在控制台打印一份出来
        if is_error:
            local_log.error(format_append_log)
        else:
