...
class DiscoveryServiceProxyFactory(Singleton):
    ...
    def get_discovery_service_proxy(self, mode: str = None) -> DiscoveryServiceProxy:
        ...
        if mode == "eureka":
            ...
            self.__instance = K8sDiscoveryServiceProxy.instance()
        else:
...
class K8sDiscoveryServiceProxy(DiscoveryServiceProxy, Singleton):
    """
    这个模式的使用有一定的条件:
    1. 只能工作于  WORK_ON_K8S 模式下
    2. 任务分发必须使用 SCHEDULE 模式

    这种工作模式,就彻底的不向eureka注册了,不依赖与 eureka
    """
    ...
    def do_service(self, service="", return_type="json", app_name="sobeyficus",
        ...
        if work_on_k8s:
            ...
            raise Exception("only work on WORK_ON_K8S mode")
