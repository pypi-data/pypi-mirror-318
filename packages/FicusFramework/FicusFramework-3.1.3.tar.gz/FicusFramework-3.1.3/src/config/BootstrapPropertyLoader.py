...
def init_from_yaml_property(app):
    ...
    try:
        config.eureka_renewal_interval_in_secs = int(yml["eureka"]["instance"]["lease-renewal-interval-in-seconds"])
    except:
        pass

    try:
        if "heartbeat" in yml and "interval" in yml["heartbeat"]:
            config.heartbeat_interval = int(yml["heartbeat"]["interval"])
    except:
        pass
...
def init_from_environ_property():
    ...
    if "eureka.instance.lease-renewal-interval-in-seconds" in environs:
        config.eureka_renewal_interval_in_secs = int(environs["eureka.instance.lease-renewal-interval-in-seconds"])
        log.info(
            f"从系统环境变量中获取到eureka.instance.lease-renewal-interval-in-seconds:{config.eureka_renewal_interval_in_secs}")
    elif "EUREKA_INSTANCE_LEASE_RENEWAL_INTERVAL_IN_SECONDS" in environs:
        config.eureka_renewal_interval_in_secs = int(environs["EUREKA_INSTANCE_LEASE_RENEWAL_INTERVAL_IN_SECONDS"])
        log.info(
            f"从系统环境变量中获取到EUREKA_INSTANCE_LEASE_RENEWAL_INTERVAL_IN_SECONDS :{config.eureka_renewal_interval_in_secs}")

    if "heartbeat.interval" in environs:
        config.heartbeat_interval = int(environs["heartbeat.interval"])
        log.info(f"从系统环境变量中获取到heartbeat.interval:{config.heartbeat_interval}")
    elif "HEARTBEAT_INTERVAL" in environs:
        config.heartbeat_interval = int(environs["HEARTBEAT_INTERVAL"])
        log.info(f"从系统环境变量中获取到HEARTBEAT_INTERVAL:{config.heartbeat_interval}")
