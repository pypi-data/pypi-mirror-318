...
def get_install_application_cmd(config:dict, install_ai_branch:str) -> str:
    ...
    if download_file_from_gitlab(access_token=ACCESS_TOKEN,
        ...
        try:
            ...
            install_cmd = install_cmd + " " + ('-m' if config['install_mode']=='maintainance' else '')
...
def run():
    ...
    if reply:
        cmd = get_install_application_cmd(reply, args.branch)
        if cmd:
            os.system(cmd)
        else:
            import sys
