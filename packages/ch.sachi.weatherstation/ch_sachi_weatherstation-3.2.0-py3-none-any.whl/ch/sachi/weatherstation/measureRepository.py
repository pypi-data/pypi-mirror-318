...
class MeasureRepository:
    ...
    def get_sensors(self) -> List[int]:
        ...
        with conn:
            records = self.__select_and_fetch(conn, 'SELECT distinct(sensorid) from measure')
            sensor_ids = []
            for record in records:
                sensor_ids.append(record[0])
            return sensor_ids

    def get_measures_after(self, sensor_id: int, last: str) -> List[Measure]:
        conn = sqlite3.connect(self.database)
        with conn:
            records = self.__select_and_fetch(conn, 'SELECT m.created_at, m.temperature, m.humidity ' \
                                                    'from measure m ' \
                                                    'where m.sensorid=? and m.created_at >= datetime(?, \'+1 second\')',
                                              (sensor_id, last))
            measures_data = []
            for record in records:
    ...
    def init(self) -> None:
        ...
        with conn:
            if is_migration:
                conn.execute('ALTER TABLE measure RENAME TO oldmeasure')
            ...
            if is_migration:
                conn.execute('''
                    INSERT INTO measure(id, created_at, temperature, humidity, sensorid)
                    SELECT id, created_at, temperature, humidity, -1 from oldmeasure
                ''')
                conn.execute('DROP TABLE oldmeasure')
    ...
    def save(self, measure: Measure) -> bool:
        ...
        with conn:
            ...
            if self.__need_to_persist(last_measure, measure):
                cur = conn.cursor()
                cur.execute('INSERT INTO measure(created_at, temperature, humidity, sensorid) values(?, ?, ?, ?)',
                            (measure.measured_at, measure.temperature, measure.humidity, measure.sensor_id))
    ...
    def __get_last_measure(self, conn: Connection, sensor_id: int) -> datetime:
        cur = conn.cursor()
        cur.execute(
            'SELECT MAX(m.created_at) from measure m where m.sensorid=?', (sensor_id,)
        )
        result = cur.fetchone()
