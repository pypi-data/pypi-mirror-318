...
class RestServiceMeasures:
    ...
    def get_last_timestamp(self, sensor_id: int) -> str:
        response = requests.get(self.url + '/measures/last?sensor=' + str(sensor_id), headers=self.headers, timeout=10)
        if response.ok:
            str_response = response.content.decode('utf-8')
            logging.debug(str_response)
            if str_response:
    ...
    def post_measures(self, sensor_id: int, measures: List[Measure]) -> None:
        measures_data = []
        for measure in measures:
            data = measure.to_json(sensor_id)
