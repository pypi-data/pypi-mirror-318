...
class Main:
    ...
    def run(self) -> None:
        ...
        try:
            posted_measures = 0
            sensors = self.repo.get_sensors()
            for sensor_id in sensors:
                logging.info('Posting for ' + str(sensor_id))
                try:
                ...
                measures_to_post = self.repo.get_measures_after(sensor_id, last)
                measures_per_sensor = len(measures_to_post)
                logging.info('Trying to post ' + str(measures_per_sensor) + ' measures')
                if measures_per_sensor > 0:
                    logging.info('Posting ' + str(measures_per_sensor) + " for sensor '" + str(sensor_id) + "'")
                    try:
                        self.service.post_measures(sensor_id, measures_to_post)
                        posted_measures += measures_per_sensor
                    except Exception as postEx:
                        logging.error("Error occurred when posting measures for " + str(sensor_id) + ": " + str(postEx))
