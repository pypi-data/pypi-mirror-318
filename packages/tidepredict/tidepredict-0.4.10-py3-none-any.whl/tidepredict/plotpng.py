...
class Plotpng():
    ...
    def get_tides(self):
        ...
        df.to_csv(constants.CSVFILE, float_format='%.2f', index_label='DateTime')
        print(df.head())
        extrema_df = pd.DataFrame(extrema)
        extrema_df.index = extrema_df['time'].values.astype(np.int64) // 10 ** 9
        extrema_df.drop(columns=['time'], inplace=True)
        extrema_df.to_csv(constants.EXTRMFILE, float_format='%.2f', index_label='DateTime')
        print(extrema_df)
