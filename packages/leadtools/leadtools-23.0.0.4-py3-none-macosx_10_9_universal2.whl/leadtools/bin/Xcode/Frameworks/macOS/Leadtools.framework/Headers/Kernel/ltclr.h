...
typedef struct _YUV_PARAMS
{
   ...
   L_UINT uOffsetCount; // number of values in the pOffsets array
}
...
/* Convert an array of CMYK planes contained in ppSrcBitmapArray and stores the output in pDstBitmap.
   uStructSize is size of pDstBitmap structure and should be set to sizeof(BITMAPHANDLE).
   All the bitmaps in ppSrcBitmapArray must be grayscale 8 bits/pixel and must have the same image size.
   uSrcBitmapArrayCount indicates the number of bitmaps in ppSrcBitmapArray and should be 4 or 5.
   This is designed to be used together with L_LoadFileCMYKArray.
 */
L_LTCLR_API L_INT EXT_FUNCTION L_ClrConvertCMYKArray(L_HANDLE ClrHandle, pBITMAPHANDLE* ppSrcBitmapArray, L_UINT uSrcBitmapArrayCount, pBITMAPHANDLE pDstBitmap, L_UINT uStructSize);
