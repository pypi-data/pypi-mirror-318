...
def solve(matrix):
    ...
    variables = symbols(f"x:{num_vars}")
    equations = [Eq(sum(row[i] * variables[i] for i in range(num_vars)), row[-1]) for row in matrix]
    solution = linsolve(equations, *variables)
    if solution:
        return list(solution.args[0])
    else:
        return None
...
def Fourier(func,N=10):
    def F(t):      
        def a(n):
            return (1/np.pi)*integral(lambda x:func(x)*np.cos(n*x),-np.pi,np.pi)       
        def b(n):
            return (1/np.pi)*integral(lambda x:func(x)*np.sin(n*x),-np.pi,np.pi)
        S=a(0)/2
        for i in range(1,N+1):
            S+=a(i)*np.cos(i*t) + b(i)*np.sin(i*t)
        return S 
    return F
