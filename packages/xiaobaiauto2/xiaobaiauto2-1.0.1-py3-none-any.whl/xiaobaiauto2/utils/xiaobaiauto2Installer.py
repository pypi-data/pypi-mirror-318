...
from os import remove, path, popen, rename, cpu_count
import platform, sys, subprocess, ssl, tarfile, ctypes
from xiaobaiauto2.utils.xiaobaiauto2Downloader import download_thread
    ...
    if url not in [None, '']:
        try:
            if 'unzip' not in kwargs.keys() and 'dst_path' in kwargs.keys():
                ...
                download_thread(url=url, filename=kwargs.get('dst_path') + '/' + download_filename, t=cpu_count()*3)
            else:
                # urlretrieve(url, download_filename, back)
                download_thread(url=url, filename=download_filename, t=cpu_count()*3)
