...
class EventStorageCursor:
    ...
    def search_relapsed_events(self, group_name, severity, ttl, is_target):
        ...
        * there is record in ``events_thresholded`` table with ``events_thresholded.createtime >= thresholds.relapsetime``
        ...
        """
        self.cursor.execute(
            f"SELECT events_json.id, events_json.event, ARRAY_AGG(events_thresholded.keyid) AS keyids FROM events_json INNER JOIN events_thresholded ON events_json.id = events_thresholded.eventid INNER JOIN thresholds ON events_thresholded.keyid = thresholds.id WHERE events_thresholded.groupname = %s AND events_thresholded.eventseverity = %s AND events_thresholded.createtime >= thresholds.relapsetime AND events_thresholded.istarget IS {'TRUE' if is_target else 'NOT TRUE'} AND thresholds.ttltime <= %s GROUP BY events_json.id",
