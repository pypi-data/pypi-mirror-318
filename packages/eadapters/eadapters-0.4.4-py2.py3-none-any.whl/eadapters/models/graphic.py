...
class EGraphic(base.EBase):
    ...
    def _build(cls, model, map):
        ...
        for name, size in (
            ...
            model[name_i] = model[name_i] or cls._get_image(
                model, size=str(size), strict=False
            )
    ...
    def _get_image(cls, model, index=1, size="thumbnail", strict=True):
        ...
        for image in images:
            if not image["order"] == index and strict:
                continue
            return image
        return None
    ...
    def _get_images(cls, model, size="thumbnail", sort=True):
        ...
        for image in images:
            if not image or not isinstance(image, dict):
                continue
            if not image["size"] in sizes:
                continue
            _images.append(image)
        if sort:
            _images.sort(key=lambda item: item["order"])
        return _images
