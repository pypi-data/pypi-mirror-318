...
See also the corresponding user manual page for further information."""
    ...
    'outlines_enabled() -> bool\n\nEnable depth aware outlines.\n\nDefault: ``False``'
    outlines_depth_from: float = 0.5
    'outlines_depth_from() -> float\n\nSpecifies the minimum depth difference required to draw an outline with the minimum width. The outline width is linearly interpolated from `outlines_min_width` to `outlines_max_width` as the depth difference varies between `outlines_depth_from` and `outlines_depth_to`.\n\nDefault: ``0.5``'
    outlines_depth_to: float = float('inf')
    "outlines_depth_to() -> float\n\nSpecifies the depth difference at which the outline reaches its maximum width. The outline width is linearly interpolated from `outlines_min_width` to `outlines_max_width` as the depth difference varies between `outlines_depth_from` and `outlines_depth_to`. \n\nA special value of `float('inf')` can be used to enable 'uniform width mode,' where `outlines_depth_to` is set equal to `outlines_depth_from` and the outline width remains constant, equal to `outlines_min_width`.\n\nDefault: ``float('inf')``"
    outlines_min_width: int = 1
    'outlines_min_width() -> int\n\nMinimum line width used for outlines at a depth difference of `outlines_depth_from` or below. Outline width is interpolated linearly from `outlines_min_width` to `outlines_max_width` as the depth difference ranges from `outlines_depth_from` to `outlines_depth_to`.\n\nDefault: ``1``'
    outlines_max_width: int = 4
    "outlines_max_width() -> int\n\nMaximum line width used for outlines at a depth difference of `outlines_depth_to` or greater. Outline width is interpolated linearly from `outlines_min_width` to `outlines_max_width` as the depth difference ranges from `outlines_depth_from` to `outlines_depth_to`. This value is ignored in 'uniform width mode', see `outlines_depth_to` for details.\n\nDefault: ``4``"
    outlines_use_custom_color: bool = False
    'outlines_use_custom_color() -> bool\n\nEnable custom outline color. When False (default), the outline color is set automatically based on the background color: white outlines for dark backgrounds and black outlines for light backgrounds.\n\nDefault: ``False``'
    outlines_color: Color = (0.0, 0.0, 0.0)
    'outlines_color() -> tuple[float, float, float]\n\nRGB outline color (without alpha channel). This setting is only used when `outlines_use_custom_color` is set to ``True``.\n\nDefault: ``(0.0, 0.0, 0.0)``'
...
rendering backends from other vendors.
 ...
 In case your system has multiple CUDA-capable devices, you can select a specific one for rendering using the `CUDA_VISIBLE_DEVICES <https://developer.nvidia.com/blog/cuda-pro-tip-control-gpu-visibility-cuda_visible_devices/>`__ environment variable."""
    ...
    "outlines_depth_to() -> float\n\nSpecifies the depth difference at which the outline reaches its maximum width. The outline width is linearly interpolated from `outlines_min_width` to `outlines_max_width` as the depth difference varies between `outlines_depth_from` and `outlines_depth_to`. \n\nA special value of `float('inf')` can be used to enable 'uniform width mode', where `outlines_depth_to` is set equal to `outlines_depth_from` and the outline width remains constant, equal to `outlines_min_width`.\n\nDefault: ``float('inf')``"
    outlines_min_width: int = 1
    'outlines_min_width() -> int\n\nMinimum line width used for outlines at a depth difference of `outlines_depth_from`. Outline width is interpolated linearly from `outlines_min_width` to `outlines_max_width` as the depth difference ranges from `outlines_depth_from` to `outlines_depth_to`.\n\nDefault: ``1``'
    outlines_max_width: int = 4
    "outlines_max_width() -> int\n\nMaximum line width used for outlines at a depth difference of `outlines_depth_to` or greater. Outline width is interpolated linearly from `outlines_min_width` to `outlines_max_width` as the depth difference ranges from `outlines_depth_from` to `outlines_depth_to`. This value is ignored in 'uniform width mode', see `outlines_depth_to` for details.\n\nDefault: ``4``"
    outlines_use_custom_color: bool = False
    'outlines_use_custom_color() -> bool\n\nEnable custom outline color. When False (default), the outline color is set automatically based on the viewport background color: white outlines for dark backgrounds and black outlines for light backgrounds.\n\nDefault: ``False``'
    outlines_color: Color = (0.0, 0.0, 0.0)
    'outlines_color() -> tuple[float, float, float]\n\nRGB outline color (without alpha channel). This setting is only used when `outlines_use_custom_color` is set to ``True``.\n\nDefault: ``(0.0, 0.0, 0.0)``'
