...
class UnrealCv_API(object):
    ...
    Examples:
        >>> unrealcv = UnrealCv_API(9000, '127.0.0.1', (640, 480)) # connect to the UnrealCV server
        >>> unrealcv.config_ue((320, 240), low_quality=True) # Configure Unreal Engine settings
        >>> unrealcv.init_map() # Get initial camera configurations and building a color dictionary for objects.
        >>> img = unrealcv.get_image(0, 'lit', show=True) # Get an image from a camera and display it via OpenCV
    """
    ...
    def init_map(self):
        """
        Get initial camera configurations and building a color dictionary for objects.
        """
