...
class RunUnreal():
    ...
    Example:
        Note: The ENV_BIN path to the binary is relative to the UnrealEnv in gym-unrealcv directory
        >>> ue_binary = RunUnreal(ENV_BIN='UE4_ExampleScene_WindowsNoEditor\\UE4_ExampleScene\\Binaries\\Win64\\UE4_ExampleScene', ENV_MAP='Greek_Island')
        >>> env_ip, env_port = ue_binary.start(resolution=(640, 480), sleep_time=5) % simple start
        >>> env_ip, env_port = ue_binary.start(resolution=(640, 480), gpu_id=1, offscreen=True, sleep_time=5) % offscreen rendering with gpu 1 in a multi-gpu system
        >>> env_ip, env_port = ue_binary.start(docker=True, resolution=(640, 480), local_host=True, sleep_time=5) % start in docker with local host networking
        >>> ue_binary.close() % close the environment
    """
...
class RunDocker():
    def __init__(self, path2env, image='zfw1226/unreal:latest'):
        ...
        os.system('xhost +') # allow docker to access the display
        self.image = image
