...
from .utils.progress import LoadProgress, VerilogLoaderCallback
...
def __load_verilog(file) -> obj.Design:
    ...
    try:
        with LoadProgress() as load_progress:
            Callback = VerilogLoaderCallback(load_progress, Lock())
            design = fromVerilog(
                file,
                cb_input=Callback.input,
                cb_token=Callback.token,
            )
    except ImportError:
