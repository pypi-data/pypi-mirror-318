...
@dataclass
class VerilogLoaderCallback:
    load_progress: LoadProgress
    process_lock: Lock

    def input(self, lexer: Lexer):
        if not isinstance(lexer.lexdata, str):
            raise ValueError("lexer.lexdata should be a string")
        self.process_lock.acquire()
        id = self.load_progress.add("Verilog")
        task = self.load_progress.task(id)
        task.total = lexer.lexdata.count("\n") + 1
        lexer.task = task  # type: ignore
        self.process_lock.release()

    def token(self, lexer: Lexer, token: LexToken):
        self.process_lock.acquire()
