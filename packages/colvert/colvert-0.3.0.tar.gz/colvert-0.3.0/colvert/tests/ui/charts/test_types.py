...
def test_type_float():
    ...
    assert t.render(None, []) == '<label for="title" class="form-label">Title</label><input value="1" type="number" step="0.1" id="title" name="title" class="form-control" onchange="document.getElementById(\'results\').dispatchEvent(new Event(\'sql-change\'))">'


def test_type_int():
    t = OptionTypeInt("title", label="Title")
    assert t.render(2, []) == '<label for="title" class="form-label">Title</label><input value="2" type="number" step="1" id="title" name="title" class="form-control" onchange="document.getElementById(\'results\').dispatchEvent(new Event(\'sql-change\'))">'
    t = OptionTypeInt("title", label="Title", default=5, max=100)
    assert t.render(None, []) == '<label for="title" class="form-label">Title</label><input value="5" type="number" max="100" step="1" id="title" name="title" class="form-control" onchange="document.getElementById(\'results\').dispatchEvent(new Event(\'sql-change\'))">'
...
def test_type_result_column_ignore():
    t = OptionTypeResultColumn("title", label="Title", ignore_columns=["world"])
    assert t.render("hello", ["hello", "world"]) == '<label for="title" class="form-label">Title</label><select autocomplete="on" id="title" name="title" class="form-control" onchange="document.getElementById(\'results\').dispatchEvent(new Event(\'sql-change\'))"><option value=""></option><option value="hello" selected>hello</option></select>'
