...
class OptionTypeInt(BaseOptionType):
    def __init__(self, name: str, label: str, step: int=1, min=None, max=None, **kwargs) -> None:
        self.step = step
        self.min = min
        self.max = max
        super().__init__(name, label, **kwargs)
...
class OptionTypeResultColumn(BaseOptionType):
    ...
    """
    def __init__(self, name: str, label: str, ignore_columns: List[str]=[]) -> None:
    ...
    def render(self, value, result_columns: List[str]) -> str:
        ...
        for result_column in result_columns:
            if result_column.lower() in self._ignore_columns:
                continue
            elif value == result_column:
                selected = "selected"
            else:
                selected = ""
