...
from ipulse_shared_base_ftredge import (DuplicationHandling,
                                        ...
                                        LogLevel as LVL,
                                        Action as A,
                                        ProgressStatus as PS,
                                        StructLog as SLog,
                                        log_info, log_warning, log_error, log_by_lvl)
...
def get_secret_from_gcp_secret_manager_extended(
    ...
    try:
        ...
        if response and response.payload and response.payload.data:
            ...
            done_log=SLog(level=LVL.INFO, action=A.READ, resource=DR.SECRET_MANAGER,  progress_status= PS.DONE, q=1, description=result["status"]["metadata"])
            result["status"]["execution_state"] += f">>{done_log.getEvent()} : {secret_id}"
            result["status"]["overall_status"] = PS.DONE
...
def read_file_from_gcs_extended(storage_client:GCSClient, bucket_name:str, file_path:str,
    ...
    try:
        ...
        handle_pipeline_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE,
                                   pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
    return result
...
def write_file_to_gcs_extended(storage_client: GCSClient,
    ...
    cloud_storage_ref = DR.GCS.value
    ...
    try:
        ...
        result["status"]["overall_status"] = PS.DONE if success else PS.FAILED
    except Exception as e:
        handle_pipeline_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_PERSIST_WRITE_IN_CLOUD_STORAGE_FAILED,
                                    pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
...
def create_bigquery_table_extended(project_id: str,
    ...
    result = {
        "data": None,
        "status":{
        "action_type": Action.PERSIST_CREATE_TABLE,
    ...
    try:
     ...
     # Check if the TABLE exists before attempting to delete it
        ...
        handle_pipeline_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_PERSIST_CREATE_DB_TABLE_FAILED,
                                   pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
    return result
...
def write_load_from_json_into_bigquery_extended(project_id: str,
    ...
    try:
        ...
        handle_pipeline_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_PERSIST_WRITE_IN_DB_FAILED,
                                   pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
...
def write_merge_batch_into_bigquery_extended(
    ...
    try:
        ...
        if not error_already_logged:
            handle_pipeline_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_PERSIST_MERGE,
                                    pipelinemon=pipelinemon,
                                    logger=logger, print_out=print_out, raise_e=raise_e) 
    ...
    ############################################################   
        ...
        if temp_table_name:
            try:
                ...
                handle_pipeline_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_PERSIST_DELETE_DB_TABLE_FAILED,
                                          pipelinemon=pipelinemon,
                                           logger=logger, print_out=print_out, raise_e=raise_e)
...
def write_create_bigquery_table_from_json_extended(
    ...
    try:
        ...
        handle_pipeline_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_PERSIST_CREATE_AND_LOAD_DB_TABLE_FAILED,
                                   pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
...
def read_query_sql_bigquery_table_extended( project_id: str,
    ...
    try:
        ...
        handle_pipeline_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_READ_DB,
                                    pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
