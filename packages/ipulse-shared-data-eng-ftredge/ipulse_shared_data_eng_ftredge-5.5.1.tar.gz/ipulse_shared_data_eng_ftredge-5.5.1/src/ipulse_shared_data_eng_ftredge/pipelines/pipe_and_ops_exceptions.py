...
#TODO Check if Task can also be PipelineSequence , PipelineGroup
...
def handle_pipeline_operation_exception(
    e: Exception,
    result: Union[Dict[str, Any], OpResult],
    ...
    if isinstance(result, OpResult):
        ...
        result_status_info = result.info
    elif isinstance(result, dict):
        # Legacy dict handling
        result["status"]["execution_state"] += ">>EXCEPTION "
        result["status"]["overall_status"] = ProgressStatus.FAILED
