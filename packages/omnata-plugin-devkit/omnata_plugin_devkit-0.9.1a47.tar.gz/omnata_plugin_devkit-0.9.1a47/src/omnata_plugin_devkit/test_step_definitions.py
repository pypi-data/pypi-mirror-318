...
@step("we deploy the plugin as a temporary native app")
def step_impl_deploy_native_app(context):
    if context.plugin_class is None:
        raise ValueError(
            'You must define which plugin class and module is to be used ("we use the x class from the y module")'
        )
    context.plugin_class_upper = context.plugin_class.__name__.upper()
    if not hasattr(context, "session"):
        step_impl_attach_snowpark_session(context)
    snowpark_session:Session = context.session
    # create PLUGIN_TESTING_TEMP database if it doesn't exist
    testing_database = "PLUGIN_TESTING_TEMP"
    snowpark_session.sql(f"CREATE DATABASE IF NOT EXISTS {testing_database}").collect()
    testing_code_schema = "CODE_SCHEMA"
    snowpark_session.sql(f"CREATE SCHEMA IF NOT EXISTS {testing_database}.{testing_code_schema}").collect()

    context.temp_timestamp_part = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
    snowpark_session.sql(f"CREATE SCHEMA IF NOT EXISTS {testing_database}.DATA").collect()
    # generate a temporary name for the application package and application
    context.application_package_name = f"PLUGIN_TEST_PACKAGE_{context.plugin_class_upper}_{context.temp_timestamp_part}"
    context.application_name = f"PLUGIN_TEST_APP_{context.plugin_class_upper}_{context.temp_timestamp_part}"
    context.external_access_integration_name = f"PLUGIN_TEST_EAI_{context.plugin_class_upper}_{context.temp_timestamp_part}"
    context.other_secrets_name = f"PLUGIN_TEST_OTHER_SECRETS_{context.plugin_class_upper}_{context.temp_timestamp_part}"
    context.network_rule_name = f"PLUGIN_TEST_NETWORK_RULE_{context.plugin_class_upper}_{context.temp_timestamp_part}"
    context.application_version = "DEVELOPMENT"
    context.plugin_path = os.path.join(context._root["config"].base_dir, "..")  # pylint: disable=protected-access
    uploader = PluginUploader(snowflake_connection=snowpark_session)
    plugin_path = Path(context.plugin_path)
    uploader.upload_plugin(
        plugin_directory=plugin_path.resolve(),
        database_name=testing_database,
        schema_name=testing_code_schema,
        stage_name=context.plugin_class_upper,
        log_level="DEBUG"
    )
    packager = NativeAppPackaging(
        snowflake_connection=snowpark_session,
        package_name=context.application_package_name,
    )
    packager.create_package(developer_role=None)
    patch_number = packager.create_package_version(
        code_database=testing_database,
        code_schema=testing_code_schema,
        code_stage=context.plugin_class_upper,
        version_name="TESTING",
    )
    created = packager.deploy_application(
        application_name=context.application_name,
        version_name=context.application_version,
        patch_number=patch_number,
    )
    assert created, f"Failed to create application {context.application_name} from package {context.application_package_name} version {context.application_version} patch {patch_number}"

@step("we configure external access for the native app with the following parameters")
def step_impl_configure_external_access_integration(context):
    connection_secrets = {}
    network_addresses = []
    if len(context.table) != 1:
        raise ValueError("Expected a single row in the table")
    for row in context.table:
        if row["Property"] == "connection_secrets":
            connection_secrets = json.loads(row["Value"])
        if row["Property"] == "network_addresses":
            network_addresses = json.loads(row["Value"])
    # create a secret
    snowpark_session:Session = context.session
    snowpark_session.sql(f"call {context.application_name}.PLUGIN.CREATE_GENERIC_SECRET_OBJECT(?,?)",
        [
            f'DATA.{context.other_secrets_name}',
            json.dumps(connection_secrets)
        ]
    ).collect()
    # create a network rule
    snowpark_session.sql(f"call {context.application_name}.PLUGIN.CREATE_NETWORK_RULE_OBJECT(?,PARSE_JSON(?)::array)",
        [
            f'DATA.{context.network_rule_name}',
            json.dumps(network_addresses)
        ]
    ).collect()
    # create an external access integration
    snowpark_session.sql(f"""create or replace external access integration {context.external_access_integration_name} 
        allowed_network_rules = ({context.application_name}.DATA.{context.network_rule_name})
        allowed_authentication_secrets = ({context.application_name}.DATA.{context.other_secrets_name})
        enabled = true
    """).collect()

    snowpark_session.sql(f"""GRANT USAGE ON INTEGRATION {context.external_access_integration_name} 
                         TO APPLICATION {context.application_name}""").collect()
    # now we do the bindings
    callout_parameters = {
        context.external_access_integration_name: {
            "other_secret": context.other_secrets_name,
        }
    }
    snowpark_session.sql(f"call {context.application_name}.PLUGIN.UPDATE_API_CONFIGURATION(?)",
        [
            json.dumps(callout_parameters)
        ]
    ).collect()
