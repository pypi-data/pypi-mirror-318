...
def init_new_package(package_name: str) -> None:
    """Clone domainname files into `package_name` and run `make init` in it."""
    new_package_path = Path.cwd() / package_name

    pipe = Popen(  # noqa: S603
        ['git', 'clone', 'https://github.com/albertas/domainname', new_package_path],  # noqa: S607
        stdin=PIPE,
        stdout=PIPE,
    )
    pipe.communicate()[0]
...
def main() -> None:
    ...
    if parsed_args.version:
        print(f'domainname {__version__}')  # noqa: T201
