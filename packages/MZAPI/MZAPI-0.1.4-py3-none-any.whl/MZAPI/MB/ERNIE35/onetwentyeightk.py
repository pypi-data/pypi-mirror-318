...
class EightK_preview:
    """
    EightK_preview 类用于与百度ERNIE 3.5 128K模型进行交互。
    ...
    def get_response(
        ...
        """
        with self.tracer.start_as_current_span("ERNIE-3.5-128K") as span:
            access_token = self.access_token.get_access_token()
            url = (
                "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/ernie-3.5-128K?access_token="
                + access_token
            )
            payload = json.dumps(
                {
            ...
            self.log.start_process_log(response.json(), "ERNIE-3.5-128K", W)
            M = response.json()
            uuid1 = Request_id
            question = data
            answer = M.get("result")
            ...
            self.sql.get_response(uuid1, question, answer, W, self.ip, "ERNIE-3.5-128K")
            return X
