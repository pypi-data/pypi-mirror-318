    ...
    def __init__(self):
        action = AppActionEntity(
            ...
            dataSchema={
                ...
                "properties": {
                    "oneOf": [
                        {"contactId": {"type": "string", "title": "Contact ID", "description": "The ID of the contact"}},
                        {"contactEmail": {"type": "string", "title": "Contact Email", "description": "The email of the contact", "format": "email"}},
                        {"contactPhone": {"type": "string", "title": "Contact Phone", "description": "The phone of the contact", "format": "tel"}},
                    ]
                },
            },
            uiSchema={
    ...
    async def run(self, context: StepContext, app: AppDefinition, credentials: AppCredentials | None, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if credentials.credentialsType == "apikey":
            ...
            if contactId is not None:
                async with http_client.stream("GET", f"https://api.trata.ai/v1/prospects/{contactId}", headers={"Authorization": f"Bearer {api_key}"}) as response:
                    response.raise_for_status()
                    contact_data = await response.json()
            elif contactEmail is not None:
                async with http_client.stream("GET", f"https://api.trata.ai/v1/prospects?email={contactEmail}", headers={"Authorization": f"Bearer {api_key}"}) as response:
                    response.raise_for_status()
                    contact_data = await response.json()
            elif contactPhone is not None:
                async with http_client.stream("GET", f"https://api.trata.ai/v1/prospects?phone={contactPhone}", headers={"Authorization": f"Bearer {api_key}"}) as response:
                    response.raise_for_status()
                    contact_data = await response.json()
            return contact_data
        else:
