...
def kill_routine(command, routines):
    ...
    def attempt_termination(proc):
        if proc.is_running():
            try:
                Logger.log.info(f"Attempting to send SIGINT to process with PID {proc.pid}...")
                # Send SIGINT first for a graceful shutdown attempt
                if os.name == 'nt':  # For Windows
                    proc.send_signal(signal.CTRL_C_EVENT)  # Send CTRL_C_EVENT signal
                else:  # For Unix-based systems
                    proc.send_signal(signal.SIGINT)  # Send SIGINT signal

                try:
                    proc.wait(timeout=15)
                except psutil.TimeoutExpired:
                    Logger.log.info(f"Process with PID {proc.pid} did not terminate with SIGINT, attempting SIGTERM...")
                    proc.terminate()  # Send SIGTERM
                    try:
                        proc.wait(timeout=15)
                    except psutil.TimeoutExpired:
                        Logger.log.info(f"Process with PID {proc.pid} did not terminate with SIGTERM, attempting SIGKILL...")
        ...
        for routine in routines:
            ...
            if kill and 'process' in routine:
                if not attempt_termination(routine['process']):
                    ...
                    Logger.log.info('Terminate: %s/%s %.2fs DONE!' %
