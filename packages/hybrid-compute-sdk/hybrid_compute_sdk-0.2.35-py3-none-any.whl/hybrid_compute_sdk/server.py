...
class HybridComputeSDK:
    ...
    def gen_response(self, req, err_code, resp_payload):
        ...
        limits = {
            'verificationGasLimit': "0x15000",  # Increased from 0x10000
            'preVerificationGas': "0x15000",    # Increased from 0x10000
        }
        ...
        BASE_GAS = 200000  # Increased base gas
        BYTE_GAS = 800    # Increased per-byte gas
        BUFFER_MULTIPLIER = 1.2  # 20% safety buffer
        raw_call_gas = BYTE_GAS * len(resp_payload) + BASE_GAS
        call_gas = int(raw_call_gas * BUFFER_MULTIPLIER)

        print("call_gas calculation:", {
            'payload_length': len(resp_payload),
            'encoded_length': 4 + len(p_enc2),
            'raw_gas': raw_call_gas,
            'buffered_gas': call_gas
        })
        ...
        gas_fees = ethabi.encode(
            ['uint256', 'uint256'],
            [int(5e9), int(1e9)]  # 5 gwei max fee, 1 gwei priority fee
        )
