...
class HybridComputeSDK:
    def __init__(self):
        ...
        try:
            ...
            self.HC_CHAIN = 28882
            self.HH_ADDR = '0x11c4DbbaC4A0A47a7c76b5603bc219c5dAe752D6'
            self.HA_ADDR = '0x77fbd8f873e9361241161de136ad47883722b971'
            self.HA_OWNER = '0x4B063eF8d8a6d441d56903c9b4130E79E6099383'
            self.hc1_key = '0x1b41b21f86e0ad9955d7c150c76b4369cdee2af208a59946fb348d7d8a1e9413'
    ...
    def gen_response(self, req, err_code, resp_payload):
        ...
        p_enc2 = self.selector_hex("execute(address,uint256,bytes)") + \
            ethabi.encode(['address', 'uint256', 'bytes'], [
                 Web3.to_checksum_address(self.HelperAddr), 0, p_enc1]) # b61d27f6
        ...
        call_gas = max(705*len(resp_payload) + 170000, 200000)  # Ensure minimum 200k

        print("call_gas calculation", len(resp_payload), 4+len(p_enc2), call_gas)

        verificationGasLimit = 500000  # 0x7A120
        callGasLimit = 200000          # 0x30D40

        account_gas_limits = \
            ethabi.encode(['uint128'],[verificationGasLimit])[16:32] + \
            ethabi.encode(['uint128'],[callGasLimit])[16:32]
