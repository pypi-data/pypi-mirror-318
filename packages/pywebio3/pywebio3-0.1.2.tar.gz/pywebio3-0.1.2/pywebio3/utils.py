...
def pyinstaller_datas(cli_args=False):
    ...
    datas = [
        (STATIC_PATH, 'pywebio3/html'),
        (normpath(STATIC_PATH + '/../platform/tpl'), 'pywebio3/platform/tpl')
    ]
    if cli_args:
        args = ''
        for item in datas:
...
def check_webio_js():
    js_files = [os.path.join(STATIC_PATH, 'js', i) for i in ('pywebio3.js', 'pywebio3.min.js')]
    if any(os.path.isfile(f) for f in js_files):
        return
    error_msg = """
Error: Missing pywebio3.js library for frontend page.
This may be because you cloned or downloaded the project directly from the Git repository.

You Can:
  * Manually build the pywebio3.js file. See `webiojs/README.md` for more info.
OR
