...
def _build_key_expression(filter_: Dict[str, Any]):
...
class DynamoDbEngine(BaseEngine):
    ...
    def _do_search_entities(
        ...
        start_key = None
        scan_kwargs = {"KeyConditionExpression": _build_key_expression(keys)}
        if index:
    ...
    def _get_next_version(self, id_: int):
        ...
        if existing:
            max_version = max(ent["version"] for ent in existing)
