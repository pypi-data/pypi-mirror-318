...
def rotate_file(path: pathlib.Path, generations: int = 0) -> None:
    ...
    if generations > 0:
        ...
        for generation in reversed(range(generations)):
            temp_path = path.with_suffix(f".{generation:0{num_length}}{suffix}") if generation else path
            if temp_path.exists():
                temp_path.replace(path.with_suffix(f".{generation + 1:0{num_length}}{suffix}"))
        return

    # when generations is negative, find all backup files
    backup_files = sorted(
        path.parent.glob(f"{path.stem}.[0-9][0-9][0-9][0-9][0-9][0-9]_[0-9][0-9][0-9][0-9][0-9][0-9]{suffix}")
    )
    while len(backup_files) >= -generations:
        backup_files.pop(0).unlink()
    path.replace(path.with_suffix(f".{datetime.datetime.now(datetime.UTC).strftime('%y%m%d_%H%M%S')}{suffix}"))


class FileHandler(IndentedMixin, Handler):
    ...
    def is_rollover_required(self, content) -> bool:
        try:
            return all((self.max_size, ((self.stream.path.stat().st_size + len(content)) > self.max_size)))
