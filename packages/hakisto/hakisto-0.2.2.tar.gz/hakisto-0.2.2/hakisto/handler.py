...
from .severity import CRITICAL, severities, ERROR
...
class Handler(metaclass=ABCMeta):
    ...
    :param name: Handler name. When this is an empty string (the default), it signifies this as
        a ROOT Handler. Providing a ``name`` will use it as (sub-)topic, except when the ``name``
    ...
    def config(self, subject):
        ...
        if "set_severity" in subject:
            self.severity = subject["set_severity"]
        if "set_date_format" in subject:
            self.date_format = subject["set_date_format"]
    ...
    def process(self, subject):
        ...
        if subject.severity in (ERROR, CRITICAL):
    ...
    def handle_exception(self, subject) -> None:
        ...
        for i in subject.traceback:
            content_lines.append(i.source_location)
            content_lines.append(self.format_frame_information(i.frame_information))
