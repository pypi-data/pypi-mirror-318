...
@tree.command()
@click.argument('config_path', type=click.Path(exists=True), default='.')
def show(config_path):
    """
    Show the project tree structure based on config.json.
    CONFIG_PATH: Path to the config.json file or the directory containing it.
    """
    # Automatically append 'config.json' if a directory is provided
    if os.path.isdir(config_path):
        config_path = os.path.join(config_path, 'config.json')

    if not os.path.isfile(config_path):
        click.echo(f"Error: config.json not found at {config_path}")
        return

    try:
        builder = ProjectTreeBuilder(config_path)
