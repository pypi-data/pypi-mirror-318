...
class QuitMenu(object):
    def quit_backup(self,parent):
        DL.DayLogger.DayLogger.addTodayP(db.ENGINE)
        parent.cleanup_system(parent)

    def quit_backup_clear(self,parent):
        DL.DayLogger.DayLogger.addTodayP(db.ENGINE)
        bare_ca(None)
        parent.cleanup_system(parent)

    def tag_quit_backup_clear(self,parent):
    ...
    def __init__(self,parent=None):
        if parent != None:
            htext=f'''
<ENTER>/<RETURN>,h,help,? - help info
q,quit - just quit
qb,quit backup - quit and backup to daylog
qbc,quit backup clear - quit backup to daylog, clear current list values
tqbc,tag quit backup clear - tag daylog values, quit backup to daylog, clear current list values
To Get To This Menu use any of the following:
    'q','qm','q?','quit menu','quit al la carte'
                    '''
            ...
            'help':{
                'cmds':['help','h','?'],
                'exec':lambda htext=htext:print(htext),
            ...
            'quit backup':{
                'cmds':['qb','quit backup',],
                'exec':lambda parent=parent:QuitMenu.quit_backup(None,parent),
                'desc':"quit backup"
                },
            'quit backup clear':{
                'cmds':['qbc','quit backup clear'],
                'exec':lambda parent=parent:QuitMenu.quit_backup_clear(None,parent),
                'desc':"quit backup clear"
                },
            'tag quit backup clear':{
                'cmds':['tqbc','tag quit backup clear'],
                'exec':lambda parent=parent:QuitMenu.tag_quit_backup_clear(None,parent),
            ...
            for i in self.options:
                line=f"{Fore.light_salmon_1}{self.options[i]['cmds']} {Fore.light_sea_green}- {self.options[i]['desc']}"
                htext.append(line)
            htext='\n'.join(htext)
            htext+=f"{Style.reset}"
            ...
            while True:
                ...
                for option in self.options:
                    if self.options[option]['exec'] != None and (cmd.lower() in self.options[option]['cmds'] or cmd in self.options[option]['cmds']):
                        self.options[option]['exec']()
                    elif self.options[option]['exec'] == None and (cmd.lower() in self.options[option]['cmds'] or cmd in self.options[option]['cmds']):
                        return
