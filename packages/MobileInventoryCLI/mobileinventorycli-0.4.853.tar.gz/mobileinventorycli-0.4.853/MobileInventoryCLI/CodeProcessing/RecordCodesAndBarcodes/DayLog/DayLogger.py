		...
		while True:
			try:
				...
				with Session(ENGINE) as session:
					...
					# Create a series
					series = df_from_records[field]

					# Generate x-values for the sine wave
					x = np.arange(0, len(series),step=1,dtype=float)
					...
					plt.plot(x, series, label=f'Original Data From {field} (SR={sr})')
					plt.show()
					plt.clf()

					plt.plot(x, sine_wave, label=f'Signal Generated For {field} (SR={sr})')
					plt.show()
