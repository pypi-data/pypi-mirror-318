...
class AivBot:
    ...
    async def _getApiParam(self, apiName, apiFun):
        ''' 2024.11
            获取Api函数的参数
            参数的配置数据格式如下:  ('type' 可选类型有: 'file'、'text'、'bool'、'combo'、'number')
    ...
    async def _onStartTask(self,task):
        ...
        try:
            ...
            # -----------------------------------------------------------------------
                ...
                warnMsg = f'{self.botInfo.botName} 运行后返回的数据格式必须是:'+ ' {"file": xxx, "text": [xxx]}'
            ...
            errMsg = f'Bot: {self.botInfo.botName} > {apiName}() 出错--->\n {e}'
