...
class BatchSampleReader:
    ...
    def read(self, verbose=False, with_tqdm=True, n_jobs=8, rename_obs_with_sample_name=True):
        if with_tqdm:
        ...
        with ThreadPoolExecutor(n_jobs) as executor:
            ...
            for task, sample_name, row_dic in (
                ...
                if rename_obs_with_sample_name:
...
def batch_read(root: Path | str, *, verbose=False, with_tqdm=True, n_jobs=8, rename_obs_with_sample_name=True):
    ...
    return reader.read(
        ...
        with_tqdm=with_tqdm,
        n_jobs=n_jobs,
        rename_obs_with_sample_name=rename_obs_with_sample_name,
    )
