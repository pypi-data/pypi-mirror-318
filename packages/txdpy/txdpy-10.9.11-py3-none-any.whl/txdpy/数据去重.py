...
class CheckDuplicate:
    ...
    def 值列表删除值(self,删除值:Union[str,list,tuple,set]):
        ...
        :param 删除值:可以是任何类型
                     ...
                     其他类型会被当做单个值删除
        """
        assert type(删除值), "删除值类型必须为：[str,list,tuple,set]"
        # 执行DELETE语句
        if type(删除值) in [list,tuple,set]:
            sql = f"DELETE FROM {self.key} WHERE 值 IN ({', '.join([dumps(x) for x in 删除值])});"
        else:
            sql = f"DELETE FROM {self.key} WHERE 值 = '{dumps(删除值)}';"
    ...
    def 查询所有去重项目名称(self):
        ...
        # 打印所有表名
        logger.info(dumps([table[0] for table in tables]))
