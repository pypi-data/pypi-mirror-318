...
class CheckDuplicate:
    ...
    def 添加值(self,值):
        ...
        data_to_insert = (dumps(值,ensure_ascii=False),)
        # 执行插入数据的 SQL 语句
    ...
    def 值列表删除值(self,删除值:Union[str,list,tuple,set]):
        ...
        if type(删除值) in [list,tuple,set]:
            sql = f"DELETE FROM {self.key} WHERE 值 IN ({', '.join([dumps(x,ensure_ascii=False) for x in 删除值])});"
        else:
            sql = f"DELETE FROM {self.key} WHERE 值 = '{dumps(删除值,ensure_ascii=False)}';"
