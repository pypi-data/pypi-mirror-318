...
async def main(config_name: str = None) -> None:
    ...
    try:
        ...
        config_paths = [
            ...
            os.path.expanduser(f"~/.mcp2mqtt/{config_name}"),  # 用户主目录
        ]
        
        # 添加系统级目录
        if os.name == 'nt':  # Windows
            config_paths.append(os.path.join(os.environ.get("ProgramData", "C:\\ProgramData"),
                                           "mcp2mqtt", config_name))
        else:  # Linux/Mac
            config_paths.append(f"/etc/mcp2mqtt/{config_name}")
        
        # 查找存在的配置文件
        config_path = next((path for path in config_paths if os.path.isfile(path)), None)
        
        if not config_path:
            logger.error(f"Configuration file not found in any of the following locations: {config_paths}")
            raise FileNotFoundError(f"Configuration file not found in any of the following locations: {config_paths}")
