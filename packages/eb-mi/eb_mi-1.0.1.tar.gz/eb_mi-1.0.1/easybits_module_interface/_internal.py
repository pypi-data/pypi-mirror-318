...
class Consumer(ConnectionSetup):
    ...
    def add_callback(self, queue: str, callback: Callable[[Message], None], exchange=None):
        """
        ...
        """
        self.callback_map[queue] = (self.__callback_wrapper(callback), exchange)
    ...
    async def run(self):
        ...
        **Note:** Register callbacks using :py:obj:`~add_callback` before calling this method.
        ...
        try:
            ...
            for queue_name, (callback, exchange_name) in self.callback_map.items():
...
class ModuleInterface:
    ...
    def add_callback(self, queue: str, callback, exchange=None) -> None:
        """
        Add a callback to be called when a message is received on :py:obj:`~queue`.
        ...
        :return: None
        """
        self.__consumer.add_callback(queue, callback, exchange)
...
async def bootstrap_class(module_implementation, **config) -> None:
    """
    ...
    connection = await aio_pika.connect_robust(broker_config['uri'])
...
async def main(module_implementation, **config) -> None:
    ...
    broker.uri        str            Broker uri
    """
    module = await bootstrap_class(module_implementation, **config)
    try:
        await module.run()
    finally:
