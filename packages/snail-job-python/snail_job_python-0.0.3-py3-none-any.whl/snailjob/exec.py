...
class ExecutorManager:
    ...
    def _execute_wrapper(
        ...
        try:
            ...
            result: ExecuteResult = job_method(job_args)
            dispatch_result = DispatchJobResult.build(
                request=request,
                execute_result=ExecuteResult.success() if result is None else result,
            )
            # 添加 ChangedWfContext 到 dispatch_result 中
            if job_args.change_wf_context:
                dispatch_result.wfContext = json.dumps(
                    job_args.change_wf_context.to_dict()
                )
