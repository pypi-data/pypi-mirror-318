...
BEGIN
    INSERT INTO event(id, diff, fs) VALUES (
        ...
        JSON_OBJECT('type', 'insert', 'path', (
            WITH ancestors AS (
                SELECT 1 AS id, parent_id, '/' || REPLACE(name, '/', '|') AS path FROM data WHERE id=NEW.id
                UNION ALL
                SELECT ancestors.id + 1, data.parent_id, '/' || REPLACE(data.name, '/', '|') || ancestors.path FROM ancestors JOIN data ON (ancestors.parent_id = data.id) WHERE ancestors.parent_id
            )
...
BEGIN
    ...
    SELECT *, (
        ...
        SELECT JSON_OBJECT('type', 'update', 'path0', (
            CASE 
                WHEN OLD.parent_id = 0 THEN '/' || REPLACE(OLD.name, '/', '|') 
                ELSE (
                    WITH ancestors AS (
                        SELECT 1 AS id, parent_id, '/' || REPLACE(name, '/', '|') AS path FROM data WHERE id=OLD.parent_id
                        UNION ALL
                        SELECT ancestors.id + 1, data.parent_id, '/' || REPLACE(data.name, '/', '|') || ancestors.path FROM ancestors JOIN data ON (ancestors.parent_id = data.id) WHERE ancestors.parent_id
                    )
                    SELECT path || '/' || REPLACE(OLD.name, '/', '|') FROM ancestors ORDER BY id DESC LIMIT 1
                ) 
            END
        ), 'path', (
            CASE 
                ...
                ELSE (
                    WITH ancestors AS (
                        SELECT 1 AS id, parent_id, '/' || REPLACE(name, '/', '|') AS path FROM data WHERE id=NEW.parent_id
                        UNION ALL
                        SELECT ancestors.id + 1, data.parent_id, '/' || REPLACE(data.name, '/', '|') || ancestors.path FROM ancestors JOIN data ON (ancestors.parent_id = data.id) WHERE ancestors.parent_id
                    )
                    SELECT path || '/' || REPLACE(NEW.name, '/', '|') FROM ancestors ORDER BY id DESC LIMIT 1
                )
            END
...
def insert_dir_items(con, items, commit: bool = False):
    fields: tuple[str, ...] = ("id", "parent_id", "name", "mtime")
    if isinstance(items, dict):
        fields = tuple(frozenset(("id", "parent_id", "name", "mtime")) & items.keys())
    elif items:
        fields = tuple(frozenset(("id", "parent_id", "name", "mtime")) & items[0].keys())
    upsert_items(
        con, 
        items, 
        table="dir", 
...
# TODO: 需要进行优化，使用 p115client.tool.iter_fs_files 进行优化
def iterdir(
    client: P115Client, 
    cid: int = 0, 
    /, 
...
# TODO: 允许全量拉取并发执行，冷却时间为 1 秒
# TODO: 增加一个选项，允许对数据进行全量而不是增量更新，这样可以避免一些问题（需要先把所有数据的 is_alive 置 0）
