...
class Agent(AgentGeneric):
    ...
    def get_file_content_by_action_reference(self, key_name_in_action: str) -> str | None:
        ...
        if not existing_files:
            logging.error(f"No files for tag {key_name_in_action} in {self.task_dir}")
            return None
    ...
    def do_action(self) -> bool:
        ...
        for row in result_set: # for each table
            ...
            if table_name in fully_excluded_file_list:
                continue
            logging.debug(f"  Working with table {table_name}")
            ...
            with temporary_dir_context() as content_temp_dir:
                logging.debug(f"Source files to zip later will be put into {content_temp_dir}")
                ...
                logging.debug(f"Hash of temp-dir content is {package_hash}")
            ...
            if new_row_with_id:
                added += 1
                logging.info(f"Generation of {table_name} is done {new_row_with_id[0][0]}")
        
        logging.info(f"Done generation of {added} items")
        return True
