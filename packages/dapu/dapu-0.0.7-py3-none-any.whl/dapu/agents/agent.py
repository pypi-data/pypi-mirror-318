...
class AgentGeneric:
    ...
    def __init__(self, task_id: str, action: dict, context: DapuContext, route_alias: str):
        ...
        self.task_dir: str = self.find_task_dir_path(must_exists=True) # {work_dir}/{route}/{schema}/{table}, where resides haulwork.yaml
    ...
    def do_action(self) -> bool:
        ...
        """
        logging.error("You are using too general class or subclass don't have do_action() implemented")
        return False
    ...
    def collect_files(self, file_element: str='file') -> list[str]:
        ...
        if file_element not in self.action:
            logging.debug(f"No element {file_element} in action definition") # don't need to be error (consumer decides)
            return []
        def_files = []
        ...
        for file_name in def_files:
            file_local = os.path.join(self.task_dir, file_name)
    ...
    def apply_files_to_target(self, existing_files: list[str], replacements: list[tuple[str | Placeholder, str]] | None) -> bool:
        ...
        for file_full_name in existing_files:
            ...
            sql: str = make_replacements(read_sql_from_file(file_full_name), replacements) # does logging
            if sql is None:
                return False # break on first error
            
            try:
                logging.debug(f"Will apply SQL from file {file_full_name}")
                self.context.target(sql, False)
            except Exception as e1:
                logging.error(f"Applying SQL file failed ({file_full_name})")
                #logging.error(f"{sql}") # deep logging or local logging only (too unsafe for remote logging)
                logging.error(e1)
                return False # break on first error
            logging.debug(f"SQL file {file_full_name} applied")
        return True
    ...
    def find_temp_dir_path(self) -> str:
        ...
        logging.debug(f"Temporary directory path for target is {temp_path_for_target}, if needed")
        return temp_path_for_target


    def find_task_dir_path(self, must_exists: bool=False) -> str:
        """
        Full path from current (internal) task_id (gives 3 directories) and self.context root path (work_dir)
        """
        if not self.task_id:
            logging.error(f"Empty task_id {self.task_id}")
            return None
        if self.context.work_dir is None:
            logging.error(f"Context work_dir is missing for task {self.task_id}")
            return None
        path_way: list = self.task_id.split('.')
        if len(path_way) < 3:
            logging.error(f"Too short task_id {self.task_id}")
            return None
        path: str = self.context.full_name_from_pull(path_way)
        if must_exists and not os.path.exists(path):
            logging.error(f"Path {path} for task '{self.task_id}' not exists in local file system")
            return None
        logging.debug(f"Dir for task {self.task_id} is {path}")
        return path
