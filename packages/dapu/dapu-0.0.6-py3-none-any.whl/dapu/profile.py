from dapu.perks import get_custom_logger

logging = get_custom_logger('dapu')
...
class ProfileHandler:
    ...
    def profile_exists(self, profile_type: str, alias: str) -> bool:
        ...
        """
        logging.debug(f"Request for '{alias}' of type '{profile_type}'")
        list_of_profiles_of_type: list[dict] | None = self.data.get(profile_type, None)
        if not list_of_profiles_of_type:
            logging.debug(f"No profiles of type '{profile_type}'")
            return False # no such type profiles
        for one_profile in list_of_profiles_of_type:
            logging.debug(f"Profile python type is '{type(one_profile)}'")
            logging.debug(f"name is {one_profile.get(self.name_key)}")
    ...
    def profile_add(self, profile_type: str, profile: dict | None) -> bool:
        ...
        """
        logging.debug(f"Adding type {profile_type} profile")
        if profile is None: # no profile, no adding
            logging.error("Profile is none")
            return False
        if not profile.get(self.name_key, None): # badly formed profile
            logging.warning(f"Profile don't have name ('{self.name_key}' is missing)")
            return False
        if not profile_type in self.data: # if first of such profile_type
            self.data[profile_type] = [] # lets make empty list
            logging.debug(f"First of profile_type {profile_type}")
        self.data[profile_type].append(profile) # and append to list
        return True
    
    
    def profiles_set(self, profile_type: str, list_of_profiles: list[dict] | None) -> bool:
        """
    ...
    def profiles_add(self, profile_type: str, list_of_profiles: list[dict] | None) -> int:
        """
