...
class DapuObserver(DapuProcess):
    ...
    def run(self):
        ...
        for task_id in unused:
            if self.is_existing_task_id(task_id):
                logging.info(f"asked for info about {task_id}")
                info_objects.append(task_id)
            else:
                logging.info(f"taking {task_id} as option or commandment")
                options.append(task_id)
        all_flag = False if len(info_objects) > 0 else True

        if 'task' in options:
            if not all_flag:
                logging.info(f"Info about tasks")
    ...
    def task_info(self, task_id: str) -> None:
        ...
        result_set = self.context.target(sql, style="name")
        if result_set:
            logging.info(f"{result_set[0]}")

            sql_reconf = f"""SELECT task_option, task_newvalue FROM {reconf} WHERE task_id = '{task_id}'"""
            result_set = self.context.target(sql_reconf, style="name")
            if result_set:
                logging.info("reconf")
                for row in result_set:
                    logging.info(f"{row}")
            else:
                logging.info("NO RECONF")

            sql_dep_master = f"""SELECT task_id_slave FROM {registry_depends} WHERE task_id_master = '{task_id}'"""
            result_set = self.context.target(sql_dep_master, style="name")
            if result_set:
                logging.info("DEPENDANTS")
                for row in result_set:
                    logging.info(f"{row}")
            else:
                logging.info("NO dependants")
            sql_dep_slave = f"""SELECT task_id_master FROM {registry_depends} WHERE task_id_slave = '{task_id}'"""
            result_set = self.context.target(sql_dep_slave, style="name")
            if result_set:
                logging.info("DEPENDS ON")
                for row in result_set:
                    logging.info(f"{row}")
            else:
                logging.info("NOT depending (independent)")
            sql_job_busy = f"""SELECT * FROM {agenda} WHERE task_id = '{task_id}' AND status = {JobStatus.BUSY.value}"""
            result_set = self.context.target(sql_job_busy, style="name")
            if result_set:
                logging.info("CURRENTLY RUNNING")
                for row in result_set:
                    logging.info(f"{row}")
            else:
                logging.info("NO active runs")
            ...
            result_set = self.context.target(sql_job_idle, style="name")
            if result_set:
                logging.info("WAITING")
                for row in result_set:
                    logging.info(f"{row}")
            else:
                logging.info("NO waitings")
            

    def agenda_job_info(self, task_id: str, options: list|None) -> None:
        logging.info(f"Job of task {task_id} is:")


    def agenda_info(self, options: list|None) -> None:
        logging.info(f"All jobs in agenda:")
