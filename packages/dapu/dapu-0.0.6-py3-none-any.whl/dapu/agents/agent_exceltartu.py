...
class Agent(AgentGeneric):
    ...
    def do_action(self) -> bool:
        logging.info(f"start {self.myname} for {self.task_id}")
        route_code, schema_name, table_name = split_task_id(self.task_id)
        if 'source' not in self.action:
            logging.error(f'Missing source (excel file to import data from)');
            return False
        if 'cols' not in self.action:
            logging.error(f'Missing cols (listof columns of target table with instructions how them fullfil from excel)');
            return False
        excel_dir = self.context.profiles.profile_get(self.context.PROFILE_TYPE_FILE, self.route_alias).get('path', None)
        if excel_dir is None:
            logging.error(f"Missing path in profile {self.route_alias} (for task_id '{self.task_id}')")
            return False
        if not os.path.exists(excel_dir):
            logging.error(f"Missing folder '{excel_dir}' (for task_id '{self.task_id}')")
            return False
        if 'name' not in self.action['source']:
            logging.error(f"Missing name of file (action>source>name) in loading definition (for task_id '{self.task_id}')")
            return False
        
        excel_file = os.path.join(excel_dir, self.action['source']['name']) # full name
        if not os.path.exists(excel_file):
            logging.info(f"No file '{excel_file}' (for task_id '{self.task_id}'), may be already consumed, end of pulling")
        ...
        if insert_mode != 'simple':
            logging.error('Realiseerimata insert meetod, jääb katki')
            return False        
    ...
    def load_sheet(self, sheet, min_row, max_row, schema_name, table_name, insert_mode):
        ...
        for row in sheet.iter_rows(min_row=min_row, max_row=max_row, values_only=True):
            ...
            if insert_mode == 'simple':
                ...
                sql_ins = None
            if sql_ins:
                logging.debug(f"  rea {rida} lisamine")
                self.context.target(sql_ins, False)
                logging.debug(f"  rida {rida} lisatud")
            
        
    def calculate_deletion_dates(self, sheet, min_row, max_row, delete_date_column) -> tuple[str, str]:
        excel_col = None
        ...
        for jrk, row in enumerate(sheet.iter_rows(min_row=min_row, max_row=max_row, min_col=excel_pos, max_col=excel_pos, values_only=True), min_row):
            ...
            if cell_value is not None:
                ...
                logging.debug(f'Kuupäevaveerus leiti mittesobiv väärtus. Rida {jrk} veerg {excel_col}')
                return (None, None)
