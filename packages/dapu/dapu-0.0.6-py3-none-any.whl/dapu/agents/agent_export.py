...
class Agent(AgentGeneric):
    ...
    def do_action(self) -> bool:
        ...
        if not existing_files:
            logging.error(f"No files for tag {file_element} for {self.task_id}")
            return False
        ...
        ts_col_pos: int = self.find_ts_col_pos(action_def.get('tscol', ''), all_cols_array, len(all_cols_array)) if method == 'ts' else -1
        ts_col_name: str = self.action.get('tscol', '') if method == 'ts' else ''
        ...
        if destination == 'file':
            ...
            logging.debug(f"Temporary file is {task_temp_file_name}")
        ...
        while True:
            try:
                ...
                sql = make_replacements(sql_template, replacements)
                rowset = self.context.hub.run(self.route_alias, sql)
                logging.info(f"Data loaded from source {self.route_alias} into memory, {len(rowset)} records")
            except Exception as e1:
                logging.error(f"Data pull failed ({schema_name}.{table_name})")
                logging.error(f"{e1}")
                return False # quit on first batch with error

            if destination == 'file':
                if len(rowset) == 0:
                    logging.info(f"No records pulled ({schema_name}.{table_name}). Making temporary file {task_temp_file_name} empty too.")
                    with open(task_temp_file_name, 'w', encoding='utf-8') as sf:
                        sf.write("")
                    return True
                else:
                    # rowset export to postgre importable data file
                    logging.info(f"Pulled {len(rowset)} records ({schema_name}.{table_name}). Saving them to temporary file {task_temp_file_name}")
                    return save_data_for_postgre(task_temp_file_name, rowset)

            if destination == 'direct':
                if len(rowset) == 0:
                    logging.info(f"No records pulled ({schema_name}.{table_name})")
                    break
                finished = self.save_rows(rs, all_cols_array, pk_cols_array, method, ts_col_pos)
                if finished:
                    break
            if len(rs) < 3000: # temp solution for MS SQL time problem. can be rewritten to use some key (limit_quitter_in_use, limit_quitter_limit)
                break
        return True
    ...
    def save_rows(self, rowset, all_cols_array: list[str], pk_cols_array: list[str], method: str, ts_col_pos: int) -> bool:
        ...
        """
        logging.debug(f"Got {len(rowset)} records, trying insert+conflict")
        if len(rowset) == 0: # hopefully we never got empty rowset
            return True # redundant
        
        nearness = '2 minutes' # PG interval literal expression
        route_code, schema_name, table_name = self.split_task_id(self.task_id)
        logging.debug(f"Table {table_name} must have CONSTRAINT ak_{table_name}_4_uniq")
        ...
        for pos, this_row in enumerate(rowset):
            ...
            if method == 'ts' and ts_col_pos >= 0 and ts_col_pos <= range_end:
                ...
                if not last_ts:
                    # FATAL!?!
                    logging.error(f"Last change timestamp column value is empty/null, it is not meant to be so, cannot continue, fix input data")
