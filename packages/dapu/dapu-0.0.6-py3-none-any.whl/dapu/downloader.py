...
from dapu.globals import context
...
def make_dir_empty(folder: str):
    ...
    Keeps folder
    """
    logging.info(f"Lets delete all from {folder}")
    for filename in os.listdir(folder):
        ...
        try:
            if os.path.isfile(file_path) or os.path.islink(file_path):
                ...
                logging.info(f"Deleting subfolder {filename}")
...
class DapuDownloader(DapuProcess):
    ...
    def run(self) -> int:
        ...
        """
        logging.info(f"Working directory is {context.work_dir}")
        extract_to = context.more_args[0][0]
        logging.info(f"Extraction directory in {extract_to}")
        ...
        result_set = context.target(sql) 
        count = 0
        zip_error_ids = []
        for row in result_set:
            logging.info(f"{row[4]} - {row[2]}")
        ...
        if id_str:
            ...
            context.target(sql_del, False)
            
        context.disconnect_main()
