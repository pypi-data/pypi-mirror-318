...
class BasicInterface:
    ...
    def init_sending(self, owner_sign_key_send_id: int, owner_enc_key_send_id: int):
        ...
        """
        beacon_a, beam_a = asyncio.run(
            self._single_relay_connect_outgoing(owner_sign_key_send_id, owner_enc_key_send_id))
    ...
    async def _single_relay_connect_outgoing(self, owner_key_id: int, owner_enc_key_id: int) -> Tuple[
        Beacon, Beam]:
        """
        ...
        if not beacon:
            ...
            exit(1)
        await self._init_out_encryption(beam)
        return beacon, beam
