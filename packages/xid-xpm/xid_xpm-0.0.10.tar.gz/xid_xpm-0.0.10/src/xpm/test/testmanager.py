...
class TestManager:
	...
	#--------------------------------------------------------------------------------
	@property
	def TextTestRunner(self) -> TextTestRunner:
		return self.__textTestRunner
	...
	def __init__(self) -> None:
		self.__testLoader: TestLoader = TestLoader()
		self.__textTestRunner: TextTestRunner = TextTestRunner()
	...
	def Run() -> None:
		...
		if rootPath not in sys.path:
			...
			Builtins.Print(f"[TestManager] Add Project Root: {rootPath}")

		# 소스 경로 추가.
		# src 안의 서브패키지들을 메인 패키지 처럼 src. 접근 없이 곧바로 사용하기 위한 설정.
		if sourcePath not in sys.path:
			sys.path.append(sourcePath)
			Builtins.Print(f"[TestManager] Add Source Packages: {sourcePath}")
