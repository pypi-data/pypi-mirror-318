    ...
    if action_type == "tiktokcircle" or action_type == "tiktokpuzzle" or action_type == "tiktok3d" or action_type == "tiktokicon":
        try:
            ...
            while time.time() - start_time < wait_captcha_seconds:
                ...
                wait_is_exist_capctha_slide = driver.execute_script("""var elements = document.evaluate('//div[contains(@class,"captcha_verify_slide")]//div[contains(@class,"slidebar") or contains(@class,"captcha_verify_slide")] | //div[contains(@class,"captcha_verify_img")]/img[contains(@class,"captcha_verify_img_slide")]/following::div/following::div//div[contains(@class,"captcha-drag-icon")]', document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null); return elements.snapshotLength > 0;""")             
                wait_is_exist_3d_capctha = driver.execute_script("""var elements = document.evaluate('//img[contains(@id,"captcha-verify-image")][contains(@src,"/3d_")] | //div[contains(@class,"cap")]//img/following-sibling::button/parent::div/parent::div/parent::div/div//img[contains(@src,"/3d_") and //div[contains(@class,"cap")]//img/following-sibling::button]', document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null); return elements.snapshotLength > 0;""")
            ...
            for i in range(0, number_captcha_attempts):
                ...
                if not (is_exist_capctha_whirl or is_exist_capctha_slide or is_exist_3d_capctha or is_exist_icon_capctha): 
                    ...
                    if is_exist_3d_capctha:
                        get_captcha_data = driver.execute_script("""var imgElement = document.evaluate('//div[contains(@class,"captcha_verify_img") and count(img) = 1] | //div[contains(@class,"cap")]//img/following-sibling::button/parent::div/parent::div/parent::div/div//img[contains(@src,"/3d_")]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; if (imgElement) {var imgRect = imgElement.getBoundingClientRect(); return {width: imgElement.width, height: imgElement.height, x: imgRect.left + window.scrollX, y: imgRect.top + window.scrollY};} else {return null;}""")
