...
class Grid(GridCore, Interpolation, Prediction, Focal, Zonal):
    ...
    def randomize(self):
        return self.local(np.random.rand(self.height, self.width))
    ...
    def con(
        ...
        return con(
            predicate if isinstance(predicate, Grid) else self == predicate,
            replacement,
            self if fallback is None else fallback,
        )

    def set_nan(self, predicate: Operand, fallback: Optional[Operand] = None):
