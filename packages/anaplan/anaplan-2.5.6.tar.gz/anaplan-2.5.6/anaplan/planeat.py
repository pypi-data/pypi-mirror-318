...
Args:
    ...
    complexity (str, optional): 'low' low and for lite models. 'high' more complex and quick learnable models. Default: 'low'
...
### ERROR AND CONFIGURATION CHECKS:
   ...
   if complexity != 'low' and complexity != 'high':
      raise ValueError("ccomplexity hyperparameter must be 'low' or 'high''")
   ...
   for i in tqdm(range(len(bad_weights)), desc="GENERATION: " + str(what_gen), bar_format=bar_format, ncols=50, ascii="▱▰"):
      ...
      if policy == 'normal_selective':
         ...
         if strategy == 'cross_over':
            bad_weights[i], bad_activations[i] = cross_over(best_weight, best_weights[i], best_activations=best_activation, good_activations=best_activations[i], cross_over_mode=cross_over_mode, activation_selection_add_prob=activation_selection_add_prob, activation_selection_change_prob=activation_selection_change_prob, activation_selection_rate=activation_selection_rate, complexity=complexity)
...
def cross_over(best_weight, good_weight, best_activations, good_activations, cross_over_mode, activation_selection_add_prob, activation_selection_change_prob, activation_selection_rate, complexity):
   """
   ...
   if complexity == 'low':
      ...
      new_activations = good_activations + best_activations
