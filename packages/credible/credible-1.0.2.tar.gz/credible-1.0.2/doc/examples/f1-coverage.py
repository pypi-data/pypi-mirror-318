...
for (precision_idx, precision_p), (recall_idx, recall_p) in itertools.product(
    ...
    recall_binomial = rng.binomial(n=positives, p=recall_p, size=mc_simulation)
    ...
    precision_binomial = rng.binomial(
    ...
    variates = credible.bayesian.utils.f1_posterior(
        tp, fp, fn, lambda_=1.0, nb_samples=mc_simulation, rng=rng
    )
    (
        mean,
        mode,
