...
from clickzetta.zettapark._internal.analyzer.query_plan_node import Limit, LogicalPlan
...
from clickzetta.zettapark.column import Column, _to_col_if_sql_expr
...
class DataFrame:
    ...
    **Creating a DataFrame**
    ...
    Creating tables and data to run the sample code:
        ...
        ...     t.flush()
        ...     put_result = session.file.put(t.name, "volume:table://test_table/test_dir")
    ...
    Example 2
        Creating a DataFrame by reading files from a volume::

            >>> from clickzetta.zettapark.types import StructType, StructField, IntegerType, StringType
            >>> df_catalog = session.read.schema(StructType([StructField("id", StringType()), StructField("name", StringType())])).csv("volume:table://test_table/test_dir")
            >>> df_catalog.show()
            ---------------------
            |"ID"  |"NAME"      |
            ---------------------
    ...
    **Performing an action on a DataFrame**
    ...
    def group_by(
        ...
        *cols: Union[ColumnOrName, Iterable[ColumnOrName]],
        ...
        Examples:
            ...
            >>> df.group_by(["a", lit("zetta")]).agg(sum_("b")).collect()
            [Row(A=1, LITERAL()='zetta', SUM(B)=3), Row(A=2, LITERAL()='zetta', SUM(B)=3), Row(A=3, LITERAL()='zetta', SUM(B)=3)]
        ...
        **kwargs,
        ...
        if isinstance(right, DataFrame):
            ...
            if column_to_bool(using_columns) is False:
                ...
                using_columns = using_columns
            elif isinstance(using_columns, Iterable):
                using_column_names = []
                for idx, col in enumerate(using_columns):
                    if isinstance(col, Column):
                        if isinstance(col._expression, NamedExpression):
                            using_column_names.append(
                                unquote_name(col._expression.name)
                            )
                        else:
                            raise TypeError(f"Column at index {idx} has no name")
                    elif isinstance(col, str):
                        using_column_names.append(col)
                    else:
                        raise TypeError(
                            f"All list elements for 'on' or 'using_columns' must be str or Column. "
                            f"Got: '{type(col)}' at index {idx}"
                        )
                using_columns = using_column_names
            else:
                raise TypeError(
    ...
    def write(self) -> DataFrameWriter:
        ...
        a ClickZetta database or a volume path
        ...
        Example::
            ...
            -------------
            <BLANKLINE>
            >>> df.write.copy_into_location("volume:table://test_table/copied_from_dataframe")  # default CSV
...
{output_queries}"""
    ...
    def dtypes(self) -> List[Tuple[str, str]]:
        dtypes = [
            (name, convert_data_type_to_name(field.datatype))
