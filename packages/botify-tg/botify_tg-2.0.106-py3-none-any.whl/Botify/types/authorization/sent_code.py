...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~Botify.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~Botify.Client.sign_in` or :meth:`~Botify.Client.sign_up`).

        next_type (:obj:`~Botify.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~Botify.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~Botify.Client.resend_code`.
    """
