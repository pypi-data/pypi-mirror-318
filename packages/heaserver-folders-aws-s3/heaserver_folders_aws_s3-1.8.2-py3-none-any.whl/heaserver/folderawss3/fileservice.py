...
from heaobject.awss3key import KeyDecodeException, decode_key, encode_key, is_root, parent, split
...
from .util import clear_target_in_cache, move
...
async def post_file_mover(request: web.Request) -> web.Response:
    ...
    bucket_id = request.match_info['bucket_id']
    id_ = request.match_info['id']
    target_url, target_bucket_name, target_key, target_volume_id, _ = await awsservicelib._copy_object_extract_target(await request.json())
    sub = request.headers.get(SUB, NONE_USER)
    try:
        key = decode_key(id_)
    except KeyDecodeException as e:
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        try:
            ...
            return await move(activity, request, None, sub, volume_id, bucket_id, id_, key, target_volume_id,
                              target_bucket_name, parent_key, AWSS3FileObject, target_path)
        finally:
...
async def _get_file(request: web.Request) -> web.Response:
    ...
    async with DesktopObjectActionLifecycle(request=request,
                                            code='hea-get',
                                            description=f'Getting {awsservicelib._activity_object_display_name(bucket_name, file_name)}',
                                            activity_cb=publish_desktop_object) as activity:
