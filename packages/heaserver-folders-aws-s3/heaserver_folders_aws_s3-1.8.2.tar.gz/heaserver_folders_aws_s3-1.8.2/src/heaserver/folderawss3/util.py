...
async def _move_rename(activity: DesktopObjectAction, request: web.Request, mongo_client: mongo.Mongo | None,
    ...
    :param request: the HTTP request (required).
    :param mongo_client: a MongoDB client.
    ...
    async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3_client:
        try:
            ...
            async def move_started(source_bucket_id: str, source_key_: str, source_version: str | None, target_bucket_id_: str, target_key_: str):
                ...
                logger.debug('move_started: metadata_ for bucket %s and key %s: %s', source_bucket_id, source_key_, metadata_)
            ...
            async def move_completed(source_bucket_id: str, source_key_: str, source_version: str | None, target_bucket_id_: str, target_key_: str, target_version: str | None):
                """
                The moved S3 object has a key like a path, and ancestors may not have S3 objects associated with them yet
                have metadata, so we need check and update the metadata.
                ...
                try:
                    while path:
                        logger.debug('move_completed: metadata_ for bucket %s and key %s: %s', source_bucket_id, source_key_, metadata_)
                        if path not in processed_keys:
                            if metadata_ is not None and mongo_client is not None:
