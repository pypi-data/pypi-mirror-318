...
@pytest.mark.parametrize(
    "split,lengths",
    [
        ("optic-cup", dict(train=50, test=51)),
        ("optic-disc", dict(train=50, test=51)),
    ],
...
def test_protocol_consistency(
    ...
    database_checkers.check_split(
        make_split("mednet.config.segment.data.drishtigs1", f"{split}.json"),
        lengths=lengths,
        prefixes=[
...
def test_loading(database_checkers, name: str, dataset: str):
    datamodule = importlib.import_module(
        f".{name}",
        "mednet.config.segment.data.drishtigs1",
    ).datamodule
...
def test_raw_transforms_image_quality(database_checkers, datadir):
    reference_histogram_file = (
        datadir / "histograms/raw_data/histograms_drishtigs1_optic_cup_all.json"
    )

    datamodule = importlib.import_module(
        ".optic_cup_all", "mednet.config.segment.data.drishtigs1"
    ).datamodule
...
def test_model_transforms_image_quality(database_checkers, datadir, model_name):
    reference_histogram_file = (
        datadir
        / f"histograms/models/histograms_{model_name}_drishtigs1_optic_cup_all.json"
    )
