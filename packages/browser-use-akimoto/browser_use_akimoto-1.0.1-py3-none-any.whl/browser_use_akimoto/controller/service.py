...
from browser_use_akimoto.agent.views import ActionModel, ActionResult
from browser_use_akimoto.browser.context import BrowserContext
from browser_use_akimoto.controller.registry.service import Registry
from browser_use_akimoto.controller.views import (
	ClickElementAction,
	DoneAction,
...
from browser_use_akimoto.utils import time_execution_async, time_execution_sync
...
class Controller:
	...
	def _register_default_actions(self):
		...
		@self.registry.action(
			description='Scroll down inside the (DOM) first element with role="feed"',
			param_model=ScrollAction,
			requires_browser=True,
		)
		async def partialScrollDown(params: ScrollAction, browser: BrowserContext) -> ActionResult:
			"""
			Locates the first element with role="feed" and scrolls it downward by the given pixel amount.
			"""
			page = await browser.get_current_page()

			# Try using Playwright's role-based locator
			# If "feed" is not recognized, you can switch to: page.locator('[role="feed"]')
			locator = page.get_by_role("feed")

			# Count how many feed elements there are
			count_all = await locator.count()
			print(f'Elements with role="feed": {count_all}')

			if count_all == 0:
				msg = 'No elements found with role="feed".'
				return ActionResult(extracted_content=msg, include_in_memory=True)

			# Default scroll amount if not specified
			scroll_amount = params.amount if params.amount is not None else 200

			# Perform partial scroll
			js_script = f"el => el.scrollBy(0, {scroll_amount});"
			try:
				await locator.first.evaluate(js_script)
				msg = f'Partial scroll: first feed element scrolled down {scroll_amount}px.'
				return ActionResult(extracted_content=msg, include_in_memory=True)
			except Exception as e:
				return ActionResult(error=f'partialScrollDown error: {str(e)}')


		@self.registry.action(
			description='Scroll up inside the (DOM) first element with role="feed"',
			param_model=ScrollAction,
			requires_browser=True,
		)
		async def partialScrollUp(params: ScrollAction, browser: BrowserContext) -> ActionResult:
			"""
			Locates the first element with role="feed" and scrolls it upward by the given pixel amount.
			"""
			page = await browser.get_current_page()
			...
			try:
				await locator.first.evaluate(js_script)
				msg = f'Partial scroll: first feed element scrolled up {scroll_amount}px.'
				return ActionResult(extracted_content=msg, include_in_memory=True)
			except Exception as e:
				return ActionResult(error=f'partialScrollUp error: {str(e)}')
		...
		@self.registry.action(
			description="Show an alert dialog in the browser with the given text",
			requires_browser=True,
		)
		async def show_alert(text: str, browser: BrowserContext) -> ActionResult:
			"""
			Show a JavaScript alert() dialog with the specified text.
			"""
			page = await browser.get_current_page()

			# JavaScript snippet to display alert
			script = f"alert('{text}')"
			await page.evaluate(script)

			msg = f"Displayed an alert dialog with: {text}"
			logger.info(msg)
			return ActionResult(extracted_content=msg, include_in_memory=True)
