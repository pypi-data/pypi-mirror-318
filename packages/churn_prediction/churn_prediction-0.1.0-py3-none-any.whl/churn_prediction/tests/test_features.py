...
def test_column_renamer(sample_data):
    """Testa se o ColumnRenamer renomeia as colunas corretamente."""
    renamer = ColumnRenamer()
    transformed_data = renamer.fit_transform(sample_data)
    assert "Módulo Financeiro" in transformed_data.columns
    assert (
        "Frequência de utilização de feature do sistema: Módulo financeiro"
        not in transformed_data.columns
    )
