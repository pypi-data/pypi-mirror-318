...
import typing
from . import F
__all__ = ['Classifier', 'F', 'FaceDetector', 'FaceObject', 'FaceObjects', 'FaceRecognizer', 'HandLandmarks', 'LayerInfo', 'MUD', 'NN', 'NanoTrack', 'OCR_Box', 'OCR_Object', 'OCR_Objects', 'Object', 'ObjectFloat', 'Objects', 'PP_OCR', 'Retinaface', 'SelfLearnClassifier', 'Speech', 'SpeechDecoder', 'SpeechDevice', 'YOLO11', 'YOLOv5', 'YOLOv8']
class Classifier:
    label_path: str
    labels: list[str]
    mean: list[float]
...
class NN:
    ...
    def forward_image(self, img: maix._maix.image.Image, mean: list[float] = [], scale: list[float] = [], fit: maix._maix.image.Fit = ..., copy_result: bool = True, dual_buff_wait: bool = False, chw: bool = True) -> maix._maix.tensor.Tensors:
        ...
        Default true to avoid problems, you can set it to false manually to make speed faster.
          - dual_buff_wait: bool type, only for dual_buff mode, if true, will inference this image and wait for result, default false.
          - chw: chw channel format, forward model with hwc format image input if set to false, default true(chw).
