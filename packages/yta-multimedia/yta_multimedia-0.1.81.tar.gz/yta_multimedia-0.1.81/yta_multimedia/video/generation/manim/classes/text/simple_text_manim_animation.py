from yta_multimedia.video.generation.manim.classes.base_manim_animation import BaseManimAnimation, NewBaseManimAnimation
...
class BaseManimAnimationWrapper:
    ...
    """
    animation_generator_instance: NewBaseManimAnimation = None

    def __init__(self, animation_generator: NewBaseManimAnimation):
        if not PythonValidator.is_subclass(animation_generator, NewBaseManimAnimation):
            raise Exception('The provided "animation_generator" is not a subclass nor an instance of a subclass of NewBaseManimAnimation class.')
        
        if PythonValidator.is_a_class(animation_generator):
            animation_generator = animation_generator()

        self.animation_generator_instance = animation_generator
...
class ExampleManimAnimationWrapper(BaseManimAnimationWrapper):
    """
    Example manim wrapper class that collects the 
    parameters, validates them and builds the scene
    using the associated generator class.
    """
    text: str = None
    duration: float = None
    ...
    def __init__(self, text: str, duration: float):
        ...
        super().__init__(ExampleManimAnimationGenerator)

class ExampleManimAnimationGenerator(NewBaseManimAnimation):
    def construct(self):
        """
...
# TODO: Remove this below when the 2 'example' classes
# above are working perfectly with the new base class
