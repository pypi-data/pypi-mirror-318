...
class VideoModification(ApplicableInVideo):
    """
...
class ColorTemperatureVideoModification:
    start_time: float = None
    end_time: float = None
    layer: int = None
    factor: int = None
    ...
    def __init__(self, start_time: float, end_time: float, layer: int, factor: int = 45):
        ...
        # TODO: 'start_time', 'end_time' and 'layer' will be
        # common for any kind of video modification, but
        # 'factor' is specific of this type of effect video
        # modification
        self.start_time = start_time
        self.end_time = end_time
        self.layer = layer
    ...
    def apply(self, video: Clip) -> Clip:
        if self.start_time < video.duration and self.end_time < video.duration:
            raise Exception('This VideoModification cannot be applied in the provided "video".')
        
        left_clip, center_clip, right_clip = subclip_video(video, self.start_time, self.end_time)
        center_clip = VideoEditor.change_color_temperature(center_clip, self.factor)
        clips = [clip for clip in [left_clip, center_clip, right_clip] if clip is not None]
