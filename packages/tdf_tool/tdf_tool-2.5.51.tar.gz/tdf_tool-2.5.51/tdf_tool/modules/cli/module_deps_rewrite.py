...
class ModuleDependenciesRewriteUtil:
    ...
    # # 配置的所有依赖模块（包含源码依赖和强制重写依赖）
    # def _configDepsModules(self) -> list[str]:
    #     modules = []
    #     # 源码依赖
    #     modules.append(self.moduleNameList)
    #     # 强制重写依赖
    #     forceOverrides = self.initialConfig.forceOverrides
    #     modules.append([list(item.keys())[0] for item in forceOverrides])
    ...
    def _recursionDeps(self, deps: dict) -> dict:
        ...
        for node in deps:
            if self.moduleNameList.__contains__(node["name"]):
                module = node["name"]
                depsModules = self._recursion(module, modules, deps, 0)
    ...
    # 递归分析依赖
    def _recursion(self, moduleName: str, modules: list[str], deps: dict, level: int) -> list[DepsNode]:
        ...
        if [item["name"] for item in deps].__contains__(moduleName):
            ...
            for module in depsModules:
                resultModule = resultModule + [
                    item
                    for item in self._recursion(module.name, modules, deps, level + 1)
    ...
    def rewrite(self, reWriteOnlyChange=False):
        ...
        # 重写壳
        Print.step("Rewrite in shell...")
        
        self._reWriteInShell()
        
        Print.step("Deps analysis...")
        depsJson = json.loads(os.popen("flutter pub deps --json").read())
        packageConfig = self._recursionDeps(depsJson["packages"])  # 依赖树维护
        ...
        for module in self.moduleNameList:
            ...
            for key in packageConfig.keys():
                if key == module:
                    moduleDeps: list[str] = [item.name for item in packageConfig[key]]
        ...
        for index, moduleName in enumerate(depsSortedList):
            ...
            for node in packageConfig[moduleName]:
                if self.moduleNameList.__contains__(node.name) and node.type.indirect:
