...
class Account(User):
    # fmt: off
    auth_token:  Union[str,None] = Field(default=None, pattern=r"^[a-f0-9]{40}$")
    ct0:         Union[str,None] = None  # 160
    password:    Union[str,None] = None  # 128
    email:       Union[str,None] = None  # 254
    totp_secret: Union[str,None] = None  # 16
    backup_code: Union[str,None] = None  # 12
    status: AccountStatus = AccountStatus.UNKNOWN
    # fmt: on

    @property
    def hidden_auth_token(self) -> Union[str,None]:
        return hidden_value(self.auth_token) if self.auth_token else None

    @property
    def hidden_password(self) -> Union[str,None]:
        return hidden_value(self.password) if self.password else None

    @property
    def hidden_totp_secret(self) -> Union[str,None]:
        return hidden_value(self.totp_secret) if self.totp_secret else None

    @property
    def hidden_backup_code(self) -> Union[str,None]:
    ...
    def get_totp_code(self) -> Union[str, None]:
