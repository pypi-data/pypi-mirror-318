...
def get_body(packet, is_req):
    try:
        print(type(packet))
        if is_req is True and 'req_text' in packet:
            a = packet['req_text']
            print(f"req_text类型：{a}")
            return "".join([item for item in f"{a}".split("\r\n\r\n") if "HTTP/" not in item])
        elif is_req is False and 'res_text' in packet:
            a = packet['res_text']
            print(f"res_text类型：{a}")
            return "".join([item for item in f"{a}".split("\r\n\r\n") if "HTTP/" not in item])
        else:
            print("里面没有")
            return ""
