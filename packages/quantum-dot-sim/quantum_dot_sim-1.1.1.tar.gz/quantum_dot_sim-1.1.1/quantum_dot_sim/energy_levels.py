...
def calculate_energy_levels(radius, material_properties):
    """
    Calculate energy levels for a spherical quantum dot using an approximation.

    Parameters:
        radius (float): Radius of the quantum dot (in meters).
        material_properties (dict): Dictionary containing material-specific properties such as 'effective_mass' (kg).
            - Default 'effective_mass' is the electron mass (9.1e-31 kg) if not provided.

    Returns:
        list: Energy levels (in joules).
    """
