...
class Gauge(QtWidgets.QWidget):
	...
	def set_MinValue(self, min):
		...
		self.step = (self.value_max - self.value_min) / 100
	...
	def wheelEvent(self, event):
		if self.wheel_enabled:
			...
			if delta > 0:
				next = self.value + self.step
				if next > self.value_max:
					next = self.value_max
				self.update_value(next)  # Scroll up
			else:
				next = self.value - self.step
				if next < self.value_min:
					next = self.value_min
				self.update_value(next)  # Scroll down