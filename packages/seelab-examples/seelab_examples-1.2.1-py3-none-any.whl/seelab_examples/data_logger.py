...
class Expt(QtWidgets.QWidget, ui_data_logger.Ui_Form):
	...
	def __init__(self, device=None):
		...
		self.currentPage = 0
		self.isPaused = False
		self.fields = ['A1','A2','A3','SEN','PCS','CUSTOMA1']
		self.min = [-16,-16,-3.3,0,0, 0,0]
		self.max = [16,  16, 3.3,3.3,3.3,50,50]
		self.cbs = {'A1':self.A1Box,'A2':self.A2Box,'A3':self.A3Box,'SEN':self.SENBox,'PCS':self.PCSBox,'CUSTOMA1':self.CUSTOMBox}
		pos = 0
		colors = ['#00ffff','#008080','#ff0000','#800000','#ff00ff','#80f080','#00FF00','#008888','#ffff00','#808000','#0000ff','#000080','#a0a0a4','#808080','#ffffff','#4000a0']
		...
		for a,b,c in zip(self.fields,self.min,self.max):
			...
			curve = self.graph.plot(pen=colors[len(self.curves.keys())], connect="finite", name=a)
	...
	def setEquation(self):
		...
		self.cbs['CUSTOMA1'].setChecked(True)
		self.toggled()
