    ...
    def start(self, root: str, attrib: dict):
        ...
        for k, v in dic_attrib:
            ...
            if v == "true":
                ...
                attributes.append(
                    Attribute(namespace, name, 0xFFFFFFFF, 0x1000000, int(v[-8:], 16)).proto
                )
            elif re.search("^@[0-9a-fA-F]+$", v):
                attributes.append(
                    Attribute(namespace, name, 0xFFFFFFFF, 0x1000000, int(v[1:], 16)).proto
                )
                ...
                attributes.append(
                    Attribute(namespace, name, 0xFFFFFFFF, 0x1C000000, int(v[2:], 16)).proto
                )
            elif re.search(r"^\?[0-9a-fA-F]+$", v):
                attributes.append(
                    Attribute(namespace, name, 0xFFFFFFFF, 0x02000000, int(v[2:], 16)).proto
                )
            else:
