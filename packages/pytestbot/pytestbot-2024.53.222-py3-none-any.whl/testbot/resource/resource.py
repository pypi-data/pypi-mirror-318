...
class Resource(metaclass=ABCMeta):
    ...
    def init_resource(self, test_type: TestType, step: CaseStepEntry):
        """
        初始化测试资源
        ...
        for checkitem in checklist:
                ...
                if self.type == "PCDevice":
                    for name, port in self.ports.items():
                        if port.type == "CommSerialPort":
                            ...
                            port.init_resource(test_type=test_type, step=step)
                if self.type == "CommSerialPort":
                    with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有指令串口") as step2:
            ...
            if checkitem == CheckItem.COMM_SERIAL_PORT_CHECK__HAS_IP:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否可以通过指令串口获取TV IP地址") as step2:
                    pass
            # TV端是否可以正常访问公司网络，如Panda、AI服务
            if checkitem == CheckItem.COMM_SERIAL_PORT_CHECK__ACCESS_INTERNAL_NET:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：TV端是否可以正常访问公司网络，如Panda、AI服务") as step2:
                    pass
            # TV端是否可以正常访问国内网络
            if checkitem == CheckItem.COMM_SERIAL_PORT_CHECK__ACCESS_DOMESTIC_NET:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：TV端是否可以正常访问国内网络") as step2:
                    pass
            # TV端是否可以正常访问海外网络
            if checkitem == CheckItem.COMM_SERIAL_PORT_CHECK__ACCESS_OVERSEAS_NET:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：TV端是否可以正常访问海外网络") as step2:
                    pass
            ...
            if checkitem == CheckItem.INFRA_SERIAL_PORT_CHECK__EXIST:
                ...
                if self.type == "InfraredSerialPort":
                    with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有红外串口") as step2:
                        self.logger.info(f"端口信息：{self.to_dict()}")
                        self._instance = serial.Serial(port=self.name, baudrate=self.baud_rate, timeout=1)
                        if self._instance:
                            step2.passed(message="")
            ...
            if checkitem == CheckItem.INFRA_SERIAL_PORT_CHECK__NORMAL:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：通过红外串口发送红外指令是否正常") as step2:
                    pass
            ...
            if checkitem == CheckItem.CAP_SERIAL_PORT_CHECK__EXIST:
                ...
                if self.type == "VideoStreamSerialPort":
                    with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有采集卡串口") as step2:
                        self.logger.info(f"端口信息：{self.to_dict()}")
            ...
            if checkitem == CheckItem.CAP_SERIAL_PORT_CHECK__NORMAL:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否能够通过采集卡串口采集图像正常") as step2:
                    pass

            # 是否有音频口
            if checkitem == CheckItem.AUDIO_PORT_CHECK__EXIST:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有音频口") as step2:
                    pass
            # 音频口是否检测有声音
            if checkitem == CheckItem.AUDIO_PORT_CHECK__HAS_SOUND:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：音频口是否检测有声音") as step2:
                    pass

            # 是否有电源通断口
            if checkitem == CheckItem.POWER_PORT_CHECK__EXIST:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有电源通断口") as step2:
                    pass
            ...
            if checkitem == CheckItem.ADB_WIRELESS_PORT_CHECK__EXIST:
                ...
                if self.type == "AdbPort":
                    with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有ADB无线连接") as step2:
                        self.logger.info(f"端口信息：{self.to_dict()}")
                        ip_port = self.name
            ...
            if checkitem == CheckItem.ADB_WIRE_PORT_CHECK__EXIST:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有ADB有线连接") as step2:
                    pass
            ...
            if checkitem == CheckItem.GRPC_PORT_CHECK__EXIST:
                ...
                if self.type == "GRPCPort":
                    with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有gRPC连接") as step2:
                        self.logger.info(f"端口信息：{self.to_dict()}")
                        ip_port = self.name
                        if CheckItem.COMM_SERIAL_PORT_CHECK__EXIST in checklist:
                            ips = res_obj.CommSerialWrapperModule.get_ip_addresses()
                            if len(ips) >= 1:
                                with step2.start(headline=f"获取最新IP地址：{ips[0]:60000}") as step3:
                                    ip_port = f"{ips[0]:60000}"
                        with step2.start(headline=f"检查apk是否安装，没有安装则下载并安装最新版本[未实现]") as step3:
                            pass
                        with step2.start(headline=f"检查apk是否为最新版本，不是最新版本则下载最新版本/卸载当前版本/安装最新版本[未实现]") as step3:
                            pass
                        with step2.start(headline=f"创建gRPC客户端") as step3:
                            ...
                            if self._instance:
                                ...
                                step2.failed(message="")
                        with step2.start(headline=f"创建gRPC心跳保活线程") as step3:
            ...
            if checkitem == CheckItem.ETHER_PORT_CHECK__EXIST:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有网卡通断口") as step2:
                    pass
            # 是否能够从PC端访问TV端的IP地址
            if checkitem == CheckItem.ETHER_PORT_CHECK__ACCESS_IP:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否能够从PC端访问TV端的IP地址") as step2:
                    pass

            # 是否有U盘通断口
            if checkitem == CheckItem.UDISK_PORT_CHECK__EXIST:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有U盘通断口") as step2:
                    pass
            # 是否有U盘插入
            if checkitem == CheckItem.UDISK_PORT_CHECK__HAS_UDISK:
                with step.start(headline=f"对资源类型：{self.type}，资源名称：{self.name}，执行检查项：{checkitem}：是否有U盘插入") as step2:
                    pass
            pass
