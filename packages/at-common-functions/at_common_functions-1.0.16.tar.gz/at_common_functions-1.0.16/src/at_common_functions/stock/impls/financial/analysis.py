...
class FinancialAnalysis:
    ...
    def _calculate_trend(self, series: pd.Series, metric_name: str, periods: int = 4, relative_threshold: float = 0.01) -> str:
        """Calculate trend direction and strength.

        Args:
            series: Time series data to analyze
            metric_name: Name of the metric being analyzed (e.g., 'ROE', 'Current Ratio')
            periods: Number of periods to consider for trend analysis
            relative_threshold: Threshold for determining if a trend is stable
        ...
        if len(series) < periods:
            raise ValueError(
                f"Insufficient data points for {metric_name} trend analysis. "
                f"Required {periods} periods but received {len(series)} periods. "
                f"Available dates: {series.index.tolist()}"
            )
        
        recent_values = series.tail(periods)
        if recent_values.isna().any():
            nan_dates = recent_values[recent_values.isna()].index.tolist()
            raise ValueError(
                f"Cannot calculate {metric_name} trend due to missing values. "
                f"Found NaN values for dates: {nan_dates}. "
                f"Please ensure complete data for the last {periods} periods"
            )
