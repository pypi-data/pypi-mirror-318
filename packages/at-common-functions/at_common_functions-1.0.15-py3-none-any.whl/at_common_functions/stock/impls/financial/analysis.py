...
class FinancialAnalysis:
    ...
    def _calculate_trend(self, series: pd.Series, periods: int = 4, relative_threshold: float = 0.01) -> str:
        """Calculate trend direction and strength.

        Args:
            series: Time series data to analyze
            periods: Number of periods to consider for trend analysis

        Raises:
            ValueError: If insufficient or invalid data is provided

        Returns:
            str: Trend direction ("stable", "increasing", or "decreasing")
        """
        if len(series) < periods:
            raise ValueError(f"Insufficient data points. Expected {periods}, got {len(series)}")
            
        recent_values = series.tail(periods)
        if recent_values.isna().any():
            raise ValueError("Cannot calculate trend with missing (NaN) values")
    ...
    def calculate_profitability_metrics(self, period: str = 'annual') -> Dict[str, FinancialMetric]:
        ...
        metrics['return_on_equity'] = FinancialMetric(
            ...
            benchmark=0.15,
            trend=self._calculate_trend(roe_series),
            period_change=roe_series.pct_change().iloc[-1]
        )
        ...
        metrics['return_on_assets'] = FinancialMetric(
            ...
            benchmark=0.05,
            trend=self._calculate_trend(roa_series),
            period_change=roa_series.pct_change().iloc[-1]
        )
        ...
        metrics['operating_margin'] = FinancialMetric(
            ...
            benchmark=0.15,
            trend=self._calculate_trend(operating_margin_series),
            period_change=operating_margin_series.pct_change().iloc[-1]
        )
        ...
        metrics['net_profit_margin'] = FinancialMetric(
            ...
            benchmark=0.10,
            trend=self._calculate_trend(net_margin_series),
            period_change=net_margin_series.pct_change().iloc[-1]
        )
    ...
    def calculate_liquidity_metrics(self, period: str = 'annual') -> Dict[str, FinancialMetric]:
        ...
        metrics['working_capital'] = FinancialMetric(
            ...
            trend=self._calculate_trend(working_capital),
            period_change=working_capital.pct_change().iloc[-1]
        )
        ...
        metrics['current_ratio'] = FinancialMetric(
            ...
            benchmark=2.0,
            trend=self._calculate_trend(current_ratio_series),
            period_change=current_ratio_series.pct_change().iloc[-1]
        )
        ...
        metrics['quick_ratio'] = FinancialMetric(
            ...
            benchmark=1.0,
            trend=self._calculate_trend(quick_ratio_series),
            period_change=quick_ratio_series.pct_change().iloc[-1]
        )
    ...
    def calculate_efficiency_metrics(self, period: str = 'annual') -> Dict[str, FinancialMetric]:
        ...
        metrics['asset_turnover'] = FinancialMetric(
            ...
            benchmark=1.0,
            trend=self._calculate_trend(asset_turnover_series),
            period_change=asset_turnover_series.pct_change().iloc[-1]
        )
        ...
        metrics['inventory_turnover'] = FinancialMetric(
            ...
            benchmark=6.0,
            trend=self._calculate_trend(inventory_turnover_series),
            period_change=inventory_turnover_series.pct_change().iloc[-1]
        )
    ...
    def calculate_solvency_metrics(self, period: str = 'annual') -> Dict[str, FinancialMetric]:
        ...
        metrics['debt_to_equity'] = FinancialMetric(
            value=debt_to_equity_series.iloc[-1],
            description="Measures financial leverage",
            benchmark=1.5,
            trend=self._calculate_trend(debt_to_equity_series),
            period_change=debt_to_equity_series.pct_change().iloc[-1]
        )
        ...
        metrics['interest_coverage'] = FinancialMetric(
            value=interest_coverage_series.iloc[-1],
            description="Measures ability to meet interest payments",
            benchmark=3.0,
            trend=self._calculate_trend(interest_coverage_series),
            period_change=interest_coverage_series.pct_change().iloc[-1]
        )
    ...
    def calculate_cash_flow_metrics(self, period: str = 'annual') -> Dict[str, FinancialMetric]:
        ...
        metrics['operating_cash_flow_ratio'] = FinancialMetric(
            ...
            benchmark=0.10,
            trend=self._calculate_trend(operating_cash_flow_ratio_series),
            period_change=operating_cash_flow_ratio_series.pct_change().iloc[-1]
        )
        ...
        metrics['free_cash_flow_yield'] = FinancialMetric(
            ...
            benchmark=0.05,
            trend=self._calculate_trend(free_cash_flow_yield_series),
            period_change=free_cash_flow_yield_series.pct_change().iloc[-1]
        )
