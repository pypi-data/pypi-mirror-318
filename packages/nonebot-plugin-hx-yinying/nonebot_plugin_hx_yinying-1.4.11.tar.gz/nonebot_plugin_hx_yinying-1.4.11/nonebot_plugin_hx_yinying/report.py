...
def get_file():
    ...
    尝试下载报错主要文件
    """
    url = "https://api.leafone.cn/api/lanzou?type=down&url=https://wwp.lanzoup.com/i1TJF1wvd6aj"
    try:
...
async def post_run(bot: Bot, event: MessageEvent, e: Exception) -> None:
    ...
    try:
        ...
        logger.warning("[Hx]尝试制作错误报告失败，尝试使用文字发送")
        img = None
    try:
        groupid = event.group_id
        id = event.user_id
        if not img:
            msg = MessageSegment.text("抱歉，我出现了一点问题，请尝试使用其他指令，或者联系开发者")
        else:
        ...
        if groupid:
            ...
            await bot.call_api("send_private_msg",user_id=id,message=msg)
    except:
