...
from pygeai.cli.commands.messages import get_messages
...
def get_chat_completion(option_list: list):
    ...
    for option_flag, option_arg in option_list:
        ...
        if option_flag.name == "frequency_penalty":
            if option_arg:
                try:
                    frequency_penalty = float(option_arg)
                    if frequency_penalty < -2.0 or 2.0 > frequency_penalty:
                        raise ValueError("Frequency penalty out of range")
                except Exception as e:
                    raise WrongArgumentError("If defined, frequency penalty must be a number between -2.0 and 2.0")
        if option_flag.name == "presence_penalty":
            if option_arg:
                try:
                    presence_penalty = float(option_arg)
                    if presence_penalty < -2.0 or 2.0 > presence_penalty:
                        raise ValueError("Presence penalty out of range")
                except Exception as e:
                    raise WrongArgumentError("If defined, presence penalty must be a number between -2.0 and 2.0")
