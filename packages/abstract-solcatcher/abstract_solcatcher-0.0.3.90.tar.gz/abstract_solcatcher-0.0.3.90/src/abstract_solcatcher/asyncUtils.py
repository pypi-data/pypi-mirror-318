...
async def make_request(url, payload, headers=None):
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            return await  response.json()       # or just return response if you want the full object
...
def get_solcatcherSettings(getApi=False,**kwargs):
    ...
    if apiKey or getApi:
        is isinstance(apiKey,bool):
            apiKey=None
        headers = get_db_header(header=headers,api_key=apiKey)
