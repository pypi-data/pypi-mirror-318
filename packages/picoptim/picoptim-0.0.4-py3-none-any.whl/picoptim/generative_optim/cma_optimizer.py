...
class OptimResultCMA(OptimResult):
    ...
    HIST_COV_PATH = "HIST_COV"
    FULL_EVALS_PATH = "FULL_EVALS"
    ...
    def save(self, name: str, path: str = ".", overwrite: bool = True) -> str:
        ...
        io.rw_arr.save(
            io.combine_to_path(self.HIST_COV_PATH, io.rw_arr.ext, acc_path),
            np.asarray(self._hist_cov),
        )

        # Save additional attribute full_evals
        if self.full_evals is not None:
            self.full_evals.save("full_evals", acc_path, overwrite=overwrite)
    ...
    def _load(cls, name: str, directory: Optional[str] = None):
        ...
        hist_cov = io.rw_arr.save(
            io.combine_to_path(cls.HIST_COV_PATH, io.rw_arr.ext, acc_path)
        )

        try:
            full_evals = FunEvals.load(cls.FULL_EVALS_PATH, acc_path)
...
OPTIM_RESULT_INDICATOR.add(OptimResultCMA)
