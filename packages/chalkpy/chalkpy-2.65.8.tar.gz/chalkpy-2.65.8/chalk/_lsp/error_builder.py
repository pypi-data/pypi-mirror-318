...
class FeatureClassErrorBuilder:
    ...
    def invalid_attribute(
        self, namespace: str, item: str, candidates: List[str], back: int, saved_frame: FeatureWrapper | None = None
    ):
        back = back + 1
        message = f"Invalid attribute '{item}' on feature class '{namespace}'."
...
class SQLFileResolverErrorBuilder:
    ...
    def add_diagnostic_with_spellcheck(
        ...
        if len(candidates) > 0:
            all_scores = [
                (
                    Levenshtein.distance(spellcheck_item, candidate),
                    candidate,
                )
                for candidate in candidates
            ]
