...
FunctionCapturedGlobal = Union[FunctionCapturedGlobalFeatureClass, FunctionCapturedGlobalBuiltin]
"""
        ...
        if inspect.isclass(global_value):
            try:
                ...
                if object.__getattribute__(global_value, "__chalk_feature_set__") is True:
                    is_feature_set_namespace = getattr(global_value, "__chalk_namespace__")
                    if type(is_feature_set_namespace) is not str:
                        is_feature_set_namespace = None
            except:
                # If there was any kind of exception trying to extract the feature class info from this value,
                # then it is not a `FeatureClass`.
                is_feature_set_namespace = None

            if is_feature_set_namespace:
