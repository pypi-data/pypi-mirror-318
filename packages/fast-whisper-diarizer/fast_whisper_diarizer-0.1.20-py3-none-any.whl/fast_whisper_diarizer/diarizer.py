...
from .utils import check_audio_for_speech, setup_diarizer_config, check_audio_for_human_voice
...
def perform_diarization(audio_path: str, temp_dir: str) -> List[List[float]]:
    ...
    logging.debug("Waveform shape before processing: %s", waveform.shape)

    # Ensure waveform has the correct dimensions
    if waveform.dim() == 1:
        waveform = waveform.unsqueeze(0)

    logging.debug("Waveform shape after adjustment: %s", waveform.shape)

    if sample_rate != 16000:
        resampler = torchaudio.transforms.Resample(
            orig_freq=sample_rate, new_freq=16000)
        waveform = resampler(waveform)
        logging.debug("Resampled audio from %d Hz to 16000 Hz", sample_rate)
    ...
    # if (not check_audio_for_speech(resampled_audio_path)):
    ...
    # Perform diarization with logging
    try:
        msdd_model.diarize()
    except IndexError as e:
        logging.error("IndexError during diarization: %s", str(e))
        raise
    except ValueError as e:
        logging.error("ValueErrorfvdsvfds: %s", str(e))
        raise
