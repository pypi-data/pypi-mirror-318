...
from dask.distributed import as_completed, get_client, progress
...
class DataCube:
    ...
    def apply_function(
        ...
        processing = futures.persist()  # start computation in the background

        progress(processing, notebook=False)  # watch progress

        results = processing.compute()
    ...
    def _execute_parallel(
        ...
        items = list(items)
        logger.info(
            f"Starting parallel operation with {len(items)} items using {self.execution_mode.value} mode"
        )
