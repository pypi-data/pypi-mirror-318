...
class DataCube:
    ...
    def apply_function(
        ...
        # Move process_chunk outside of apply_function
        def get_processed_args(dc, idx, args, kwargs, tile_kwargs):
            _args = []
            for arg in args:
        ...
        def process_chunk_wrapper(idx):
            processed = get_processed_args(self, idx, args, kwargs, tile_kwargs)
            if processed is None:
                return None
            
            _args, _kwargs = processed
            result = f(*_args, **_kwargs)
            if output is not None:
                self.set_data(output, idx, result, group)
            return result

        results = self.executor.map(process_chunk_wrapper, idxs)
