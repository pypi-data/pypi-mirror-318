...
class DataCube:
    ...
    def _execute_parallel(
        ...
        if self.execution_mode == ExecutionMode.SEQUENTIAL:
            ...
            return Parallel(n_jobs=self.n_jobs)(joblib_delayed(func)(item) for item in items)

        elif self.execution_mode == ExecutionMode.DASK.value:
            logger.info(f"Using Dask with {self.n_jobs} workers")
            import dask.bag as db
            from dask.diagnostics import ProgressBar
            from dask.distributed import get_client

            idx_bag = db.from_sequence(items)
            delayed_func = dask_delayed(func)
            futures = idx_bag.map(lambda item: delayed_func(item))
            processing = futures.persist()  # start computation in the background
            client = get_client()
            scheduler_address = client.scheduler_info()['address']

            # Use ProgressBar for local execution, progress() for distributed
            if show_progress:
                if "localhost" in scheduler_address:
                    with ProgressBar():
                    ...
                    try:
                        ...
                        print(f"Error watching progress: {e}")
                    results = processing
            return results
