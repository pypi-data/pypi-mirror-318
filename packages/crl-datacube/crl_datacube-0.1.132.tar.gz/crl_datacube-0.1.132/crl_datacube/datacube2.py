...
class DataCube:
    ...
    def _execute_parallel(
        ...
        if self.execution_mode == ExecutionMode.SEQUENTIAL:
            ...
            idx_bag = db.from_sequence(
                items, partition_size=100
            )  # Add partition size for better distribution

            # Option 1: Using delayed (your current approach)
            delayed_func = dask.delayed(func)
            futures = idx_bag.map(delayed_func)

            # Submit computation and get futures
