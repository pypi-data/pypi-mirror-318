...
def _execute_parallel(
    ...
    items = list(items)
    logger.info(
        f"Starting parallel operation with {len(items)} items using {execution_mode.value} mode"
    )
    logger.debug(f"Function to execute: {func.__name__}")
    ...
    if execution_mode == ExecutionMode.SEQUENTIAL:
        ...
        if show_progress:
            return Parallel(n_jobs=n_jobs)(delayed(func)(item) for item in tqdm(items))
        ...
        futures = []
        for item in items:
            future = dask_client.submit(func, item)
            futures.append(future)
