...
class DataCube:
    ...
    def get_xarray_tiles(
        ...
        # to_return = self.executor.map(process, all_tiles)
    ...
    def apply_function(
        ...
        def monitored_task(task, pbar):
            result = task.compute()
            pbar.update(1)
            return result

        # Create delayed objects
        tasks = [delayed(process)(idx) for idx in idxs]

        # Use tqdm to monitor progress
        with tqdm(total=len(tasks)) as pbar:
            # Wrap each task with a monitoring function
            monitored_tasks = [delayed(monitored_task)(task, pbar) for task in tasks]
            
            # Compute the wrapped tasks
            results = compute(*monitored_tasks)
    ...
    def _execute_parallel(
        ...
        items = list(items)
        logger.info(f"Starting parallel operation with {len(items)} items using {self.execution_mode.value} mode")
        ...
        if self.execution_mode == ExecutionMode.SEQUENTIAL:
            ...
            idx_bag = db.from_sequence(items)
            # Map the processing function over the indices
            futures = idx_bag.map(lambda item: func(item))

            # Compute with progress tracking
            with tqdm(total=len(items), desc="Processing tiles") as pbar:
...
#     logger.info(
#         f"Starting parallel operation with {len(idxs)} items using {execution_mode.value} mode"
#     )
#     logger.debug(f"Function to execute: {func.__name__}")
...
#         logger.info("Using sequential processing")
#         if show_progress:
#             return [func(item) for item in tqdm(idxs)]
#         return [func(item) for item in idxs]

#     elif execution_mode == ExecutionMode.JOBLIB:
#         logger.info(f"Using joblib with {n_jobs} workers")
#         if show_progress:
#             return Parallel(n_jobs=n_jobs)(delayed(func)(item) for item in tqdm(idxs))
#         return Parallel(n_jobs=n_jobs)(delayed(func)(item) for item in idxs)
...
#         total_tiles = len(idxs)
#         idx_bag = db.from_sequence(idxs)
#         futures = idx_bag.map(lambda idx: func(idx))
#         # Compute with progress tracking
