...
from crl_datacube.parallel import TaskExecutor, ExecutionMode
...
class DataCube:
    ...
    def apply_function(
        ...
        # Create chunk information for each index
        chunk_infos = [
            {
                'dc': self,
                'idx': idx,
        ...
        results = self.executor.map(_process_datacube_chunk, chunk_infos)
...
# Add this at module level, outside of any class
def _process_datacube_chunk(chunk_info):
    """Standalone function for processing datacube chunks"""
    dc = chunk_info['dc']
    idx = chunk_info['idx']
    f = chunk_info['function']
    args = chunk_info['args']
    kwargs = chunk_info['kwargs']
    tile_kwargs = chunk_info['tile_kwargs']
    output = chunk_info['output']
    ...
    # Store result if output is specified
    if output is not None:
        dc.set_data(output, idx, result, group)
