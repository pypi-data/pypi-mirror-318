...
class BoxMetadataQuery(BaseModel):
    template_key: str = ""
    """
    ...
    """
       ...
       similar to a SQL SELECT statement. Values in the search query can 
       be turned into parameters specified in the query_param arguments 
       list to prevent having to manually insert search values into the 
       query string.
    ...
    """
       ...
       match the type of the corresponding metadata template field.
    """

    ancestor_folder_id: Optional[str] = "0"
    """Limits the search results to items within the given folder id"""
...
class _BoxAPIWrapper(BaseModel):
    ...
    def _is_returnable(self, file_info: box_sdk_gen.FileFull, path: str) -> bool:
        extension = file_info.name.split(".")[1]

        if self.suffixes is not None and extension not in self.suffixes:
            return False

        if self.glob is not None and not fnmatch.fnmatch(path, self.glob):
            return False

        if self.exclude is not None and fnmatch.fnmatch(path, self.exclude):
            return False

        if self.docs and hasattr(DocumentFiles, extension.upper()):
            return True

        if self.images and hasattr(ImageFiles, extension.upper()):
            return True

        return False

    def _generate_path(self, file_info: box_sdk_gen.FileFull) -> str:
        path = "https://app.box.com"

        try:
            for path_token in file_info.path_collection.entries:
                path += f"/{path_token.id}"
    ...
    def _get_extra_fields(
        ...
        for field in extra_fields:
            ...
            try:
                metadata[field] = file_dict[field]
            except KeyError:
                metadata[field] = ""
    ...
    def get_document_by_file_id(self, file_id: str) -> Optional[Document]:
        ...
        fields = ["name", "type", "extension"]

        if self.extra_fields is not None:
            fields += self.extra_fields

        file = self._box.files.get_file_by_id(  #  type: ignore[union-attr]
            file_id, fields=fields
        )
        ...
        if file.type == "file":
            if hasattr(DocumentFiles, file.extension.upper()):
                ...
                if self.extra_fields is not None:
                    metadata = self._get_extra_fields(
                        file=file, metadata=metadata, extra_fields=self.extra_fields
                    )
    ...
    def search_box(
        self, query: str, return_ids: bool = False
        ...
        fields = ["id", "type", "extension", "name", "path_collection"]
        ...
        try:
            ...
            if self.box_search_options is None:
                results = self._box.search.search_for_content(  #  type: ignore[union-attr]
                    query=query, fields=fields, type="file"
                )
            else:
                results = self._box.search.search_for_content(  #  type: ignore[union-attr]
                    query=query,
            ...
            for file in results.entries:
                if (
                    file is not None
                    and file.type == "file"
                    and self._is_returnable(file, self._generate_path(file))
                ):
    ...
    def metadata_query(self, query: BoxMetadataQuery) -> List[str]:
        if self._box is None:
            self.get_box_client()

        file_ids = []

        results = self._box.search.search_by_metadata_query(  #  type: ignore[union-attr]
            from_=query.template_key,
            ancestor_folder_id=query.ancestor_folder_id,
            query=query.query,
    ...
    def get_blob_from_file_id(self, file_id: str) -> Union[Blob, None]:
        if self._box is None:
            self.get_box_client()

        fields = ["id", "name", "path_collection", "size"]

        if self.extra_fields is not None:
            fields += self.extra_fields

        file_info = self._box.files.get_file_by_id(  #  type: ignore[union-attr]
            file_id, fields=fields
        )
        ...
        path = self._generate_path(file_info)

        if self._is_returnable(file_info, path):
            downloaded_file_content: box_sdk_gen.ByteStream = (
                self._box.downloads.download_file(file_id=file_id)  #  type: ignore[union-attr]
            )

            file_content = box_sdk_gen.read_byte_stream(downloaded_file_content)
            ...
            if self.extra_fields is not None:
                metadata = self._get_extra_fields(
                    file=file_info, metadata=metadata, extra_fields=self.extra_fields
                )
