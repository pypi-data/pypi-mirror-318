...
watch(recipientOptions, (recipientOptionsValue) => {
  if (recipientOptionsValue) {
    const preSelected = recipientOptionsValue.find(
      (recipient) => recipient.is_pre_selected
    ) as IClientQuoteRecipient;
    if (preSelected) recipients.value = [preSelected];
  }
});

watch(fromOptions, (fromOptionsValue) => {
  if (fromOptionsValue) {
    from.value = fromOptionsValue.find((sender) => sender.is_pre_selected);
  }
});
</script>
