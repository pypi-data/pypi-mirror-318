...
def manage_layout_form_submit_button_recursive(
    ...
    if layout["interactionType"] == INTERACTION_TYPE.LAYOUT:
        if isinstance(layout["model"]["children"], list):
            new_children = [
                manage_layout_form_submit_button_recursive(child, hide_submit_button)
                for child in layout["model"]["children"]
            ]
        else:
            new_children = manage_layout_form_submit_button_recursive(
...
def manage_layout_form_submit_button(layout: ComponentReturn) -> ComponentReturn:
    """
    ...
    return manage_layout_form_submit_button_recursive(layout, hide_submit_button)
