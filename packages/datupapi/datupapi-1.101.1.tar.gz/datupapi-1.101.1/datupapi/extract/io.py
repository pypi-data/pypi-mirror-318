...
class IO(Config):
    ...
    def populate_dbtable_threads(self,
        ...
        def process_batch(batch, is_first_batch):
            try:
                engine = create_engine(db_api + db_user + ':' + db_passwd + '@' + hostname + ':' + str(port) + '/' + db_name)
                action = "replace" if is_first_batch else "append"
                self.logger.info(f"Procesando batch de tamaño {len(batch)} en la tabla {table_name} con acción {action}.")
                if is_first_batch:
                    batch.to_sql(table_name, con=engine, if_exists='replace', index=False, chunksize=chunk_size)
                else:
        ...
        self.logger.info(f"Carga completa para la tabla {table_name}. Total de registros insertados: {total_inserted_records}")
