...
class HttpServiceBase:
    ...
    def _get_data(self, endpoint: str, params: dict = None, limit: int = 100, paginated: bool = True) -> dict:
        ...
        offset = 0
        all_data = []
        max_retries = 5  # Max number of retries for handling rate-limiting
        ...
        while True:
            ...
            while retries < max_retries:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        retry_after = int(response.headers.get("Retry-After", 1))  # Use Retry-After header if available
                        logger.warning(
                            f"Rate limit hit. Retrying in {retry_after + backoff_factor * retries} seconds...")
                        time.sleep(retry_after + backoff_factor * retries)
                        retries += 1
                    else:
            ...
            if retries >= max_retries:
                logger.error(f"Max retries reached for endpoint {endpoint}. Exiting...")
                break
