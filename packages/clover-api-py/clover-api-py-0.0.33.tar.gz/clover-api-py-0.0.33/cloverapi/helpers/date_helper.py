...
class DateHelper:
    ...
    def filters_period(self, period: Union[str, int]) -> (int, int):
        ...
        # If the period is an integer, process it immediately
        if isinstance(period, int):
            return self.last_n_days(period)

        # If the period is a string, process it
        if isinstance(period, str):
            period = period.lower().strip()  # Ensure it's lowercase and remove leading/trailing spaces
        ...
        # If neither string nor integer, raise an error
    ...
    def date_range(self, start_date: str, end_date: str) -> (int, int):
        """
        Get the start and end timestamps for a specific date range.
        ...
        try:
            ...
            start = datetime.strptime(start_date, "%m-%d-%Y").replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=self.zoneinfo)
            end = datetime.strptime(end_date, "%m-%d-%Y").replace(hour=23, minute=59, second=59, microsecond=999999, tzinfo=self.zoneinfo)
