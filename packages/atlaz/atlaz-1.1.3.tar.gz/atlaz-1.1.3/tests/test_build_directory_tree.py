...
def build_directory_tree_string(selected_files: List[str]) -> str:
    ...
    """
    tree = {}
    # First, determine all directories
    directories = set()
    for file_path in selected_files:
        ...
        if file_path.endswith('/'):
            ...
            # Any parent of this file path is a directory
            directories.update(Path(*path.parts[:i+1]) for i in range(len(path.parts)-1))
    
    for file_path in selected_files:
        path = Path(file_path)
        # Make path relative by stripping leading '/'
        try:
            relative_path = path.relative_to(path.anchor)
        except ValueError:
            # If path is relative, keep it as is
            relative_path = path
        ...
        for idx, part in enumerate(parts):
            ...
            if is_file:
                if part in current_level:
                    ...
                    current_level[part] = None  # Represent file as None
            else:
                if part in current_level:
    ...
    def traverse(current_dict: Dict[str, Any], prefix: str = ""):
        ...
        for idx, (key, subtree) in enumerate(current_dict.items()):
