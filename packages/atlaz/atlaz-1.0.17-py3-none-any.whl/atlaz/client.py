            ...
            with requests.post(url, json=payload, headers=headers, stream=True) as response:
                ...
                if final_response and final_response.get("response"):
                    for file_item in final_response["response"]:
                        base_name = Path(file_item["name"]).stem
                        ext = ".txt"
                        category_dir = None
                        if 'replacing' in file_item["name"]:
                            category_dir = replacing_dir
                        elif 'created' in file_item["name"]:
                            category_dir = created_dir
                        elif 'original' in file_item["name"]:
                            category_dir = original_dir
                        if category_dir:
                            # Check if any files matching the base name exist before globbing
                            existing_files = list(category_dir.glob(f"{base_name}*{ext}"))
                            if existing_files:
                                highest_index = max(
                                    [int(f.stem[len(base_name):]) for f in existing_files if f.stem[len(base_name):].isdigit()]
                                    + [-1]
                                )
                            else:
                                highest_index = -1

                            new_file_name = f"{base_name}{highest_index + 1}{ext}"
                            new_file_content = file_item["content"]
                            target_file = category_dir / new_file_name
