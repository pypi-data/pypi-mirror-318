...
def generate_random_colors(
    ...
    **kwargs,
    ...
    color_gradations_division_rate = kwargs.get(
        "color_gradations_division_rate", 0.5)
    ...
    for _base_color in base_colors:
        ...
        slice_colors_total = round(color_gradations *
                                   color_gradations_division_rate)
        slice_colors = get_slice_colors(_base_color, slice_colors_total,
                                        color_slice_type, **kwargs)
        slice_colors = slice_colors[1:]
        if random_color == _base_color:
            logger.debug(
                f"slice_colors_total: {slice_colors_total}, {len(slice_colors)}"
            )
            logger.debug(slice_colors)
            logger.debug(kwargs)
        ...
        if color_slice_type is SliceType.LIGHTER:
            target_reversed_color = CU.darken(_base_color,
            ...
            target_reversed_color = CU.lighten(_base_color,
                                               reversed_color_offset_rate)
            target_reversed_color_hls = CU.hex2hls(target_reversed_color)
        ...
        reversed_slice_type = SliceType.DARKER if (
            color_slice_type is SliceType.LIGHTER) else SliceType.LIGHTER

        reverse_direction_slice_colors = get_slice_colors(
            _base_color, reversed_slice_colors_total, reversed_slice_type,
            **kwargs)
        ...
        if random_color == _base_color:
            logger.debug(
                f"reversed_slice_colors_total: {reversed_slice_colors_total}, {len(reverse_direction_slice_colors)}"
            )
            logger.debug(reverse_direction_slice_colors)
