...
def opt_equal_risk_contribution(covar: np.ndarray,
    ...
    #    bounds = np.array([(x, y) for x, y in zip(constraints.min_weights.to_numpy(), constraints.max_weights.to_numpy())])
    #else:
    #    bounds = None
    ...
    #print(f"weights={optimal_weights}, sum={np.sum(optimal_weights)}")
...
def opt_equal_risk_contribution0(covar: np.ndarray,
                                constraints: Constraints,
                                risk_budget: np.ndarray = None
                                ) -> np.ndarray:
    """
...
def run_unit_test(unit_test: UnitTests):
    ...
    if unit_test == UnitTests.RISK_PARITY:
        ...
        w_rb = opt_equal_risk_contribution0(covar=covar,
