...
def estimate_rolling_lasso_covar_vol_norm_returns_different_freq(benchmark_prices: pd.DataFrame,
                                                                 prices: pd.DataFrame,
                                                                 rebalancing_freqs: pd.Series,
                                                                 time_period: qis.TimePeriod,  # when we start building portfolios
                                                                 lasso_model: LassoModel,
                                                                 ...
                                                                 demean: bool = True,
                                                                 squeeze_factor: Optional[float] = None,
                                                                 residual_var_weight: float = 0.0
                                                                 ) -> Dict[pd.Timestamp, pd.DataFrame]:
    """
    estimate rolling covar using vol_normalised_returns
    use benchmarks to compute the benchmark covar matrix
    use lasso to estimate betas
    rebalancing_freq is rebalancing for x returns
    rebalancing_freqs is rebalancing for y  returns
    """
    # 1. compute volatilities
    x_returns = qis.to_returns(prices=prices, is_log_returns=True, drop_first=True, freq=returns_freq)
    returns_np = x_returns.to_numpy()
    if demean:
        x = returns_np - qis.compute_ewm(returns_np, span=span)
    else:
        x = returns_np
    x_vols = qis.compute_ewm_vol(data=x, span=span)
    x_vol_norm = x / x_vols
    covar_tensor_txy = qis.compute_ewm_covar_tensor(a=x_vol_norm, span=span, nan_backfill=qis.NanBackfill.ZERO_FILL)

    # 2. compute x-factors ewm covar at rebalancing freq
    ...
    for idx, (date, value) in enumerate(rebalancing_schedule.items()):
        if value and date >= time_period.start:
            ...
            if squeeze_factor is not None:
                covar_t = squeeze_covariance_matrix(covar=covar_t, squeeze_factor=squeeze_factor)
            x_covars[date] = covar_t

    # 3. estimate betas of y-returns at different samples
    rebalancing_freqs = rebalancing_freqs[prices.columns]
    group_freqs = qis.get_group_dict(group_data=rebalancing_freqs)
    betas_freqs: Dict[str, Dict[pd.Timestamp, pd.DataFrame]] = {}
    residual_vars_freqs: Dict[str, Dict[pd.Timestamp, pd.Series]] = {}
    y_volss: Dict[str, pd.DataFrame] = {}
    x_volss: Dict[str, pd.DataFrame] = {}
    for freq, asset_tickers in group_freqs.items():
        y = qis.to_returns(prices=prices[asset_tickers], is_log_returns=True, drop_first=True, freq=freq)
        x = qis.to_returns(prices=benchmark_prices, is_log_returns=True, drop_first=True, freq=freq)
        y_vols = qis.compute_ewm_vol(data=y, span=span)
        x_vols = qis.compute_ewm_vol(data=x, span=span)
        y_vol_norm = y.divide(y_vols)
        x_vol_norm = x.divide(x_vols)
        betas_freqs[freq], residual_vars_freqs[freq], r2_t = lasso_model.estimate_rolling_betas(x=x_vol_norm, y=y_vol_norm)
        y_volss[freq] = y_vols
        x_volss[freq] = x_vols

    # 4. compute y_covars at x_covars frequency
    ...
    for idx, (date, x_covar) in enumerate(x_covars.items()):
        ...
        for freq in group_freqs.keys():
            ...
            asset_last_betas.append(betas_freqs[freq][last_update_date])
            last_residual_vars.append(residual_vars_freqs[freq][last_update_date])
            asset_last_vols.append(y_volss[freq].loc[last_update_date, :])
        asset_last_betas = pd.concat(asset_last_betas, axis=1)  # pandas with colums = assets
        last_residual_vars = pd.concat(last_residual_vars, axis=0)  # series
        asset_last_vols = pd.concat(asset_last_vols, axis=0)  # series

        # align
        asset_last_betas = asset_last_betas.reindex(index=x_covar.index).reindex(columns=prices.columns).fillna(0.0)
        last_residual_vars = last_residual_vars.reindex(index=prices.columns).fillna(0.0)
        asset_last_vols = asset_last_vols.reindex(index=prices.columns).fillna(0.0)
        ...
        if not np.isclose(residual_var_weight, 0.0):
            betas_covar += residual_var_weight * np.diag(last_residual_vars.to_numpy())
        # adjust vol
        inferred_vol = np.sqrt(np.diag(betas_covar))
        target_vol = asset_last_vols.to_numpy()
        norm_to_target_vols = target_vol / inferred_vol
        betas_covar = betas_covar * np.outer(norm_to_target_vols, norm_to_target_vols)
...
def wrapper_estimate_rolling_lasso_covar(benchmark_prices: pd.DataFrame,
    ...
    if isinstance(rebalancing_freq, str):
        ...
        if not is_apply_vol_normalised_returns:
            ...
            pd_covars = estimate_rolling_lasso_covar_vol_norm_returns_different_freq(benchmark_prices=benchmark_prices,
                                                                                     prices=prices,
                                                                                     time_period=time_period,
                                                                                     lasso_model=lasso_model,
                                                                                     returns_freq=returns_freq,
