...
def is3PtsClockWise(pt1: pt, pt2: pt, pt3: pt, error: float = CONST_EPSILON) -> bool | None:
    ...
    """
    if (is2PtsSame(pt1, pt2, error) or is2PtsSame(pt2, pt3, error) or is2PtsSame(pt1, pt3, error)):
...
def is2SegsSame(seg1: line, seg2: line, error: float = CONST_EPSILON) -> bool:
    if (is2PtsSame(seg1[0], seg2[0], error) and is2PtsSame(seg1[1], seg2[1], error)):
        return True
    elif (is2PtsSame(seg1[0], seg2[1], error) and is2PtsSame(seg1[1], seg2[0], error)):
        return True
    else:
        return False
...
def is2SegsAffine(seg1: line, seg2: line, error: float = CONST_EPSILON):
    if (is2SegsParallel(seg1, seg2, error) and isPtOnLine(seg1[0], seg2, error)):
        return True
    else:
        return False

def is2SegsOverlap(seg1: line, seg2: line, error: float = CONST_EPSILON):
    if (is2SegsParallel(seg1, seg2, error) and (isPtOnSeg(seg1[0], seg2, error) or isPtOnSeg(seg1[1], seg2, error))):
        return True
    else:
        return False
