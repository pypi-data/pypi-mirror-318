from .API import get_tracer_output, get_propstream_output, get_zillow_output
from .utils import make_body, check_address
import os
import requests
...
if os.path.exists("config.json"):
    with open("config.json", "r") as file:
        ...
        ENV_PATH, LOG_PATH = config_file.get("env_file_path"), config_file.get(
            "log_file_path"
        )
...
class AuthManager:
    ...
    def is_authenticated(
        ...
        response = session.get(base_url + "/api/leads", headers=headers)
        if response.status_code == 401:
            raise ("there is a problem with login , check your username and password")

    def make_authenticated_request(self, url=base_url, method="GET", data=None):
...
class ProcessLogic:
    ...
    def start(self):
        ...
        zillow_output = get_zillow_output(self.DATA.full_address)
        if zillow_output:
        ...
        propstream_output = get_propstream_output(self.DATA.full_address)
        ...
        for owner in self.DATA.owners:
            tracers_output += get_tracer_output(
...
class Queue:
    ...
    def starting_process(self):
        response = requests.get(f"{queue_base_url}/process")
    ...
    def process_is_done(self):
        response = requests.post(
            f"{queue_base_url}/stop", json={"request_id": self.request_id}
        )
        if response.status_code == 200:
            return True
        return False
