...
class HuggingChatSearch(SearchService):
    ...
    def process_response(self, response: str,**kwargs) -> str:
        query = re.escape(kwargs.get("query", ""))
        try:
            match = re.search(fr'{query}(.*)Ask anything',response,re.DOTALL)
            if match:
                response = match.group(1).strip()
            else:
                match = re.search(fr'{query}(.*)Model:', response, re.DOTALL)
                if match:
                    response = match.group(1).strip()
                else:
                    response = ""
        ...
        try:
            ...
            while True:
                ...
                if text == prev or (time.time() - start_time > max_time):
                    return self.process_response(response=text,query=query)
                prev = text
