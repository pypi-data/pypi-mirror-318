...
class GemgroqAPI:
    ...
    def __init__(self, groq_api_key: Optional[str] = None, gemini_api_key: Optional[str] = None):
        """
        Initialize the API with either environment variables or direct API keys.
        
        Args:
            groq_api_key (Optional[str]): Groq API key. If None, will look for GROQ_API_KEY in environment
            gemini_api_key (Optional[str]): Gemini API key. If None, will look for GEMINI_API_KEY in environment
        """
        load_dotenv()
        ...
        if not groq_key:
            raise ValueError("Groq API key not found. Please provide it directly or set GROQ_API_KEY environment variable.")
        self.groq_client = groq.Client(api_key=groq_key)
        
        # Initialize Gemini
        gemini_key = gemini_api_key or os.getenv('GEMINI_API_KEY')
        if not gemini_key:
            raise ValueError("Gemini API key not found. Please provide it directly or set GEMINI_API_KEY environment variable.")
