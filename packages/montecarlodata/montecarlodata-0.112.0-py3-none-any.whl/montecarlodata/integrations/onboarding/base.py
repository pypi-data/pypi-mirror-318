...
class BaseOnboardingService:
    ...
    def load_ssl_options(self, options: dict) -> dict:
        # Remove nulls and rename fields for they keys that start with ssl_ or skip_cert_verification
        ssl_options = {k.replace("ssl_", ""): v for k, v in options.items()
                       if v is not None and (k.startswith("ssl_") or k == "skip_cert_verification")}

        # Find cert keys and validate
        active_agent = self._user_service.get_collector_agent(options.get("dc_id"))
        cert_keys_in_options = {"ca", "cert", "key"}.intersection(ssl_options)
        if active_agent and cert_keys_in_options:
            keys_as_str = ",".join(cert_keys_in_options)
            complain_and_abort(f"{keys_as_str} options are not supported for agents.")
