...
from datafact.proj import DEFAULT_TARGET, DatasetLocalTarget, DatasetRemoteTarget, load_config, locate_draft_file
...
def publish_remote(source, target: DatasetRemoteTarget, tags: list[str]):
    dataset = dsh.remote(target['host'], profile=target['profile']).dataset(target['name'])
    dataset.upload_from_file(source, tags=tags)
    readme = load_readme()
    if readme:
        dataset.set_readme(readme)
...
def publish_cli(source, target, tag):
    ...
    if t['type'] == 'local':
        ...
        publish_remote(source_file, t, tags)
