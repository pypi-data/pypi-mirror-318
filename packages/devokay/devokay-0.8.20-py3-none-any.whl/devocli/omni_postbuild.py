...
from devolib.util_str import starts_with, has
...
def get_pipe_data():
    ...
    allowed_headers = ["name", "env", "region", "platform", "app_id", "store_type", "pipe", "sdk_build_host", "sdk_build_token"]
    headers_mapping = {
        "名称": "name",
        "环境": "env",
        "地区": "region",
...
def cmd_handle_postbuild(args):
    ...
    if 'pipe' in json:
        ...
        raise Exception("`pipe` is None or `sdk_params/sdk_hosts/sdk_token` is None")
    
    LOG_D(f'sdk_params: {sdk_params}')
    LOG_D(f'sdk_hosts: {sdk_hosts}')
    LOG_D(f'sdk_token: {sdk_token}')    

    # raise Exception("pipe setting get failed")
...
def cmd_regist(subparsers):
    ...
    parser.add_argument('-ho', '--host', type=str, default=None, help='token')
    parser.add_argument('-to', '--token', type=str, default=None, help='host')
