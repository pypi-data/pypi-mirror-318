...
class OpenSearchClient(BaseClient):
    def __init__(self, config_path: str = "config.yml", use_proxy: bool = False):
        ...
        self._requests_per_second = self.config['api']['opensearch'].get('requests_per_second', 4)
        ...
        self._concurrency_limiter = asyncio.Semaphore(self._concurrency_limit)  # Limit concurrent operations to 10
        self._batch_size = self.config['api']['opensearch'].get('batch_size', 500)
    ...
    async def _rate_limited_search(self, **kwargs):
        ...
        Ensures we don't exceed the specified requests per second limit and concurrency limit.
        """
    ...
    async def search_first_interaction_batch(self, to_addresses: List[str], index: str) -> Dict[str, Dict[str, Any]]:
        ...
        for batch in batches:
            tasks.append(self._search_first_interaction_batch(batch, index))
