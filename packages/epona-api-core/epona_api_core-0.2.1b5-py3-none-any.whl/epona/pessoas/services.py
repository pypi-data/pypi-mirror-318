...
async def save(payload: PessoaPayloadSchema, user: UserSchema) -> str:
    ...
    while attempt < 3:
        ...
        try:
            _uuid = str(uuid4())
            _uuid = f"{_uuid[:8]}-{_uuid[-4:]}"  # uuid encurtado para 13 caracteres
            pessoa = Pessoa(
...
async def update(payload: PessoaPayloadSchema, user: UserSchema) -> Union[int, None]:
    ...
    pessoa.created_at = str(pessoa.created_at)[:24]  # precisao de milesimos
    ...
    while attempt < 3:
        ...
        try:
            if pk:
                ...
                info_contato.created_at = str(info_contato.created_at)[:24]  # precisao de milesimos
            else:
                _uuid = str(uuid4())
                _uuid = f"{_uuid[:8]}-{_uuid[-4:]}"
            ...
            return InfoContatoResponseSchema(
                **{**dict(info_contato), "pessoa_id": info_contato.pessoa_suuid})
        except Exception as err:
...
async def get_contato_all(
    ...
    if not infos_contato:
        return None
    return [
        InfoContatoResponseSchema(**{**dict(info), "pessoa_id": info.pessoa_suuid})
        for info in infos_contato
    ]
