...
class Docker:
    ...
    def get_builder(self, builder_name, driver_name):
        return self.run(["buildx", "create", f"--name={builder_name}", f"--driver={driver_name}", "--bootstrap"])

    def set_builder(self, builder_name, *args, **kwargs):
        self.get_builder(builder_name, *args, **kwargs)
        return self.run(["buildx", "use", builder_name])
    ...
    def build(
        ...
        if not self._builder_set:
            self.set_builder("multi-platform-builder", driver_name="docker-container")
