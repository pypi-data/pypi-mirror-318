...
class XIVAPIAsset(XIVAPIWrapper):
    ...
    def __init__(self, session: aiohttp.ClientSession, asset_path: str, fmt: str = 'png'):
    ...
    async def create_asset_file(self, base_path: str) -> str:
        ...
        if os.path.isfile(icon_path):
            ...
            async with open(icon_path, 'wb') as icon_file:
...
class XIVAPIAssetMap(XIVAPIAsset):
    ...
    def __init__(self, session: aiohttp.ClientSession, territory: int, index: int):
        fmt = 'jpg'

        asset_path = f'/asset/map/{territory}/{index}'
        super().__init__(session, asset_path, fmt)
