...
class SourceMorphology(object):
    ...
    skybox_size : int, optional
        ...
        the sky background.
    asym_symmetric_mask : bool, optional
        If ``True`` (default), apply a rotationally symmetric version of
        the input mask during the asymmetry calculation. While this is
        the correct thing to do (i.e., ignore masked regions and their
        rotationally symmetric counterparts), it can be problematic when
        trying to minimize the asymmetry parameter in heavily masked
        systems (e.g., galaxy clusters and mergers), so the user might
        want to set this option to ``False`` in such cases.
    ...
    def __init__(self, image, segmap, label, mask=None, weightmap=None,
        ...
        self._asym_symmetric_mask = asym_symmetric_mask
    ...
    def _asymmetry_function(self, center, image, kind):
        ...
        # Apply mask to original and rotated images
        ...
        if self._asym_symmetric_mask:
            ...
            image_180 = np.where(~mask_180, image_180, 0.0)
