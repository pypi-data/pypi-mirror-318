...
def do_CL(X, Y, cfg):
    if cfg.GMVP_normalize:
        X, Y = F.normalize(X, dim=-1), F.normalize(Y, dim=-1)

    if cfg.GMVP_CL_similarity_metric == "InfoNCE_dot_prod":
        B = X.size()[0]
        logits = torch.mm(X, Y.transpose(1, 0))  # B*B
        logits = torch.div(logits, cfg.GMVP_T)
    ...
    elif cfg.GMVP_CL_similarity_metric == "EBM_dot_prod":
        criterion = nn.BCEWithLogitsLoss()
        neg_X = X.repeat((cfg.GMVP_CL_Neg_Samples, 1))
        neg_Y = torch.cat(
            [Y[cycle_idx(len(Y), i + 1)] for i in range(cfg.GMVP_CL_Neg_Samples)], dim=0
        )
        pred_pos = torch.sum(X * Y, dim=1) / cfg.GMVP_T
        pred_neg = torch.sum(neg_X * neg_Y, dim=1) / cfg.GMVP_T

        _device = pred_pos.device
        loss_pos = criterion(pred_pos, torch.ones(len(pred_pos)).to(_device))
        loss_neg = criterion(pred_neg, torch.zeros(len(pred_neg)).to(_device))
        CL_loss = loss_pos + cfg.GMVP_CL_Neg_Samples * loss_neg

        true_pos = torch.sum(pred_pos > 0).float()
        true_neg = torch.sum(pred_neg < 0).float()
        CL_acc = (true_pos + true_neg) / (len(pred_pos) + len(pred_neg))
...
def dual_CL(X, Y, cfg):
    CL_loss_1, CL_acc_1 = do_CL(X, Y, cfg)
    CL_loss_2, CL_acc_2 = do_CL(Y, X, cfg)
...
def update_augmentation_probability_JOAO(
    ...
    for n in range(25):
        ...
        count = 0
        count_stop = len(loader.dataset) // (loader.batch_size * 10) + 1
...
class ExtractSubstructureContextPair:
    ...
    def __call__(self, data, root_idx=None):
        ...
        if len(substruct_node_idxes) > 0:
            ...
            substruct_G, substruct_node_map = reset_idx(substruct_G)
            # need to reset node idx to 0 -> num_nodes - 1,
            # otherwise data obj does not make sense,
            # since the node indices in data obj must start at 0
            ...
            # need to convert center idx from original graph node ordering
            # to the new substruct node ordering
        ...
        if len(context_node_idxes) > 0:
            ...
            context_G, context_node_map = reset_idx(context_G)
        ...
        if len(context_substruct_overlap_idxes) > 0:
            ...
            # need to convert the overlap node indices, which is from the
    ...
    def __call__(self, data, masked_atom_indices=None):
        ...
        * mask rate number of atom indices
        ...
        if masked_atom_indices is None:
            ...
            # But will sample at least one atom
            num_atoms = data.x.size()[0]
        ...
        if self.mask_edge:
            # create mask edge labels by copying edge features of edges
            # that are bonded to mask atoms
            con_edge_ids = []  # connected_edge_indices
            for bond_idx, (u, v) in enumerate(data.edge_index.cpu().numpy().T):
                for atom_idx in masked_atom_indices:
                    if atom_idx in {u, v} and bond_idx not in con_edge_ids:
                        con_edge_ids.append(bond_idx)

            if len(con_edge_ids) > 0:
                # create mask edge labels by copying bond features of the bonds
                # connected to the mask atoms
                mask_edge_labels = []
                for bond_idx in con_edge_ids[::2]:  # because the
                    # edge ordering is such that two directions of a single
                    # edge occur in pairs, so to get the unique undirected
                    # edge indices, we take every 2nd edge index from list
                    mask_edge_labels.append(data.edge_attr[bond_idx].view(1, -1))

                data.mask_edge_label = torch.cat(mask_edge_labels, dim=0)
                # modify the original bond features of the bonds
                # connected to the mask atoms
                for bond_idx in con_edge_ids:
                    data.edge_attr[bond_idx] = torch.tensor([self.num_edge_type, 0])
                data.connected_edge_indices = torch.tensor(con_edge_ids[::2])
            else:
                data.mask_edge_label = torch.empty((0, 2)).to(torch.int64)
                data.connected_edge_indices = torch.tensor(con_edge_ids).to(torch.int64)
...
class ExtractSubstructureContextPair:
    ...
    def __call__(self, data, root_idx=None):
        ...
        if len(substruct_node_idxes) > 0:
            substruct_G = G.subgraph(substruct_node_idxes)
            substruct_G, substruct_node_map = reset_idx(substruct_G)  # need
        ...
        if len(context_node_idxes) > 0:
            context_G = G.subgraph(context_node_idxes)
            context_G, context_node_map = reset_idx(context_G)  # need to
