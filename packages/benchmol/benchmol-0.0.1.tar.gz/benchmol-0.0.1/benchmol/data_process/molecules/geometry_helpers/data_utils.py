...
"""Reference: https://github.com/chao1224/Geom3D/blob/e44b11d4959aeb757789a2bbe5080b4ccdb485a1/Geom3D/datasets/dataset_utils.py"""
...
def nx_to_graph_data_obj_simple(G):
    ...
    for _, node in G.nodes(data=True):
        atom_feature = [node['x0'], node['x1'], node['x2'], node['x3'], node['x4'], node['x5'], node['x6'], node['x7'],
                        node['x8']]
        atom_features_list.append(atom_feature)
    x = torch.tensor(np.array(atom_features_list), dtype=torch.long)
...
def get_graphs_within_cutoff(structure, cutoff=5.0, numerical_tol=1e-8, max_neighbours=None):
    ...
    center_indices, neighbor_indices, images, distance = center_indices[exclude_self], neighbor_indices[exclude_self], \
    images[exclude_self], distance[exclude_self]
    indices = []
...
def preiodic_augmentation_with_lattice(
    ...
    for first_indice, (atom_num, positions) in enumerate(zip(atom_num_list, positions_list)):
        ...
        first_key = 'id: {}, image: [{}, {}, {}]'.format(first_indice, original_image[0], original_image[1],
                                                         original_image[2])
    ...
    for range_idx in range(range_num):
        ...
        if second_key not in augmentation_record:
            augmentation_record[second_key] = [atom_num_list[second_indice],
    ...
    for range_idx in range(range_num):
        ...
        assert np.linalg.norm(
            neo_positions_list[second_key_index] - positions_list[second_indice] - lattice_shift) < 1e-10
