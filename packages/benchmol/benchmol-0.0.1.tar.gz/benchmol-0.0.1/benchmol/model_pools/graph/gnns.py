...
class GATConv(MessagePassing):
    ...
    def __init__(self, emb_dim, heads=2, concat=True, negative_slope=0.2, dropout=0., bias=True, **kwargs):
        super(GATConv, self).__init__(aggr='add', node_dim=0, **kwargs)
        assert emb_dim % heads == 0
        emb_dim = emb_dim // heads
        ...
        self.lin_l = torch.nn.Linear(heads * emb_dim, heads * emb_dim, bias=False)
        self.lin_r = self.lin_l

        self.att_l = torch.nn.Parameter(torch.Tensor(1, heads, emb_dim))
        self.att_r = torch.nn.Parameter(torch.Tensor(1, heads, emb_dim))
    ...
    def forward(self, x, edge_index, edge_attr, size=None):
        ...
        out = self.propagate(edge_index, x=(x_l, x_r), alpha=(alpha_l, alpha_r), size=size, edge_attr=edge_embeddings)
...
class GNN_graphpred(torch.nn.Module):
    ...
    def from_pretrained_1(self, model_file):
        print("===> Loading checkpoint '{}'".format(model_file))
        ckpt_model_state_dict = torch.load(model_file, map_location=torch.device('cpu'))
        # load parameters
...
if __name__ == "__main__":
    ...
    x = torch.Tensor(np.zeros(shape=(8, 2)))  # (8, 2)
    edge_index = torch.Tensor([[0, 3, 0, 0, 4, 4, 4, 5], [1, 2, 3, 2, 5, 6, 7, 4]])  # (2, 8)
    edge_attr = torch.Tensor(np.zeros(shape=(8, 2)))  # (8,2)
    batch = torch.Tensor([0, 0, 0, 0, 1, 1, 1, 1])
    batch_head = Namespace(x=x.long(), edge_index=edge_index.long(), edge_attr=edge_attr.long(), batch=batch.long())
    batch_tail = Namespace(x=x.long(), edge_index=edge_index.long(), edge_attr=edge_attr.long(), batch=batch.long())
    simGNN = GNN(num_layer=6, emb_dim=256, gnn_type="gin")
    print(simGNN)
    pred = simGNN(batch_head)
    print(pred)
