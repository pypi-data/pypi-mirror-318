...
Description
-----------
The implementation of services based on Flask SQLAlchemy.
"""
...
from .models_fsqla import db, User, Role, Entry
        ...
        return flask.make_response(
            {
                ...
                "users": tuple(
                    ...
                    for val in db.session.execute(
                        db.select(User.id, User.name).order_by(User.id)
                    )
                ),
            }
        )
    ...
    return flask.make_response(
        {
            ...
            "n_entries": db.session.scalar(
                db.select(db.func.count(Entry.id)).where(Entry.user_id == _user.id)
            ),
        }
    )
...
def entry():
    ...
    if entry_id is None:
        entry_ids = db.session.scalars(
            db.select(Entry.id).where(Entry.user_id == user_id).order_by(Entry.id)
        ).all()
        return flask.make_response(
            {
    ...
    _entry: Optional[Entry] = Entry.query.filter(
...
def login():
    ...
    if method == "GET":
        ...
        user: Optional[User] = User.query.filter(User.name == user_name).scalar()
        if user is None:
            return flask.abort(
...
if __name__ == "__main__":
    ...
    def init_db():
        with app.app_context():
            ...
            role_admin = Role(
                name="admin", level=0  # pyright: ignore [reportCallIssue]
            )
            role_reader = Role(
                name="reader", level=100  # pyright: ignore [reportCallIssue]
            )
            ...
            user = User(name="admin01")  # pyright: ignore [reportCallIssue]
            user.set_password("imadmin")
            user.roles.append(role_admin)
            user.roles.append(role_reader)
            Entry(
                data="admin's data 1",  # pyright: ignore [reportCallIssue]
                user_id=user.id,  # pyright: ignore [reportCallIssue]
                user=user,  # pyright: ignore [reportCallIssue]
            )
            Entry(
                data="admin's data 2",  # pyright: ignore [reportCallIssue]
                user_id=user.id,  # pyright: ignore [reportCallIssue]
                user=user,  # pyright: ignore [reportCallIssue]
            )
            db.session.add(user)

            user = User(name="reader01")  # pyright: ignore [reportCallIssue]
            user.set_password("regular")
            user.roles.append(role_reader)
            Entry(
                data="reader's data 1",  # pyright: ignore [reportCallIssue]
                user_id=user.id,  # pyright: ignore [reportCallIssue]
                user=user,  # pyright: ignore [reportCallIssue]
            )
            Entry(
                data="reader's data 2",  # pyright: ignore [reportCallIssue]
                user_id=user.id,  # pyright: ignore [reportCallIssue]
                user=user,  # pyright: ignore [reportCallIssue]
            )
            db.session.add(user)

            user = User(name="admin02")  # pyright: ignore [reportCallIssue]
            user.set_password("imanotheradmin")
            user.roles.append(role_admin)
            Entry(
                data="admin's data 3",  # pyright: ignore [reportCallIssue]
                user_id=user.id,  # pyright: ignore [reportCallIssue]
                user=user,  # pyright: ignore [reportCallIssue]
            )
            Entry(
                data="admin's data 4",  # pyright: ignore [reportCallIssue]
                user_id=user.id,  # pyright: ignore [reportCallIssue]
                user=user,  # pyright: ignore [reportCallIssue]
            )
            db.session.add(user)

            user = User(name="reader02")  # pyright: ignore [reportCallIssue]
            user.set_password("regular2")
            user.roles.append(role_reader)
            Entry(
                data="reader's data 3",  # pyright: ignore [reportCallIssue]
                user_id=user.id,  # pyright: ignore [reportCallIssue]
                user=user,  # pyright: ignore [reportCallIssue]
            )
            db.session.add(user)
