...
_DEP_TRACK_LEVEL = 3    # control analysis level of trace-back
...
def project_deploy(folder, main_app=None, main_file=None):
    ...
    dep = dependencies(root, excludes=['venv'], trace_level=_DEP_TRACK_LEVEL,
                       save_dep=(_DEP_TRACK_LEVEL>0), save_req=True, verbose=True)
    ...
    for module in dep['user']:
        ...
        if (not module_found):            
            for location in sys.path:
                ...
                if (location.endswith('site-packages')):
                    chk = [ item if os.path.isfile(os.path.join(location, item, 'direct_url.json')) else None
                            for item in os.listdir(location)
                            if item.startswith(mod_parts[0]) ]
                    if (any(chk)):
                        with open(os.path.join(location, chk[0], 'direct_url.json')) as jf:
                            dump = json.load(jf)
                ...
                src = os.path.join(location, mod_file)
        ...
        # (iii) finally try if 'import' works? --> then: NO USER MODULE!!!!! (but "KNOWN", wtf?)
        if (not module_found):
...
def project_startup_py(folder):
    ...
    # func_def = func_def.replace('(root,', f"(root=r'{folder}',")   
    user_imports = []
    for item in IMPORT_FOLDERS:
...
def project_startup_bat(folder, main_app, main_file=None, virtual=False):
    ...
    # fpath_red = fpath.strip( os.path.commonpath([os.getcwd(), fpath]) ) 
    # Note: The above 'fpath' "reduction" is required due to a shortcoming of 'fileparts()'
    # that will prepend the current working directory if the argument is only a relative path!    
    ...
    with open(bat_file, mode='wt') as bf:
        ...
        # str_call = f"from startup import *; init_session(); exec(open(r'{app_file}').read());"
