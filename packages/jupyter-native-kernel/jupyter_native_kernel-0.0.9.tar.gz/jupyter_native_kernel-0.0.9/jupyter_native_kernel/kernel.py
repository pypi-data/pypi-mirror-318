...
class NativeKernel(Kernel):
    ...
    def compile_with_compiler(self, compiler, source_filename, binary_filename, cflags=None, ldflags=None):
        cflags = ['-fPIC', '-shared', '-rdynamic'] + cflags
    ...
    def _filter_magics(self, code):
        ...
        for line in code.splitlines():
            if (line.startswith('//%') or line.startswith('!!%')) and line[3:7] != 'load':
    ...
    def query_openai_gpt4(self, question):
        try:
            response = self.ai_client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are an AI programming assistant, an expert in OpenMP, C, C++, and Fortran."},
    ...
    def _magic_ai(self, code):

        lines = code.strip().split('\n')
        options = lines[0][3:].strip()
        question = '\n'.join(lines[1:]).strip()
        ...
        response = self.query_openai_gpt4(question)
    ...
    def do_execute(self, code, silent, store_history=True,
        ...
        file_extension = '.c'
        if magics['compiler'] == 'gfortran':
            file_extension = '.f90'

        with self.new_temp_file(suffix=file_extension) as source_file:
