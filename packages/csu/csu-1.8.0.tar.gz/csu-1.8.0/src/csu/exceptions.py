...
class DecodeError(HTTPErrorMixin, InternalServiceError):
    ...
    def __init__(self, *, status_code, error, event_id):
        super().__init__(status_code, event_id=event_id)
        self.error = error

    def __str__(self):
        return f"{self.error!r} (status: {self.status_code})"
...
class UnexpectedStatusError(HTTPErrorMixin, InternalServiceError):
    ...
    def __init__(self, *, status_code, accept_statuses, event_id):
        super().__init__(status_code, event_id=event_id)
        self.accept_statuses = accept_statuses

    def __str__(self):
        return f"Expected{self.accept_statuses} (status: {self.status_code})"
...
class OpenServiceError(TaggedError, metaclass=ABCMeta):
    ...
    def __init__(self, message=None, *, details: Iterable = (), event_id):
        if message is None and self.default_message is None:
            cls = type(self)
            if cls is OpenServiceError:
                raise TypeError("OpenServiceError.__init__() missing 1 required positional argument: 'message'.")
            else:
                raise TypeError(
                    "OpenServiceError.__init__() missing 1 required positional argument: 'message'. "
                    f"Set {cls.__name__}.default_message to avoid this error."
                )
