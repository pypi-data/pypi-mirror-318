...
def test_message_callback(mocker):
    ...
    root._logger.warning.assert_called_with(
...
def test_state_change_logger(mocker):
    mocker.patch("pigeon_transitions.base.setup_logging")
    ...
    machine._logger.info.assert_not_called()
    machine.next()
    assert machine.state == "B"
    machine._logger.info.assert_called_with("Transitioned to state: B")
    machine.next()
    assert machine.state == "C"
    machine._logger.info.assert_called_with("Transitioned to state: C")


def test_state_change_logger_append(mocker):
    mocker.patch("pigeon_transitions.base.setup_logging")
