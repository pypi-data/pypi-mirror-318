...
def handle_block(content):
    ...
    try:
        ...
        if cursor.fetchone()[0] > 0:
            logger.warning(f"Duplicate block detected: {content['hash']}")
        ...
        if content["index"] > 0 and (not previous_hash or previous_hash[0] != content["previous_hash"]):
            logger.error(f"Block's previous hash does not match: {content['previous_hash']}")
