...
def fetch_longest_blockchain():
    ...
    for peer in PEERS:
        try:
            response = requests.get(f"{peer.replace('ws://', 'http://').replace('/ws', '/blocks/')}", timeout=5)
            response.raise_for_status()
            peer_chain = response.json().get("blocks", [])
            if len(peer_chain) > len(longest_chain):
                longest_chain = peer_chain
        except Exception as e:
            logger.error(f"Failed to fetch blockchain from {peer}: {e}")
