        ...
        while True:
            try:
                ...
                if msg_type == "transaction":
                    ...
                    logger.info(f"New peer announcement received: {content}")
                    peer_url = content.get("peer_url")
                    if peer_url:
                        if peer_url not in PEERS:
                            add_peer(peer_url)
                            logger.info(f"New peer {peer_url} added to the network.")
                            response = {"type": "new_peer_ack", "content": f"Peer {peer_url} added successfully"}
                        else:
                            logger.info(f"Peer {peer_url} already exists in the network.")
                            response = {"type": "new_peer_ack", "content": f"Peer {peer_url} already known"}
                    else:
                        logger.warning("Invalid new_peer message: 'peer_url' missing.")
                        response = {"type": "error", "content": "Invalid new_peer message: 'peer_url' missing"}
                    
                elif msg_type == "peer_list":
                    logger.info("Peer list request received.")
                    response = {"type": "peer_list_response", "content": {"peers": PEERS}}
