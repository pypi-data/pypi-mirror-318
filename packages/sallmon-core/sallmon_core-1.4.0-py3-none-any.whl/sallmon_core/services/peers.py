...
def save_local_blockchain(blockchain):
    """
    Save the blocks from the given blockchain into the local database, excluding the genesis block.
    """
    ...
    try:
        ...
        for block in blockchain:
            ...
            if block_index == 0:
                logger.info("Skipping genesis block.")
                continue  # Skip the genesis block

            if block_index not in local_blocks:
                cursor.execute('''
                ...
                ''', (
                    ...
                    block.get("timestamp"),
                    json.dumps(block.get("data")),  # Ensure the data is serialized as JSON
                    block.get("hash")
                ))
