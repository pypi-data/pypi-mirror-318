...
# Fetch peers from the /peers endpoint
def fetch_hardcoded_peers():
    """Fetch hardcoded peers from the localhost /peers endpoint."""
    try:
        response = requests.get("http://localhost:1337/peers", timeout=5)
        response.raise_for_status()
        data = response.json()
        peers = data.get("peers", [])
        logger.info(f"✅ Successfully fetched hardcoded peers: {peers}")
        return peers
    except requests.RequestException as e:
        logger.error(f"❌ Failed to fetch hardcoded peers: {e}")
        return []

# Hardcoded peers are fetched from the endpoint
HARD_CODED_PEERS = fetch_hardcoded_peers()
        ...
        for block in blockchain:
            if block["block_index"] not in local_blocks:
                if validate_block(block):
                    ...
                    logger.info(f"✅ Block {block['block_index']} saved.")
                else:
                    logger.warning(f"❌ Skipped invalid block: {block}")
        conn.commit()
