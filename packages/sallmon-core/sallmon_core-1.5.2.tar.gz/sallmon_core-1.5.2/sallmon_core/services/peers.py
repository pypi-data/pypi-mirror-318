...
def save_local_blockchain(blockchain):
    ...
    try:
        ...
        # Fetch local block hashes and indices
        cursor.execute('SELECT hash FROM blocks')
        local_hashes = {row[0] for row in cursor.fetchall()}
        logger.info(f"🗄️ Local block hashes found: {local_hashes}")

        cursor.execute('SELECT block_index FROM blocks')
        local_blocks = {row[0] for row in cursor.fetchall()}
        logger.info(f"🗄️ Local block indices found: {local_blocks}")

        # Save all valid missing blocks
        for block in blockchain:
            # Skip duplicate blocks
            if block["hash"] in local_hashes or block["block_index"] in local_blocks:
                logger.info(f"⚠️ Duplicate block {block['block_index']} or hash {block['hash']} already exists. Skipping.")
                continue

            # Skip blocks with missing or empty data
            if not block.get("data") or not isinstance(block["data"], list) or len(block["data"]) == 0:
                logger.warning(f"⚠️ Block {block['block_index']} has no valid data. Skipping.")
                continue
            ...
            if validate_block(block):
                ...
                logger.warning(f"⚠️ Block {block['block_index']} is invalid and was not saved.")
        conn.commit()
