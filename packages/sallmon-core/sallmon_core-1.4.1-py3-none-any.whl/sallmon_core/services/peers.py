...
def save_local_blockchain(blockchain):
    """
    Save the blocks from the given blockchain into the local database, ensuring the schema matches.
    """
    ...
    try:
        ...
        # Fetch local block indices to avoid duplicates
        ...
        for block in blockchain:
            # Validate and map the incoming block fields to the database schema
            try:
                ...
                if block_index is None or previous_hash is None or timestamp is None or block_hash is None:
                    logger.warning(f"Block missing required fields: {block}")
                    continue

                if block_index in local_blocks:
                    logger.info(f"Skipping already saved block: {block_index}")
                    continue
                ...
                ''', (
                    block_index,
                    previous_hash,
                    int(timestamp) if timestamp.isdigit() else 0,  # Default to 0 if timestamp is invalid
                    json.dumps(data) if data else "{}",  # Ensure data is serialized as JSON
                    block_hash
                ))
                logger.info(f"✅ Block {block_index} added to the database.")
            except Exception as e:
                logger.error(f"❌ Failed to process block {block}: {e}")
                continue

        conn.commit()
        logger.info("📦 All valid blocks saved to the database.")
