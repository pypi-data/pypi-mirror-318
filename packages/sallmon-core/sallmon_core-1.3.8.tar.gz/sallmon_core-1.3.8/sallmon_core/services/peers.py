...
def save_local_blockchain(blockchain):
    """
    Determine the blocks that are missing locally and return them as messages
    for processing and distribution to the local WebSocket service or node.
    """
    ...
    try:
        ...
        missing_blocks = [block for block in blockchain if block["index"] not in local_blocks]
        logger.info(f"📦 Missing blocks identified: {[block['index'] for block in missing_blocks]}")

        # Return missing blocks as messages
        messages = [
            {
                "type": "block",
                "content": {
                    ...
                    "data": block["data"],
                    "hash": block["hash"],
                    "created_at": block.get("created_at")  # Include created_at if present
                }
            }
            for block in missing_blocks
        ]

        logger.info(f"📝 Prepared {len(messages)} messages for missing blocks.")
        return messages
        ...
        logger.error(f"❌ Failed to determine missing blocks: {e}")
        raise
    finally:
        conn.close()
