...
def save_local_blockchain(blockchain):
    ...
    try:
        ...
        for idx, block in enumerate(blockchain):
            try:
                logger.debug(f"📦 Inserting block {idx + 1}/{len(blockchain)}:")
                logger.debug(f"  - Block Index: {block.get('index', 'N/A')}")
                logger.debug(f"  - Previous Hash: {block.get('previous_hash', 'N/A')}")
                logger.debug(f"  - Timestamp: {block.get('timestamp', 'N/A')}")
                logger.debug(f"  - Data: {block.get('data', 'N/A')}")
                logger.debug(f"  - Hash: {block.get('hash', 'N/A')}")
                ...
                logger.error(f"🚨 Block {idx + 1} is missing key: {ke}. Block: {block}")
                raise
        conn.commit()
