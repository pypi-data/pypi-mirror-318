...
def save_local_blockchain(blockchain):
    ...
    """
    # Ensure the database is stored in the correct path
    ...
    try:
        ...
        for idx, block in enumerate(blockchain):
            try:
                logger.debug(f"📦 Inserting block {idx + 1}/{len(blockchain)}: {block}")

                # Validate that all required keys are present
                missing_keys = [key for key in ["block_index", "previous_hash", "timestamp", "data", "hash"] if key not in block or block[key] is None]
                if missing_keys:
                ...
                ''', (
                    ...
                    block["previous_hash"],
                    block["timestamp"],
                    block["data"],  # Store data exactly as received
                    block["hash"]
                ))
