...
def write_table_with_metafields(spark, df, table_name, mode, skip_key):
    """
    ...
    ####### Get Notebook name and run id ############import re
    import re
    ...
    try:
        ...
        match = re.match(r"(.*)_(.*)", spark_app_name)
        if match:
            notebook_name = match.group(1)  # Everything before the last underscore
            fabric_job_id = match.group(2)  # Everything after the last underscore
    except Exception:
        # Silently fail and leave notebook_name and fabric_job_id as empty strings
        pass
