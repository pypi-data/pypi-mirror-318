...
class Reader:
    ...
    def _makeMaterials(self, materials, elements, isotopes):
        ...
        for material in materials:
            ...
            if not material["components"]:
                ...
                mat = _g4.MaterialSingleElement(
                    name,
                    Z,
                    a,
                    density,
                    registry=self._registry,
                    tolerateZeroDensity=True,
                )
                mat.set_state(state)
                ...
                for comp in material["components"]:
                    ...
                    if comp_type == "fraction":
                        # abundance = float(comp.get("n", 0.0))
                        abundance = _defines.Expression(
                            "n", comp.get("n", 0.0), self._registry, False
                        )
    ...
    def parseBox(self, node):
        ...
        x = _defines.Expression(
            solid_name + "_pX", "{}".format(node.attributes["x"].value), self._registry
        )
        y = _defines.Expression(
            solid_name + "_pY", "{}".format(node.attributes["y"].value), self._registry
        )
        z = _defines.Expression(
            solid_name + "_pZ", "{}".format(node.attributes["z"].value), self._registry
        )
        try:
    ...
    def parseTube(self, node):
        ...
        try:
            rmin = _defines.Expression(
                solid_name + "_pRMin", node.attributes["rmin"].value, self._registry
            )
        except KeyError:
    ...
    def parseCutTube(self, node):
        ...
        lx = _defines.Expression(
            solid_name + "_plNorm_x", node.attributes["lowX"].value, self._registry
        )
        ly = _defines.Expression(
            solid_name + "_plNorm_y", node.attributes["lowY"].value, self._registry
        )
        lz = _defines.Expression(
            solid_name + "_plNorm_z", node.attributes["lowZ"].value, self._registry
        )
        hx = _defines.Expression(
            solid_name + "_phNorm_x", node.attributes["highX"].value, self._registry
        )
        hy = _defines.Expression(
            solid_name + "_phNorm_y", node.attributes["highY"].value, self._registry
        )
        hz = _defines.Expression(
            solid_name + "_phNorm_z", node.attributes["highZ"].value, self._registry
        )
    ...
    def parseCone(self, node):
        ...
        try:
            rmin1 = _defines.Expression(
                solid_name + "_pRMin1", node.attributes["rmin1"].value, self._registry
            )
        except KeyError:
            rmin1 = _defines.Expression(solid_name + "_pRMin1", "0", self._registry)
        try:
            rmin2 = _defines.Expression(
                solid_name + "_pRMin2", node.attributes["rmin2"].value, self._registry
            )
        except KeyError:
        ...
        rmax1 = _defines.Expression(
            solid_name + "_pRMax1", node.attributes["rmax1"].value, self._registry
        )
    ...
    def parsePara(self, node):
        ...
        alpha = _defines.Expression(
            solid_name + "_pAlpha", node.attributes["alpha"].value, self._registry
        )
    ...
    def parseTrap(self, node):
        ...
        alp2 = _defines.Expression(
            solid_name + "_pAlp2", node.attributes["alpha2"].value, self._registry
        )
    ...
    def parseSphere(self, node):
        ...
        try:
            starttheta = _defines.Expression(
                solid_name + "_pSTheta",
                node.attributes["starttheta"].value,
                self._registry,
            )
        except KeyError:
        ...
        deltaphi = _defines.Expression(
            solid_name + "_pDPhi", node.attributes["deltaphi"].value, self._registry
        )
        deltatheta = _defines.Expression(
            solid_name + "_pDTheta", node.attributes["deltatheta"].value, self._registry
        )
    ...
    def parseTorus(self, node):
        ...
        rtor = _defines.Expression(
            solid_name + "_pRtor", node.attributes["rtor"].value, self._registry
        )
        sphi = _defines.Expression(
            solid_name + "_pSphi", node.attributes["startphi"].value, self._registry
        )
    ...
    def parseGenericPolycone(self, node):
        ...
        for chNode in node.childNodes:
            r = _defines.Expression(
                solid_name + "_rzpoint_r" + str(i),
                chNode.attributes["r"].value,
                self._registry,
            )
            z = _defines.Expression(
                solid_name + "_rzpoint_z" + str(i),
                chNode.attributes["z"].value,
                self._registry,
            )
            R.append(r)
    ...
    def parseGenericPolyhedra(self, node):
        ...
        _g4.solid.GenericPolyhedra(
            solid_name, sphi, dphi, nside, R, Z, self._registry, lunit, aunit
        )
    ...
    def parseEllipsoid(self, node):
        ...
        try:
            bcut = _defines.Expression(
                solid_name + "_zcut1", node.attributes["zcut1"].value, self._registry
            )
        except KeyError:
    ...
    def parseEllipticalCone(self, node):
        ...
        pxSemiAxis = _defines.Expression(
            solid_name + "_pxSemiAxis", node.attributes["dx"].value, self._registry
        )
        pySemiAxis = _defines.Expression(
            solid_name + "_pySemiAxis", node.attributes["dy"].value, self._registry
        )
        zMax = _defines.Expression(
            solid_name + "_zMax", node.attributes["zmax"].value, self._registry
        )
        pzTopCut = _defines.Expression(
            solid_name + "_pzTopCut", node.attributes["zcut"].value, self._registry
        )
    ...
    def parseHype(self, node):
        ...
        try:
            innerStereo = _defines.Expression(
                solid_name + "_innerStereo",
                node.attributes["inst"].value,
                self._registry,
            )
        except KeyError:
        ...
        outerRadius = _defines.Expression(
            solid_name + "_outerRadius", node.attributes["rmax"].value, self._registry
        )
        outerStereo = _defines.Expression(
            solid_name + "_outerStereo", node.attributes["outst"].value, self._registry
        )
        halfLenZ = _defines.Expression(
            solid_name + "_halfLenZ", node.attributes["z"].value, self._registry
        )
    ...
    def parseExtrudedSolid(self, node):
        ...
        for chNode in node.childNodes:
            if chNode.tagName == "twoDimVertex":
                x = _defines.Expression(
                    solid_name + "_" + str(ivec) + "_x",
                    chNode.attributes["x"].value,
                    self._registry,
                )
                y = _defines.Expression(
                    solid_name + "_" + str(ivec) + "_y",
                    chNode.attributes["y"].value,
                    self._registry,
                )
                pPolygon.append([x, y])
                ivec = ivec + 1
            if chNode.tagName == "section":
                scale = _defines.Expression(
                    solid_name + "_" + str(isec) + "_scale",
                    chNode.attributes["scalingFactor"].value,
                    self._registry,
                )
                xoff = _defines.Expression(
                    solid_name + "_" + str(isec) + "_xoff",
                    chNode.attributes["xOffset"].value,
                    self._registry,
                )
                yoff = _defines.Expression(
                    solid_name + "_" + str(isec) + "_yoff",
                    chNode.attributes["yOffset"].value,
                    self._registry,
                )
                zpos = _defines.Expression(
                    solid_name + "_" + str(isec) + "_zpos",
                    chNode.attributes["zPosition"].value,
                    self._registry,
                )
    ...
    def parseTwistedTrd(self, node):
        ...
        _g4.solid.TwistedTrd(
            solid_name, twistedAngle, x1, x2, y1, y2, z, self._registry, lunit, aunit
        )

    def parseTwistedTubs(self, node):
        solid_name = node.attributes["name"].value

        twist = _defines.Expression(
            solid_name + "_twistedangle",
            node.attributes["twistedangle"].value,
            self._registry,
        )

        inner_rad = _defines.Expression(
            solid_name + "_endinnerrad",
            node.attributes["endinnerrad"].value,
            self._registry,
        )

        outer_rad = _defines.Expression(
            solid_name + "_endouterrad",
    ...
    def parseTessellatedSolid(self, node):
        ...
        _g4.solid.TessellatedSolid(
            solid_name,
            facet_list,
            self._registry,
            _g4.solid.TessellatedSolid.MeshType.Gdml,
        )
        ...
        try:
            ...
            try:
                rotation = self.parseVector(
                    node.getElementsByTagName("rotationref")[0], "rotationref", False
                )
            except IndexError:
                rotation = _defines.Rotation(
                    "identity", "0", "0", "0", "rad", self._registry, False
                )

        _g4.solid.Union(
            solid_name,
            self._registry.solidDict[first],
            self._registry.solidDict[second],
            [rotation, position],
            self._registry,
        )
    ...
    def parseOpticalSurface(self, node):
        ...
        value = _defines.Expression(
            solid_name + "_value", node.attributes["value"].value, self._registry
        )

        surf = _g4.solid.OpticalSurface(
            solid_name, finish, model, surf_type, value, self._registry, True
        )
    ...
    def extractStructureNodeData(self, node, materialSubstitutionNames=None):
        if node.nodeType == node.ELEMENT_NODE:
            ...
            if node_name == "volume":
                ...
                vol = _g4.LogicalVolume(
                    self._registry.solidDict[solid],
                    mat,
                    name,
                    registry=self._registry,
                    auxiliary=aux_list,
        ...
        for chNode in node.childNodes:
            if chNode.nodeType == node.ELEMENT_NODE and chNode.tagName == "physvol":
                ...
                try:
                    ...
                    try:
                        position = self.parseVector(
                            chNode.getElementsByTagName("position")[0],
                            "position",
                            False,
                        )
                    except IndexError:
                ...
                try:
                    ...
                    try:
                        rotation = self.parseVector(
                            chNode.getElementsByTagName("rotation")[0],
                            "rotation",
                            False,
                        )
                    except IndexError:
                ...
                try:
                    ...
                    try:
                        scale = self.parseVector(
                            chNode.getElementsByTagName("scale")[0], "scale", False
                        )
                    except IndexError:
                        scale = None
                ...
                for ppsChNode in pps.childNodes:
                    ...
                    # default rotation
                    rotation = _defines.Rotation(
                        pvol_name, "0", "0", "0", "rad", self._registry, False
                    )
                    ...
                    for ppsChNodeTag in ppsChNode.childNodes:
                        if ppsChNodeTag.tagName == "position":
                            ...
                            position = self._registry.defineDict[
                                ppsChNodeTag.attributes["ref"].value
                            ]
                            ...
                            x = _defines.Expression(
                                pvol_name + "_Box_pX",
                                "{}".format(ppsChNodeTag.attributes["x"].value),
                                self._registry,
                                False,
                            )
                            y = _defines.Expression(
                                pvol_name + "_Box_pY",
                                "{}".format(ppsChNodeTag.attributes["y"].value),
                                self._registry,
                                False,
                            )
                            z = _defines.Expression(
                                pvol_name + "_Box_pZ",
                                "{}".format(ppsChNodeTag.attributes["z"].value),
                                self._registry,
                                False,
                            )
                            ...
                            try:
                                pRMin = _defines.Expression(
                                    pvol_name + "_Tubs_rMin",
                                    "{}".format(ppsChNodeTag.attributes["InR"].value),
                                    self._registry,
                                    False,
                                )
                            except KeyError:
                                pRMin = _defines.Expression(
                                    pvol_name + "_Tubs_rMin", "0", self._registry, False
                                )

                            pRMax = _defines.Expression(
                                pvol_name + "_Tubs_rMax",
                                "{}".format(ppsChNodeTag.attributes["OutR"].value),
                                self._registry,
                                False,
                            )
                            pDz = _defines.Expression(
                                pvol_name + "_Tubs_Dz",
                                "{}".format(ppsChNodeTag.attributes["hz"].value),
                                self._registry,
                                False,
                            )
                            try:
                                ...
                                pSPhi = _defines.Expression(
                                    pvol_name + "_Tubs_SPhi", "0", self._registry, False
                                )

                            pDPhi = _defines.Expression(
                                pvol_name + "_Tubs_DPhi",
                                "{}".format(ppsChNodeTag.attributes["DeltaPhi"].value),
                                self._registry,
                                False,
                            )
                            ...
                            dim = _g4.ParameterisedVolume.TubeDimensions(
                                pRMin, pRMax, pDz, pSPhi, pDPhi, lunit, aunit
                            )
                        elif ppsChNodeTag.tagName == "cone_dimensions":
                            try:
                                pRMin1 = _defines.Expression(
                                    pvol_name + "_Cone_rMin1",
                                    "{}".format(ppsChNodeTag.attributes["rmin1"].value),
                                    self._registry,
                                    False,
                                )
                            except KeyError:
                            ...
                            pRMax1 = _defines.Expression(
                                pvol_name + "_Cone_rMax1",
                                "{}".format(ppsChNodeTag.attributes["rmax1"].value),
                                self._registry,
                                False,
                            )

                            try:
                                pRMin2 = _defines.Expression(
                                    pvol_name + "_Cone_rMin2",
                                    "{}".format(ppsChNodeTag.attributes["rmin2"].value),
                                    self._registry,
                                    False,
                                )
                            except KeyError:
                            ...
                            pRMax2 = _defines.Expression(
                                pvol_name + "_Cone_rMax2",
                                "{}".format(ppsChNodeTag.attributes["rmax2"].value),
                                self._registry,
                                False,
                            )
                            pDz = _defines.Expression(
                            ...
                            dim = _g4.ParameterisedVolume.ConeDimensions(
                                pRMin1,
                                pRMax1,
                                pRMin2,
                                pRMax2,
                            ...
                            pRMax = _defines.Expression(
                                pvol_name + "_Orb_r",
                                "{}".format(ppsChNodeTag.attributes["r"].value),
                                self._registry,
                                False,
                            )
                            if "lunit" in ppsChNodeTag.attributes:
                            ...
                            try:
                                pRMin = _defines.Expression(
                                    pvol_name + "_Sphere_rMin",
                                    "{}".format(ppsChNodeTag.attributes["rmin"].value),
                                    self._registry,
                                    False,
                                )
                            except KeyError:
                            ...
                            pRMax = _defines.Expression(
                                pvol_name + "_Sphere_rMax",
                                "{}".format(ppsChNodeTag.attributes["rmax"].value),
                                self._registry,
                                False,
                            )
                            try:
                            ...
                            dim = _g4.ParameterisedVolume.SphereDimensions(
                                pRMin,
                                pRMax,
                                pSPhi,
                                pDPhi,
                            ...
                            pRMin = _defines.Expression(
                                pvol_name + "_Torus_rMin",
                                "{}".format(ppsChNodeTag.attributes["rmin"].value),
                                self._registry,
                                False,
                            )
                            pRMax = _defines.Expression(
                                pvol_name + "_Torus_rMax",
                                "{}".format(ppsChNodeTag.attributes["rmax"].value),
                                self._registry,
                                False,
                            )
                            pRTor = _defines.Expression(
                                pvol_name + "_Torus_rTor",
                                "{}".format(ppsChNodeTag.attributes["rtor"].value),
                                self._registry,
                                False,
                            )
                            try:
                            ...
                            dim = _g4.ParameterisedVolume.TorusDimensions(
                                pRMin, pRMax, pRTor, pSPhi, pDPhi, lunit, aunit
                            )
                        elif ppsChNodeTag.tagName == "hype_dimensions":
                            innerRadius = _defines.Expression(
                                pvol_name + "_Hype_rMin",
                                "{}".format(ppsChNodeTag.attributes["rmin"].value),
                                self._registry,
                                False,
                            )
                            outerRadius = _defines.Expression(
                                pvol_name + "_Hype_rMax",
                                "{}".format(ppsChNodeTag.attributes["rmax"].value),
                                self._registry,
                                False,
                            )
                            innerStereo = _defines.Expression(
                                pvol_name + "_Hype_iHst",
                                "{}".format(ppsChNodeTag.attributes["inst"].value),
                                self._registry,
                                False,
                            )
                            outerStereo = _defines.Expression(
                                pvol_name + "_Hype_ouSt",
                                "{}".format(ppsChNodeTag.attributes["outst"].value),
                                self._registry,
                                False,
                            )
                            lenZ = _defines.Expression(
                                pvol_name + "_Hype_z",
                                "{}".format(ppsChNodeTag.attributes["z"].value),
                                self._registry,
                                False,
                            ...
                            dim = _g4.ParameterisedVolume.HypeDimensions(
                                innerRadius,
                                outerRadius,
                                innerStereo,
                                outerStereo,
                            ...
                            pX = _defines.Expression(
                                pvol_name + "_Para_x",
                                "{}".format(ppsChNodeTag.attributes["x"].value),
                                self._registry,
                                False,
                            )
                            pY = _defines.Expression(
                                pvol_name + "_Para_y",
                                "{}".format(ppsChNodeTag.attributes["y"].value),
                                self._registry,
                                False,
                            )
                            pZ = _defines.Expression(
                                pvol_name + "_Para_z",
                                "{}".format(ppsChNodeTag.attributes["z"].value),
                                self._registry,
                                False,
                            )
                            pAlpha = _defines.Expression(
                                pvol_name + "_Para_alpha",
                                "{}".format(ppsChNodeTag.attributes["alpha"].value),
                                self._registry,
                                False,
                            )
                            pTheta = _defines.Expression(
                                pvol_name + "_Para_theta",
                                "{}".format(ppsChNodeTag.attributes["theta"].value),
                                self._registry,
                                False,
                            )
                            pPhi = _defines.Expression(
                                pvol_name + "_Para_phi",
                                "{}".format(ppsChNodeTag.attributes["phi"].value),
                                self._registry,
                                False,
                            )
                            if "lunit" in ppsChNodeTag.attributes:
                            ...
                            dim = _g4.ParameterisedVolume.ParaDimensions(
                                pX, pY, pZ, pAlpha, pTheta, pPhi, lunit, aunit
                            )
                        elif ppsChNodeTag.tagName == "trd_dimensions":
                            pX1 = _defines.Expression(
                                pvol_name + "_Trd_x1",
                                "{}".format(ppsChNodeTag.attributes["x1"].value),
                                self._registry,
                                False,
                            )
                            pX2 = _defines.Expression(
                                pvol_name + "_Trd_x2",
                                "{}".format(ppsChNodeTag.attributes["x2"].value),
                                self._registry,
                                False,
                            )
                            pY1 = _defines.Expression(
                                pvol_name + "_Trd_y1",
                                "{}".format(ppsChNodeTag.attributes["y1"].value),
                                self._registry,
                                False,
                            )
                            pY2 = _defines.Expression(
                                pvol_name + "_Trd_y2",
                                "{}".format(ppsChNodeTag.attributes["y2"].value),
                                self._registry,
                                False,
                            )
                            pZ = _defines.Expression(
                            ...
                            dim = _g4.ParameterisedVolume.TrdDimensions(
                                pX1, pX2, pY1, pY2, pZ, lunit
                            )
                            ...
                            pTheta = _defines.Expression(
                                pvol_name + "_Trap_dTheta",
                                "{}".format(ppsChNodeTag.attributes["theta"].value),
                                self._registry,
                                False,
                            )
                            pDPhi = _defines.Expression(
                                pvol_name + "_Trap_dPhi",
                                "{}".format(ppsChNodeTag.attributes["phi"].value),
                                self._registry,
                                False,
                            )
                            pDy1 = _defines.Expression(
                                pvol_name + "_Trap_dy1",
                                "{}".format(ppsChNodeTag.attributes["y1"].value),
                                self._registry,
                                False,
                            )
                            pDx1 = _defines.Expression(
                                pvol_name + "_Trap_dx1",
                                "{}".format(ppsChNodeTag.attributes["x1"].value),
                                self._registry,
                                False,
                            )
                            pDx2 = _defines.Expression(
                                pvol_name + "_Trap_dx2",
                                "{}".format(ppsChNodeTag.attributes["x2"].value),
                                self._registry,
                                False,
                            )
                            pAlp1 = _defines.Expression(
                                pvol_name + "_Trap_dAlp1",
                                "{}".format(ppsChNodeTag.attributes["alpha1"].value),
                                self._registry,
                                False,
                            )
                            pDy2 = _defines.Expression(
                                pvol_name + "_Trap_dy2",
                                "{}".format(ppsChNodeTag.attributes["y2"].value),
                                self._registry,
                                False,
                            )
                            pDx3 = _defines.Expression(
                                pvol_name + "_Trap_dy3",
                                "{}".format(ppsChNodeTag.attributes["x3"].value),
                                self._registry,
                                False,
                            )
                            pDx4 = _defines.Expression(
                                pvol_name + "_Trap_dx4",
                                "{}".format(ppsChNodeTag.attributes["x4"].value),
                                self._registry,
                                False,
                            )
                            pAlp2 = _defines.Expression(
                                pvol_name + "_Trap_dAlp2",
                                "{}".format(ppsChNodeTag.attributes["alpha2"].value),
                                self._registry,
                                False,
                            )
                            ...
                            dim = _g4.ParameterisedVolume.TrapDimensions(
                                pDz,
                                pTheta,
                                pDPhi,
                                pDy1,
                            ...
                            startphi = _defines.Expression(
                                pvol_name + "_Polycone_startphi",
                                "{}".format(ppsChNodeTag.attributes["startPhi"].value),
                                self._registry,
                                False,
                            )
                            deltaphi = _defines.Expression(
                                pvol_name + "_Polycone_deltaphi",
                                "{}".format(ppsChNodeTag.attributes["openPhi"].value),
                                self._registry,
                                False,
                            )
                            ...
                            for ppsChNodeChildren in ppsChNodeTag.childNodes:
                                rmin = _defines.Expression(
                                    pvol_name + "_PolyconeZPlane" + str(i) + "_rmin",
                                    ppsChNodeChildren.attributes["rmin"].value,
                                    self._registry,
                                    False,
                                )
                                rmax = _defines.Expression(
                                    pvol_name + "_PolyconeZPlane" + str(i) + "_rmax",
                                    ppsChNodeChildren.attributes["rmax"].value,
                                    self._registry,
                                    False,
                                )
                                z = _defines.Expression(
                                    pvol_name + "_PolyconeZPlane" + str(i) + "_z",
                                    ppsChNodeChildren.attributes["z"].value,
                                    self._registry,
                                    False,
                                )
                                Rmin.append(rmin)
                                Rmax.append(rmax)
                                Z.append(z)
                            dim = _g4.ParameterisedVolume.PolyconeDimensions(
                                startphi, deltaphi, Z, Rmin, Rmax, lunit, aunit
                            )

                        elif ppsChNodeTag.tagName == "polyhedra_dimensions":
                            startphi = _defines.Expression(
                                pvol_name + "_Polyhedra_startphi",
                                "{}".format(ppsChNodeTag.attributes["startPhi"].value),
                                self._registry,
                                False,
                            )
                            deltaphi = _defines.Expression(
                                pvol_name + "_Polyhedra_deltaphi",
                                "{}".format(ppsChNodeTag.attributes["openPhi"].value),
                                self._registry,
                                False,
                            )
                            numsides = _defines.Expression(
                                pvol_name + "_Polyhedra_numsides",
                                "{}".format(ppsChNodeTag.attributes["numSide"].value),
                                self._registry,
                                False,
                            )
                            ...
                            for ppsChNodeChildren in ppsChNodeTag.childNodes:
                                rmin = _defines.Expression(
                                    pvol_name + "_PolyhedraZPlane" + str(i) + "_rmin",
                                    ppsChNodeChildren.attributes["rmin"].value,
                                    self._registry,
                                    False,
                                )
                                rmax = _defines.Expression(
                                    pvol_name + "_PolyhedraZPlane" + str(i) + "_rmax",
                                    ppsChNodeChildren.attributes["rmax"].value,
                                    self._registry,
                                    False,
                                )
                                z = _defines.Expression(
                                    pvol_name + "_PolyhedraZPlane" + str(i) + "_z",
                                    ppsChNodeChildren.attributes["z"].value,
                                    self._registry,
                                    False,
                                )
                            ...
                            dim = _g4.ParameterisedVolume.PolyhedraDimensions(
                                startphi,
                                deltaphi,
                                numsides,
                                Z,
                            ...
                            pxSemiAxis = _defines.Expression(
                                pvol_name + "_Ellipsoid_ax",
                                "{}".format(ppsChNodeTag.attributes["ax"].value),
                                self._registry,
                                False,
                            )
                            pySemiAxis = _defines.Expression(
                                pvol_name + "_Ellipsoid_by",
                                "{}".format(ppsChNodeTag.attributes["by"].value),
                                self._registry,
                                False,
                            )
                            pzSemiAxis = _defines.Expression(
                                pvol_name + "_Ellipsoid_cz",
                                "{}".format(ppsChNodeTag.attributes["cz"].value),
                                self._registry,
                                False,
                            )
                            pzBottomCut = _defines.Expression(
                                pvol_name + "_Ellipsoid_zcut1",
                                "{}".format(ppsChNodeTag.attributes["zcut1"].value),
                                self._registry,
                                False,
                            )
                            pzTopCut = _defines.Expression(
                                pvol_name + "_Ellipsoid_zcut2",
                                "{}".format(ppsChNodeTag.attributes["zcut2"].value),
                                self._registry,
                                False,
                            )
                            ...
                            dim = _g4.ParameterisedVolume.EllipsoidDimensions(
                                pxSemiAxis,
                                pySemiAxis,
                                pzSemiAxis,
                                pzBottomCut,
                ...
                try:
                    ...
                    num = -1
                ndivisions = _defines.Expression(
                    pvol_name + "_ndivisions", num, self._registry, False
                )
