...
def rootMatrix2pyg4ometry(matrix, reader):
    if _ROOT.addressof(matrix) in reader.matrices:
        ...
        rotation = _np.frombuffer(
            reader.matrices[_ROOT.addressof(matrix)]["rootObj"].GetRotationMatrix(),
            dtype=_np.float64,
            count=9,
        ).reshape((3, 3))
        translation = _np.frombuffer(
            reader.matrices[_ROOT.addressof(matrix)]["rootObj"].GetTranslation(),
            dtype=_np.float64,
            count=3,
        )
        ...
        reader.matrices[_ROOT.addressof(matrix)] = {
            "name": matrix.GetName(),
            "count": 1,
            "pyg4RotObj": rotPyG4,
            "pyg4TraObj": traPyG4,
...
def rootShape2pyg4ometry(shape, reader, warnAboutBadShapes=True):
    ...
    if shapeClass == "TGeoBBox":
        ...
        if c1 or c2:
            ...
            if warnAboutBadShapes:
                if not _isClose(shape.GetAlpha1(), shape.GetAlpha2()):
                    print(
                        "warning: Alpha2 != Alpha1 and GDML doesn't include Alpha2 for TGeoGtra in shape '",
                        shape.GetName,
                        "'",
                    )
            # see https://root.cern/doc/master/classTGDMLWrite.html#a4ce496683e55300ec3de53a43d707427
            shapePyG4 = _g4.solid.TwistedTrap(
                shapeName,
                shape.GetTwistAngle(),
                shape.GetDZ() * 2,
                shape.GetTheta(),
        ...
        if shape.GetA() <= 0 or shape.GetB() <= 0 or shape.GetDz() <= 0:
            return None
        shapePyG4 = _g4.solid.EllipticalTube(
            shapeName,
            shape.GetA() * 2,
            shape.GetB() * 2,
            shape.GetDz() * 2,
        ...
        if shape.GetDz() <= 0 or shape.GetRhi() <= 0:
            return None
        shapePyG4 = _g4.solid.Paraboloid(
            shapeName,
            shape.GetDz() * 2,
            shape.GetRlo(),
            shape.GetRhi(),
        ...
        for iVert in range(0, nVert, 1):
            ...
            verts.append([vertex.x() * 10, vertex.y() * 10, vertex.z() * 10])
        ...
        [
            ...
            matROOT,
            traROOT,
        ] = rootMatrix2pyg4ometry(boolNodeLeftMatrix, reader)
        [
            boolNodeRightRotPyG4,
            boolNodeRightTraPyG4,
            boolNodeRightScaPyG4,
            matROOT,
            traROOT,
        ] = rootMatrix2pyg4ometry(boolNodeRightMatrix, reader)
        ...
        if boolNode.Class_Name() == "TGeoUnion":
            ...
            shapePyG4 = _g4.solid.Subtraction(
                shapeName,
                boolLeftShapePyG4,
                boolRightShapePyG4,
                [boolNodeRightRotPyG4, boolNodeRightTraPyG4],
            ...
            shapePyG4 = _g4.solid.Intersection(
                shapeName,
                boolLeftShapePyG4,
                boolRightShapePyG4,
                [boolNodeRightRotPyG4, boolNodeRightTraPyG4],
...
class Reader:
    ...
    def load(
        ...
        self.recurseVolumeTree(
            self.topVolume,
            0,
            self.maximumDepth,
            warnAboutBadShapes,
            dontLoadOverlapNodes,
        )
    ...
    def _ROOTMatStateToGeant4MatState(self, rootMaterialState):
        ...
        """
        states = {
            0: None,  # as per hthe default in MaterialBase
            1: "solid",
            2: "liquid",
            3: "gas",
        }
    ...
    def loadMaterials(self, upgradeVacuumToG4Galactic):
        ...
        for iMat in range(0, materialList.GetEntries(), 1):
            ...
            if not material.IsMixture():
                Z = material.GetZ()
                A = material.GetA()
                g4Mat = _g4.MaterialSingleElement(
                    materialName,
                    Z,
                    A,
                    density,
    ...
    def _makeG4Element(self, rootElement):
        ...
        elemMat = _g4.ElementIsotopeMixture(
            elemName, elemFormula, elemNisotopes, registry=self._registry
        )
            ...
            if materialName in self.materialSubstitutions:
                ...
                try:
                    ...
                    print(
                        "Material '",
                        materialName,
                        "' is used in geometry but not defined in list of materials - using G4_Galactic",
                    )
            ...
            if thisDepth < maximumDepth:
                for iDaughter in range(0, volume.GetNdaughters(), 1):
                    ...
                    [
                        ...
                        matROOT,
                        traROOT,
                    ] = rootMatrix2pyg4ometry(matrix, self)
                    nodeRotPyG4 = _transformation.matrix2tbxyz(
                        _np.linalg.inv(_transformation.tbxyz2matrix(nodeRotPyG4))
                    )

                    daughterVolumePyG4 = self.recurseVolumeTree(
                        node.GetVolume(),
                        thisDepth + 1,
                        maximumDepth,
                        warnAboutBadShapes,
