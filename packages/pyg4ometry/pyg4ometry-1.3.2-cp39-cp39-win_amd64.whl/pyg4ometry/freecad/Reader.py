...
class Reader:
    ...
    def relabelModel(self):
        for obj in self.doc.Objects:
            ...
            obj.Label = obj.Label.replace("-", "_")
            obj.Label = obj.Label.replace(" ", "_")
            obj.Label = obj.Label.replace(".", "_dot_")
            obj.Label = obj.Label.replace(",", "_comma_")
            obj.Label = obj.Label.replace(":", "_colon_")
            obj.Label = obj.Label.replace("\\", "_fs_")
    ...
    def convertFlat(
        ...
        for obj in self.doc.Objects:
            if obj.TypeId == "Part::Feature":
                ...
                s = pyg4ometry.geant4.solid.TessellatedSolid(
                    ...
                    m,
                    self._registry,
                    pyg4ometry.geant4.solid.TessellatedSolid.MeshType.Freecad,
                )

                # logical
                l = pyg4ometry.geant4.LogicalVolume(
                    s, daughterMaterial, obj.Label + "_lv", registry=self._registry
                )
        ...
        bSolid = pyg4ometry.geant4.solid.Box(
            "worldSolid", tsize.x, tsize.y, tsize.z, registry=self._registry
        )
        bLogical = pyg4ometry.geant4.LogicalVolume(
        ...
        for i in range(len(logicals)):
            ...
            if storePartCentrePos:
                localPos = pyg4ometry.gdml.Defines.Position(
                    names[i] + "_centre",
                    str(localCentreX),
                    str(localCentreY),
                    str(localCentreZ),
            ...
            p = _g4.PhysicalVolume(
                pyg4ometry.gdml.Defines.Rotation(
                    names[i] + "_z1",
                    str(a1),
                    str(a2),
                    str(a3),
                ...
                pyg4ometry.gdml.Defines.Position(
                    names[i] + "_p2",
                    str(x),
                    str(y),
                    str(z),
    ...
    def recurseObjectTree(self, obj):
        ...
        if obj.TypeId == "App::Part":  # mapped to logical volume, group objects mapped to physical
            ...
            bSolid = pyg4ometry.geant4.solid.Box(
                obj.Label + "_solid", 100, 100, 100, registry=self._registry
            )
            bLogical = pyg4ometry.geant4.LogicalVolume(
                bSolid, "G4_Galactic", obj.Label + "_lv", registry=self._registry
            )
            ...
            for gobj in obj.Group:
                ...
                # logical volume
                p = pyg4ometry.geant4.PhysicalVolume(
                    pyg4ometry.gdml.Defines.Rotation(
                        "z1",
                        str(tba[0]),
                        str(tba[1]),
                    ...
                    pyg4ometry.gdml.Defines.Position(
                        "p2", str(x), str(y), str(z), self._registry, False
                    ),
                    lv,
                    gobj.Label + "_pv",
                    bLogical,
            ...
            # logical
            l = pyg4ometry.geant4.LogicalVolume(
                s, "G4_Galactic", obj.Label + "_lv", registry=self._registry
            )
...
def MeshAnalysis(m):
    ...
    return {
        "l01min": l01_array.min(),
        "l01max": l01_array.max(),
        "l02min": l02_array.min(),
        "l02max": l02_array.max(),
        "l12min": l12_array.min(),
        "l12max": l12_array.max(),
        "areamin": area_array.min(),
        "areamax": area_array.max(),
    }
