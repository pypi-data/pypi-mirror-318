                ...
                if len(self.logicalVolume.daughterVolumes) == 1:
                    if isinstance(self.logicalVolume.daughterVolumes[0], ReplicaVolume):
                        [
                            daughter_meshes,
                            daughter_transforms,
                        ] = self.logicalVolume.daughterVolumes[0].createReplicaMeshes()
                        for m, t in zip(daughter_meshes, daughter_transforms):
                            meshes.append(m)
                            transforms.append(
                                [rot, _np.array(trans) + _np.array(t[1])]
                            )  # TBC - t[0] ie daughter rotation is unused / not compounded
        else:
            # rotation type replica
            for v, i in zip(
                _np.arange(offset, offset + nreplicas * width, width),
                range(0, nreplicas, 1),
            ):
        ...
        if self.axis in [self.Axis.kXAxis, self.Axis.kYAxis, self.Axis.kZAxis]:
                ...
                if len(self.logicalVolume.daughterVolumes) == 1:
                    if isinstance(self.logicalVolume.daughterVolumes[0], ReplicaVolume):
                        (
                            daughterPVs,
                            daughterTransforms,
                        ) = self.logicalVolume.daughterVolumes[0].getPhysicalVolumes()
            ...
            for v, i in zip(
                ...
                solid = _solid.Tubs(
                    ...
                    self.logicalVolume.solid.nslice,
                    addRegistry=True,
                )  # we define a new unique solid, so we do need to store it
                anLV = LogicalVolume(
                    solid,
