...
class ComparisonResult:
    ...
    def print(
        self,
        testName=None,
        testResultsToPrint=[TestResult.Failed, TestResult.NotTested],
        allTests=False,
    ):
        """
...
def geometry(referenceLV, otherLV, tests=Tests(), includeAllTestResults=False):
    ...
    result = logicalVolumes(
        referenceLV, otherLV, tests, True, includeAllTestResults, testsAlreadyDone=[]
    )
    return result
...
def logicalVolumes(
    ...
    if tests.testDaughtersByName:
        ...
        for daugtherName in overlapSet:
            ...
            if (
                "daughter_test_" + i_rDaughter.name,
                "daughter_test_" + i_oDaughter.name,
            ) in testsAlreadyDone:
                continue  # already done this test

            result = _checkPVLikeDaughters(
                i_rDaughter,
                i_oDaughter,
                tests,
                rlv.name,
...
def physicalVolumes(
    ...
    if tests.copyNumber:
        result += _copyNumber(
            testName, rpv.copyNumber, opv.copyNumber, tests, includeAllTestResults
        )
    if rpv.logicalVolume.type == "logical":
        if (
            "lv_test_" + rpv.logicalVolume.name,
            "lv_test_" + opv.logicalVolume.name,
        ) not in testsAlreadyDone:
            result += logicalVolumes(
                rpv.logicalVolume,
                opv.logicalVolume,
        ...
        if (
            "av_test_" + rpv.logicalVolume.name,
            "av_test_" + opv.logicalVolume.name,
        ) not in testsAlreadyDone:
...
def _checkPVLikeDaughters(
    referencePVLikeObject,
    otherPVLikeObject,
    tests,
    parentName,
    testName,
...
def _testDaughterNameSets(
    referenceDaughterNameSet,
    otherDaughterNameSet,
    result,
    testName,
    includeAllTestResults,
):
...
def _elements(referenceElement, otherElement, tests, lvName="", includeAllTestResults=False):
    ...
    if tests.namesIgnorePointer:
        result += _namesIgnorePointer(
            "elementNames", re.name, oe.name, lvName, includeAllTestResults
        )
...
def solids(referenceSolid, otherSolid, tests, lvName="", includeAllTestResults=False):
    ...
    if tests.namesIgnorePointer:
        result += _namesIgnorePointer(
            "solidName", rso.name, oso.name, lvName, includeAllTestResults
        )
...
def _names(testName, str1, str2, parentName="", includeAllTestResults=False):
    ...
    if not nameTest:
        ...
        result[testName] = [
            TestResultNamed(
                ": ".join(list(filter(None, [parentName, str1]))),
                TestResult.Failed,
                details,
            )
        ]
...
def _namesIgnorePointer(testName, str1, str2, parentName="", includeAllTestResults=False):
    ...
    if not nameTest:
        ...
        result[testName] = [
            TestResultNamed(
                ": ".join(list(filter(None, [parentName, strippedStr1]))),
                TestResult.Failed,
                details,
            )
        ]
...
def _vector(vectortype, r1, r2, tests, parentName="", includeAllTestResults=False):
    ...
    tols = {
        "rotation": tests.toleranceRotationFraction,
        ...
        if vectortype == "rotation":
            ...
            if drc != 0 and drc2pi != 0:
                if (
                    ...
                    result[vectortype] += [
                        TestResultNamed(
                            ": ".join(list(filter(None, [parentName, r1.name]))),
                            TestResult.Failed,
                            details,
                        )
                    ]
