...
class ApiClient(base_api_client.ApiClient):
    ...
    async def get_all_ambient_measurements_for_event(self, account_id: str, event_id: str, options: ApiGetOptions) -> ApiGetResult[List[Mapping] | None]:
        ...
        if not response.ok:
            # return ApiGetResult[List[Mapping]](None, False, response.reason)
            return ApiGetResult(None, False, response.reason)
    ...
    async def get_event_car_data_for_event_car(self, account_id: str, event_id: str, car_id: str, options: ApiGetOptions) -> ApiGetResult[Mapping | None]:
        ...
        if not response.ok:
            # return ApiGetResult[Mapping](None, False, response.reason)
            return ApiGetResult(None, False, response.reason)
    ...
    async def add_column_item_display_cell_definition(self, account_id: str, column_item_display_line_definition_id: str, json: str, cell_type: str, create_model: CreateModel) -> ApiGetResult[Mapping | None]:
        response = await self.post_request(account_id, f"{self.api_endpoint}ColumnItemDisplayCellDefinitions?columnItemDisplayLineDefinitionId={column_item_display_line_definition_id}&json={json}&cellType={cell_type}", create_model)
