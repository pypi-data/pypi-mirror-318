...
# region Manual Endpoints
    ...
    async def get_all_accounts(self) -> ApiGetResult[List[Mapping[str, str]] | None]:
        ...
        if not response.ok:
            return ApiGetResult(None, False, response.reason)
...
# region Network Functions
    ...
    async def get_request(self, account_id: str | None, uri: str, body: IApiModel = None, timeout: float | tuple[float] | None = None) -> requests.Response:
        content = body.serialize() if body is not None else {}
        headers = await self.prepare_request(account_id)

        return requests.get(uri, json=content, headers=headers, timeout=timeout, verify=not self._is_dev) # verify=False is needed for requests to localhost

    async def post_request(self, account_id: str, uri: str, body: IApiModel = None) -> requests.Response:
        content = body.serialize() if body is not None else {}
        headers = await self.prepare_request(account_id)

        return requests.post(uri, json=content, headers=headers, verify=not self._is_dev)

    async def put_request(self, account_id: str, uri: str, body: IApiModel = None) -> requests.Response:
        content = body.serialize() if body is not None else {}
        ...
        return requests.put(uri, json=content, headers=headers, verify=not self._is_dev)

    async def delete_request(self, account_id: str, uri: str) -> requests.Response:
        headers = await self.prepare_request(account_id)

        return requests.delete(uri, headers=headers, verify=not self._is_dev)
    ...
    async def upload_file(self, uri: str, file, headers: Mapping[str, str]) -> bool:
        ...
        if isinstance(file, str):
            ...
            response = requests.put(uri, data=file, headers=headers, stream=True)
            return response.ok
    ...
    def build_url_parameters(self, options: ApiGetOptions | None, additional_parameters: Mapping[str, str | bool] = {}) -> str:
    ...
    def _send_debug_request(self, method: str, uri: str, json: Mapping, headers: Mapping) -> requests.Response:
