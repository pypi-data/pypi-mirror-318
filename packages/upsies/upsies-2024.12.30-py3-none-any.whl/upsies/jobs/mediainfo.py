...
class MediainfoJob(JobBase):
    ...
    This job adds the following signals to the :attr:`~.JobBase.signal` attribute:

        ``generating_report``
            Emitted mediainfo report generation has begun. Registered callbacks get the video file
            path ``mediainfo`` was called with.

        ``generated_report``
            Emitted mediainfo report generation has finished. Registered callbacks get the video
            file path ``mediainfo`` was called with and the report text.
    """
