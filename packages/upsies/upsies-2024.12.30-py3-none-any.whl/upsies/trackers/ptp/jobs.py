...
class PtpTrackerJobs(TrackerJobsBase):
    ...
    async def autodetect_subtitle_languages(self, job):
        # `TrackerJobsBase.subtitles` from our parent class provides all kinds of subtitles,
        # e.g. from .srt, .idx/.sub files and VIDEO_TS or BDMV directories.
        subtitle_languages = [
        ...
        for language in subtitle_languages:
            self.subtitle_languages_job.add_output(language)
    ...
    async def autodetect_trumpable(self, job):
        ...
        if (
                self.options.get('hardcoded_subtitles', None) in (True, False)
                # User specified "Hardcoded subtitles".
                and self.options['hardcoded_subtitles']
        ):
        ...
        if self.options.get('no_english_subtitles', None) in (True, False):
            ...
            if (
                    ...
                    and 'en' not in subtitle_languages
            ):
    ...
    def trumpable_no_english_subtitles_prompt_callback(self, answer):
        ...
        _text, reason = answer
        if reason:
            self.trumpable_job.add_output(reason)
    ...
    def screenshots_count(self):
        ...
        async def get_screenshots_count():
            await self.type_job.wait_started()
            await self.type_job.wait()
    ...
    def _post_data_common_subtitles(self):
        ...
        # Add "No subtitles" if there aren't any or if none of the existing subtitles are supported
        # in metadata.subtitles, i.e. we couldn't find any `codes`.
        if not codes:
