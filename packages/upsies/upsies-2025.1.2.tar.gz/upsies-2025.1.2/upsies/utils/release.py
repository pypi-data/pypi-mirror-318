...
class ReleaseName(collections.abc.Mapping):
    ...
    def english_title_before_original(self):
        ...
        """
        return getattr(self, '_english_title_before_original', True)
    ...
    async def fetch_info(self, *, webdb, webdb_id, callback=None):
        ...
        # There is a weird bug if we use asyncio.gather() to await these coroutines concurrently.
        # To reproduce the bug:
        #
        # 1. Comment out all jobs in UhdTrackerJobs.jobs_before_upload except for imdb_job and type_job.
        # 2. Run any normal submit command.
        # 3. When you select an IMDb ID, a single CPU core goes into heating mode. Looks like some kind of
        #    endless loop.
        #
        # - Using asyncio.TaskGroup instead of asyncio.gather() doesn't fix anything.
        # - All the coroutines passed to asyncio.gather() return properly, but asyncio.gather() does
        #   not return at all.
        # - Awaiting each coroutine consecutively seems to fix the issue.
        #
        # This was introduced in 7f8694a4c38771657f79adfd8707b32b3c9fdd73 when
        # TrackerJobsBase._propagate_webdb_info() stopped calling these methods if not
        # necessary. Note that _propagate_webdb_info() runs them consecutively.

        await self._update_attributes(webdb, webdb_id)
        await self._update_type(webdb, webdb_id)
        await self._update_country(webdb, webdb_id)
        await self._update_year_country_required(webdb, webdb_id)
    ...
    async def _update_type(self, webdb, webdb_id):
        # Use type from user-selected webdb ID if possible.
