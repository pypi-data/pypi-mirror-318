#!/bin/env python
"""
Create a production to remove replicas from a storage element
...
def main():
    ...
    from DIRAC.TransformationSystem.Utilities.CLIParameters import Params
    ...
    Script.registerSwitch("H","Help","Create one remove replicas transformation for each MetaValue given.\n\n More on this: Is running in dry-run mode, unless enabled with -x. MetaValue can be comma separated lists.\n \n Usage: ihepdirac_transformation_rmreplicas <MetaKey>=<MetaValue> <SE> [-G<Files>] [-N<ExtraName>] -x")
    ...
    from DIRAC.TransformationSystem.Utilities.OperationTransformation import createDataTransformation
