...
class Params:
    ...
    def __init__(self):
        ...
        self.transType = "Replication-JUNO"
    ...
    def registerSwitches(self, script):
        ...
        script.registerSwitch("T:", "TransType=", "Transmation Type to use for transformation", self.setTransType)
        script.registerSwitch("F:", "Flavour=", "Flavour to create: Replication or Moving", self.setTransFlavour)
    ...
    def checkRemoveFileSettings(self, script, checkArguments=True):
        """check if all required parameters are set, print error message and return S_ERROR if not
        
        :param script: The script object
        :type script: DIRAC.Core.Base.Script
        :param bool checkArguments: if false do not check for the correct number of arguments
        """
    
        if checkArguments:
            args = script.getPositionalArgs()
        
        # Check if args[0] contains the format 'meta_key=meta_value'
        if len(args) == 1:
            meta_arg = args[0]
            try:
                # Split on '=' to get the key and value
                key, value = meta_arg.split('=')
                self.setMetaKey(key)  # Set the key
                self.setMetaValues(value)  # Set the value
            except ValueError:
                self.errorMessages.append("ERROR: Expected 'meta_key=meta_value' format")
                return S_ERROR()
        ...
        # get default metadata key if not already set
