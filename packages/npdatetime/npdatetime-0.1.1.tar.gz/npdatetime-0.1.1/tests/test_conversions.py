...
from npdatetime.config import MINDATE, REFERENCE_DATE_AD
...
class TestConversion:
   ...
   def test_reference_dates(self):
      ...
      date_nepali = npdatetime.date.from_datetime_date(from_date=date_english)
   ...
   def test_random_conversions(self):
      for rd_maps in RANDOM_CONVERSION_MAPS:
         np_dt = npdatetime.date.from_datetime_date(datetime.date(**rd_maps['ad']))
         assert np_dt == npdatetime.date(**rd_maps['bs'])

         dt = npdatetime.date.to_datetime_date(npdatetime.date(**rd_maps['bs']))
