...
def mask_sensitive_info(log_message):
    sensitive_patterns = [
        ...
        r'\b\d{3}[-.\s]??\d{2}[-.\s]??\d{4}\b',  # Example pattern for SSN
        r'\b\d{4}[-.\s]??\d{4}[-.\s]??\d{4}[-.\s]??\d{4}\b',  # Credit card pattern
        r'\b\d{10}\b',  # Example pattern for phone number
        r'\b\d{6,10}\b',  # Example pattern for customer number
        r'\b[A-Z][a-z]*\b',  # Example pattern for names (first letter capitalized)
        r'\b[A-Z][a-z]*\b',  # Example pattern for surnames (first letter capitalized)
    ]
