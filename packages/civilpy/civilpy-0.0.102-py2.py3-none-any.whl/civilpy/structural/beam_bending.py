    ...
    * The default units package units for length, force and bending moment
    ...
    def rolling_support(self, x_coord: float):
        if self._x0 <= x_coord <= self._x1:
            ...
            raise ValueError(
                "The rolling support must be located within the beam span."
            )
    ...
    def add_loads(self, loads: list):
        ...
        for load in loads:
            ...
            if isinstance(load, supported_load_types):
                ...
                raise TypeError(
                    "The provided loads must be one of the supported types: {0}".format(
                        supported_load_types
                    )
                )
        self._update_loads()
    ...
    def get_reaction_forces(self):
        ...
        F_Rx = sum(
            integrate(load, (x, x0, x1)) for load in self._distributed_forces_x
        ) + sum(f.force for f in self._point_loads_x())
        F_Ry = sum(
            integrate(load, (x, x0, x1)) for load in self._distributed_forces_y
        ) + sum(f.force for f in self._point_loads_y())
        M_R = (
            sum(integrate(load * x, (x, x0, x1)) for load in self._distributed_forces_y)
            + sum(f.force * f.coord for f in self._point_loads_y())
            + sum(-1 * f.torque for f in self._point_torques())
        )
        A = np.array([[-1, 0, 0], [0, -1, -xA], [0, -1, -xB]]).T
    ...
    def _plot_analytical(
        ...
        if maxmin_hline:
            ...
            if abs(max(y_vec)) > tol:
                ...
                plt.annotate(
                    "${:0.1f}".format(y_vec[max_idx] * (1 - 2 * inverted))
                    .rstrip("0")
                    .rstrip(".")
                    + " $ {}".format(yunits),
            ...
            if abs(min(y_vec)) > tol:
                ...
                plt.annotate(
                    "${:0.1f}".format(y_vec[min_idx] * (1 - 2 * inverted))
                    .rstrip("0")
                    .rstrip(".")
                    + " $ {}".format(yunits),
    ...
    def _draw_beam_schematic(self, ax):
        ...
        pinned_support = Polygon(
            np.array(
                [
                    self.pinned_support + 0.01 * xspan * np.array((-1, -1, 0, 1, 1)),
                    beam_bottom + 0.05 * np.array((-1.5, -1, 0, -1, -1.5)) * yspan,
                ]
            ).T
        )
        rolling_support = [
            ...
            Polygon(
                np.array(
                    [
                        self.rolling_support + 0.01 * xspan * np.array((-1, -1, 1, 1)),
                        beam_bottom
                        + 0.05 * np.array((-1.5, -1.25, -1.25, -1.5)) * yspan,
                    ]
                ).T
            ),
        ]

        supports = PatchCollection(
            [pinned_support, *rolling_support], facecolor="black"
        )
        ax.add_collection(supports)

        # Draw arrows at point loads
        arrowprops = dict(
            arrowstyle="simple", color="darkgreen", shrinkA=0.1, mutation_scale=18
        )
    ...
    def _update_loads(self):
        ...
        self._distributed_forces_x = [
            self._create_distributed_force(f) for f in self._distributed_loads_x()
        ]
        self._distributed_forces_y = [
            self._create_distributed_force(f) for f in self._distributed_loads_y()
        ]
        ...
        self._normal_forces = [
            -1 * integrate(load, (x, x0, x)) for load in self._distributed_forces_x
        ]
        self._normal_forces.extend(
            -1 * self._effort_from_pointload(f) for f in self._point_loads_x()
        )
        self._normal_forces.append(
            -1 * self._effort_from_pointload(pinned_support_load_x)
        )

        self._shear_forces = [
            integrate(load, (x, x0, x)) for load in self._distributed_forces_y
        ]
        self._shear_forces.extend(
            self._effort_from_pointload(f) for f in self._point_loads_y()
        )
        ...
        self._bending_moments = [
            integrate(load, (x, x0, x)) for load in self._shear_forces
        ]
        self._bending_moments.extend(
            self._effort_from_pointload(f) for f in self._point_torques()
        )

    def _create_distributed_force(
        self, load: DistributedLoadH or DistributedLoadV, shift: bool = True
    ):
        """
