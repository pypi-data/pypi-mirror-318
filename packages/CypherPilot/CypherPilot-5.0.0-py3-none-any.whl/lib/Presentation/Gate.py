...
class Gate:
    ...
    def __init__(self,uri,user,password,name):
        ...
        :param name:(str) The name for authentication
        """
        self.service = Service(uri,user,password,name)
    ...
    def format_validation(self, input):
        ...
        if len(input) == 1:
            ...
            if len(input) > 8:
                self.logger.debug(f"unsupported format, the input must contain maximum 8 items")
                return False
            ...
            if len(input)==7:
                if not isinstance(input[6], int):
                    ...
                    return False
            if not isinstance(input[7], str):
                self.logger.debug(f"unsupported format, the seventh element in input list should be a string")
                return False
            self.logger.debug(f"input validation is completed successfully")
            return True
