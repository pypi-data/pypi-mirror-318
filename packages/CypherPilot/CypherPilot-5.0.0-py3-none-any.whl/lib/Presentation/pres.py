...
class Presentation:
    ...
    def __init__(self):
        self.gate = Gate("bolt://localhost:7687","neo4j","qwerty",name="bron")
    ...
    def Demo_pres(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                param = ["cve","cpe",["original_id"],["metadata_vendor","metadata_product","original_id"],False,"sample1",10000,"bron"]
                self.gate.query(param)
                #self.gate.direct_query(param)

            elif choice == "2":
                param = ["cwe",None,["original_id","_key"],[],False,"sample2",10000,"bron"]
                self.gate.query(param)
                #self.gate.path_query(param)
            elif choice == "3":
                #param= ["cwe","cpe",["original_id","metadata_applicable_platform"],["original_id","metadata_vendor"],True,100]
                param= ["technique","cve",["original_id","metadata_description"],["original_id","metadata_description"],True,"sample3",10000,"bron"]
