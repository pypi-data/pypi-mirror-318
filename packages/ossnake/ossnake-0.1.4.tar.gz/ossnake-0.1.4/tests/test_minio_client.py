...
class TestMinioClient(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        print(f"{Fore.CYAN}║                     MinIO Client Tests                      ║{Style.RESET_ALL}")
    ...
    def tearDown(self):
        ...
        try:
            logger.info(f"  {Fore.CYAN}- Deleting 'small_file.txt' from MinIO{Style.RESET_ALL}")
            self.client.delete_file('small_file.txt')
            logger.info(f"  {Fore.GREEN}- Deleted 'small_file.txt' from MinIO {Style.RESET_ALL}")
        except Exception as e:
            logger.info(f"  {Fore.RED}- Failed to delete 'small_file.txt' from MinIO: {e}{Style.RESET_ALL}")
        print(f"\n{Fore.CYAN}{'='*60}{Style.RESET_ALL}\n")
    ...
    def test_upload_download_file_small(self):
        ...
        try:
            ...
            logger.info(f"  {Fore.BLUE}2. Uploading file to MinIO: {local_file}{Style.RESET_ALL}")
            ...
            logger.info(f"  {Fore.BLUE}5. Downloading file from MinIO to: {download_file}{Style.RESET_ALL}")
            start_time = time.time()
    ...
    def test_upload_download_large_file(self):
        ...
        try:
            ...
            # Call the upload_file method
            start_time = time.time()
            logger.info(f"  {Fore.BLUE}2. Uploading large file to MinIO: {local_file}{Style.RESET_ALL}")
            ...
            logger.info(f"  {Fore.BLUE}5. Downloading large file from MinIO to: {download_file}{Style.RESET_ALL}")
            start_time = time.time()
    ...
    def test_upload_file_with_content_type(self):
        ...
        try:
            ...
            start_time = time.time()
            logger.info(f"  {Fore.BLUE}2. Uploading file with content type to MinIO: {local_file}{Style.RESET_ALL}")
    ...
    def test_invalid_credentials(self):
        ...
        try:
            ...
            invalid_client = MinioClient(invalid_config)
...
class CustomTestRunner(unittest.TextTestRunner):
    ...
    def run(self, test):
        ...
        # Print test list
        test_methods = [m for m in dir(TestMinioClient) if m.startswith('test_')]
        print(f"\n{Fore.YELLOW}Tests to be executed:{Style.RESET_ALL}")
        for i, method in enumerate(test_methods, 1):
            print(f"  {i}. {method}")
        print(f"\n{Fore.CYAN}{'='*60}{Style.RESET_ALL}\n")
...
def main():
    ...
    suite.addTest(unittest.makeSuite(TestMinioClient))
