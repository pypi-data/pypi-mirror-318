...
class TestAliyunLargeMultiparts(unittest.TestCase):
    """测试阿里云OSS大文件分片上传"""
    ...
    def _log_performance_stats(self, stats: dict):
        ...
        logger.info(f"  • Provider: {Fore.YELLOW}Aliyun OSS{Style.RESET_ALL}")
    ...
    def test_resume_upload(self):
        ...
        try:
            ...
            file_size = os.path.getsize(file_path)
            total_parts = (file_size + self.chunk_size - 1) // self.chunk_size
            half_parts = total_parts // 2
            
            logger.info(f"Uploading first {half_parts} parts...")
            with open(file_path, 'rb') as f:
                for part_number in range(1, half_parts + 1):
                    data = f.read(self.chunk_size)
                    etag = self.client.upload_part(upload, part_number, data)
                    upload.parts.append((part_number, etag))
                    
            # 4. 模拟中断
            logger.info("Simulating upload interruption...")
            time.sleep(2)
            
            # 5. 继续上传剩余部分
            logger.info("Resuming upload...")
            with open(file_path, 'rb') as f:
                f.seek(half_parts * self.chunk_size)
                for part_number in range(half_parts + 1, total_parts + 1):
                    data = f.read(self.chunk_size)
                    if not data:
                        break
                    etag = self.client.upload_part(upload, part_number, data)
            ...
            # 6. 完成上传
            url = self.client.complete_multipart_upload(upload)
            logger.info(f"Resume upload completed, URL: {url}")
...
class CustomTestRunner(unittest.TextTestRunner):
    ...
    def run(self, test):
        print(f"\n{Fore.CYAN}╔══════════════════════════════════════════════════════════════╗{Style.RESET_ALL}")
        print(f"{Fore.CYAN}║              Aliyun OSS Large File Upload Tests              ║{Style.RESET_ALL}")
        print(f"{Fore.CYAN}╚══════════════════════════════════════════════════════════════╝{Style.RESET_ALL}")
        
        test_methods = [m for m in dir(TestAliyunLargeMultiparts) if m.startswith('test_')]
...
def main():
    suite = unittest.TestLoader().loadTestsFromTestCase(TestAliyunLargeMultiparts)
