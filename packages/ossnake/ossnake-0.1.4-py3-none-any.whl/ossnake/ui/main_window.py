...
from ossnake.ui.components.bucket_list import BucketList
...
class MainWindow(tkdnd.Tk if DRAG_DROP_SUPPORTED else tk.Tk):
    def __init__(self):
        ...
        # 初始化重要的实例变量为 None
        self.bucket_list = None  # 添加这行
        self.object_list = None  # 添加这行
    ...
    def _load_proxy_settings(self):
        """加载并应用代理设置"""
        try:
            from ossnake.utils.settings_manager import SettingsManager  # 添加导入
    ...
    def create_main_frame(self):
        ...
        self.logger.info(f"Available clients for UI: {available_clients}")
        
        if available_clients:
            self.logger.info("Creating UI with available clients")
            # 创建顶部工具栏框架
            ...
            if available_clients:
                ...
                message = f"""
请在配置文件中添加OSS客户端信息：
配置文件路径：{self.config_manager.config_path}
...
"""
            ...
            self.logger.info("No available clients, showing config guide")
            # ... 显示配置指南 ...
    ...
    def on_source_change(self, event):
        ...
        # 先检查属性是否存在
        if selected and hasattr(self, 'bucket_list') and hasattr(self, 'object_list'):
            if self.bucket_list is not None and self.object_list is not None:
                try:
                    # 获取新的客户端
