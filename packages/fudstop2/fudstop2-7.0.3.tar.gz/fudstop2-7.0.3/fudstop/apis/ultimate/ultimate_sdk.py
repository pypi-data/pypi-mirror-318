...
class UltimateSDK:
    ...
    async def fetch_option_trades(
        ...
        # Convert nanosecond timestamps to UTC
        ...
        df.rename(
            columns={
                "participant_timestamp": "participant_datetime",
    ...
    async def fetch_ticker_news(
        ...
        session: aiohttp.ClientSession,
        ticker: str,
        published_utc_from: str = None,
        published_utc_to: str = None,
        order: str = "asc",
        sort: str = "published_utc",
        limit: int = 10,
    ) -> pd.DataFrame:
        """
        Fetch recent news articles for a single ticker from Polygon's
        /v2/reference/news endpoint, automatically handling pagination.
        ...
        # If published_utc_from/to are provided, use the standard Polygon filter notation
        ...
        while True:
            if next_url:
                # The next_url already contains its own query params
                url = next_url
                query_params = {}
            else:
                url = base_url
            ...
            # If Polygon's status is not OK, you might raise an error or log it
            if data.get("status") != "OK":
                print(
                    f"Warning: request returned status {data.get('status')} for ticker {ticker}"
                )

            results = data.get("results", [])
            if not results:
                # No articles found, or no more pages
                break
        ...
        # If no articles, return an empty DataFrame
        if not all_results:
            return pd.DataFrame()
        ...
        # Rename or remove columns as you see fit
        # For example, rename 'article_url' -> 'url', 'description' -> 'summary'
        df.rename(
            columns={
                "article_url": "url",
                "description": "summary",
        ...
        # ----------------------------------------------------------------
        # 1) Normalize 'publisher' dict into top-level columns
        # ----------------------------------------------------------------
        if "publisher" in df.columns:
            df["publisher_name"] = df["publisher"].apply(
                lambda pub: pub.get("name") if isinstance(pub, dict) else None
            )
            df["publisher_homepage_url"] = df["publisher"].apply(
                lambda pub: pub.get("homepage_url") if isinstance(pub, dict) else None
            )
            df["publisher_logo_url"] = df["publisher"].apply(
                lambda pub: pub.get("logo_url") if isinstance(pub, dict) else None
            )
            df["publisher_favicon_url"] = df["publisher"].apply(
                lambda pub: pub.get("favicon_url") if isinstance(pub, dict) else None
            )
            # You can drop the original nested column now if you like
            df.drop(columns=["publisher"], inplace=True)

        # ----------------------------------------------------------------
        # 2) Normalize 'insights' list of dicts:
        #    This example extracts just the first item from 'insights'.
        #    If you need to handle multiple items, consider .explode().
        # ----------------------------------------------------------------
        if "insights" in df.columns:
            df["insight_sentiment"] = df["insights"].apply(
                lambda arr: arr[0]["sentiment"] if arr and len(arr) > 0 else None
            )
            df["insight_sentiment_reasoning"] = df["insights"].apply(
                lambda arr: arr[0]["sentiment_reasoning"] if arr and len(arr) > 0 else None
            )
            df["insight_ticker"] = df["insights"].apply(
                lambda arr: arr[0]["ticker"] if arr and len(arr) > 0 else None
            )
            # Drop the original
            df.drop(columns=["insights"], inplace=True)

        # ----------------------------------------------------------------
        # 3) Join array fields like 'keywords' and 'tickers' into
        #    comma-separated strings
        # ----------------------------------------------------------------
        if "keywords" in df.columns:
            df["keywords"] = df["keywords"].apply(
                lambda arr: ", ".join(arr) if isinstance(arr, list) else arr
            )
        ...
        # Example: set the index to 'published_utc' for time-series style
        # df.set_index("published_utc", inplace=True, drop=True)
    ...
    async def fetch_ticker_news_for_symbols(
        ...
        """
        Fetch recent news articles concurrently for multiple tickers
        from Polygon's /v2/reference/news endpoint.
