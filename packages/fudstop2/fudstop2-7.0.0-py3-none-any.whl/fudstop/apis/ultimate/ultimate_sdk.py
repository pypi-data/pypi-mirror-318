...
class UltimateSDK:
    ...
    def check_macd_sentiment(self, hist: List[float]) -> str:
        ...
        'hist' should be in reverse-chronological order (index 0 = newest).
        """
        try:
    ...
    async def fetch_macd(self, session: aiohttp.ClientSession, ticker: str, timespan: str = 'day') -> pd.DataFrame:
        """
        Fetch MACD data for `ticker`, plus the underlying OHLC aggregates.
        Merge them into a single DataFrame with columns:
            [timestamp, open, high, low, close, volume, macd, signal, histogram, sentiment]
        ...
        params = {
            ...
            "limit": "100",
            # If you'd like to automatically get the 'underlying' link back,
            # you can add this param:
        ...
        try:
            ...
            # Sort by timestamp ascending for easier merging
            df_macd.sort_values("timestamp", inplace=True)
            df_macd.reset_index(drop=True, inplace=True)

            # 2) From the MACD response, get the "underlying" link and fetch OHLC data
            #    If the 'underlying' key does not exist, you can skip or
            #    build that URL yourself using the timespan, etc.
            underlying = results.get("underlying", {})
            underlying_url = underlying.get("url")

            # Some folks build the underlying URL themselves. For example:
            #   underlying_url = "https://api.polygon.io/v2/aggs/ticker/{}/range/1/{}/2003-01-01/2022-07-25".format(ticker, timespan)
            # Then append ?apiKey=...
            # For demonstration, we use the one provided by the response if present:
            if not underlying_url:
                # If no underlying URL found, skip this step
                # or build your own aggregator call. Example:
                # underlying_url = f"https://api.polygon.io/v2/aggs/ticker/{ticker}/range/1/{timespan}/2003-01-01/2025-01-01?apiKey={self.api_key}"
                return pd.DataFrame()

            # Append your API key to underlying URL
            if "?" in underlying_url:
                underlying_url += f"&apiKey={self.api_key}"
            else:
                underlying_url += f"?apiKey={self.api_key}"

            async with session.get(underlying_url) as resp2:
                resp2.raise_for_status()
                underlying_data = await resp2.json()

            # Underlying aggregates should be in "results"
            raw_agg = underlying_data.get("results", [])
            if not raw_agg:
                return pd.DataFrame()

            # Convert aggregator data into a DataFrame
            df_underlying = pd.DataFrame(raw_agg)
            # Typically aggregator data has fields:
            ...
            df_underlying.sort_values("timestamp", inplace=True)
            df_underlying.reset_index(drop=True, inplace=True)

            # 3) Merge underlying OHLC with MACD
            #    We'll do an outer join on 'timestamp'
            df_merged = pd.merge(
                df_underlying,
                df_macd[["timestamp", "macd", "signal", "histogram"]],
                on="timestamp",
                how="outer"
            )

            # The Polygon MACD data is in descending order by default (if order=desc),
            # but we sorted ascending. Make sure we know which are the *latest* histogram
            # values so we can compute sentiment properly. We'll do that now:

            # Get histogram in *reverse-chronological* order for the last 3
            hist_desc = df_macd.sort_values("timestamp", ascending=False)["histogram"].dropna().values
            sentiment = self.check_macd_sentiment(hist_desc.tolist())

            # Put that sentiment in a column for all rows (or just for the last row, your choice).
            df_merged["sentiment"] = sentiment
    ...
    async def get_candle_data(self, ticker, interval):
        """
        INTERVALS:
        ...
        candle_data = await ta.get_candle_data(ticker, interval=interval, headers=self.wb_headers, count='250')
        candle_data['Open'], candle_data['Close'], candle_data['High'], candle_data['Low'], candle_data['Volume'], candle_data['Vwap'] = candle_data['Open'].astype(float), candle_data['High'].astype(float), candle_data['Low'].astype(float), candle_data['Close'].astype(float), candle_data['Volume'].astype(float), candle_data['Vwap'].astype(float)
