...
class GSheetHelper:
    # Constructor
    def __init__(self, sheet_id, scope="team-offprice-market-analytics-scope", key="bq_credential"):
        """
        ...
        credentials = dbutils.secrets.get(scope=scope, key=key)
    ...
    # Write
    def write(self, worksheet_name, val_or_df, cell, log=True):
        """
        ...
        if isinstance(val_or_df, pd.DataFrame):
            df = val_or_df
            self.__write_dataframe__(worksheet_name, df, cell)
        else:
            val = val_or_df
...
# gsheet = GSheetHelper('1XtGO4Wk0FLHxPLk6lYpF8IstO1ZpIHazi4mm39CK0jo')
# df = gsheet.read('# Logins', 'D1')
# print(df)
# gsheet.write('# Logins', 'Hello', 'F1')
# gsheet.write('# Logins', 'Hello', 'G1')
# gsheet.write('# Logins', pd.DataFrame([1, 2]), 'H1')
# df = gsheet.read('# Logins', 'F1:H2')
# print(df)
# print(df.shape)
# gsheet.delete('# Logins', 'F1:G1')