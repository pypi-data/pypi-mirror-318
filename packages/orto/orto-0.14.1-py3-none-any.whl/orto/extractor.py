...
class GMatrixExtractor(extto.BetweenExtractor):
    '''
    Extracts ELECTRONIC G-MATRIX block
    '''

    # Regex Start Pattern
    START_PATTERN = rb'(?<=ELECTRONIC G-MATRIX\s[\S\s]{20}\s)' # noqa
    ...
    @classmethod
    def extract(cls, file_name: str | pathlib.Path) -> list[dict[str, NDArray]]:
        '''
        Convenience method which instantiates class, extracts blocks, and
        returns processed datasets
...
class GMatrixEffectiveExtractor(GMatrixExtractor):
    '''
    Extracts ELECTRONIC G-MATRIX FROM EFFECTIVE HAMILTONIAN block
    '''
    # Regex Start Pattern
...
class GMatrixLExtractor(GMatrixExtractor):
    '''
    Extracts ELECTRONIC G-MATRIX: L contribution block
    '''
    # Regex Start Pattern
...
class GMatrixSExtractor(GMatrixExtractor):
    '''
    Extracts ELECTRONIC G-MATRIX: S contribution block
    '''
    # Regex Start Pattern
