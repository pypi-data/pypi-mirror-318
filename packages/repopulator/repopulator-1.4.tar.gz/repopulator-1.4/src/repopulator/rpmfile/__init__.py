'''Fork of srossross/rpmfile with egregious bugs fixed'''
...
class RPMInfo(object):
    ...
    def __init__(self, name: str, file_start: int, file_size: int, initial_offset: int, isdir: bool):
        self.name = name
    ...
    @classmethod
    def _read(cls, magic: bytes, fileobj):
        if magic == b"070701":
    ...
    @classmethod
    def _read_new(cls, fileobj: BinaryIO, magic=None):
...
class RPMFile(object):
    ...
    def __init__(self, name: Optional[int | str | bytes | PathLike[str] | PathLike[bytes]]=None, mode="rb", fileobj: Optional[BinaryIO]=None):
        if mode != "rb":
            raise NotImplementedError("currently the only supported mode is 'rb'")
        if fileobj is not None:
            self._fileobj:BinaryIO = fileobj
        elif name is not None:
            self._fileobj:BinaryIO = io.open(name, mode)
        else:
            raise ValueError('either name or fileobj argument must be provided')
...
def open(name:Optional[int | str | bytes | PathLike[str] | PathLike[bytes]]=None, mode="rb", fileobj:Optional[BinaryIO]=None):
    """
