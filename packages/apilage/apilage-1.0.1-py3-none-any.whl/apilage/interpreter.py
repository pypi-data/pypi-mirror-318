...
def execute(tokens):
    ...
    def eval_expression(expression):
        ...
        elif '+' in expression:  # Handle concatenation
            parts = expression.split('+')
            return ''.join(eval_expression(part.strip()) for part in parts)
    ...
    while i < len(tokens):
        ...
        if token_type == 'LET':
            ...
            try:
                to_print = eval_expression(expression)
                print(to_print)
            except Exception as e:
                raise SyntaxError(f"Error evaluating expression: {expression}. {e}")
            i += 2
...
def main():
    if len(sys.argv) < 2:
        print("Usage: apilage <filename.apila>")
        sys.exit(1)

    filename = sys.argv[1]
    
    # Check if the file ends with .apila
    if not filename.endswith('.apila'):
        print("Error: File must have .apila extension")
        sys.exit(1)
