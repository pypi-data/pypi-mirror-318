...
from gitlab.exceptions import GitlabDeleteError, GitlabGetError, GitlabHttpError, GitlabListError
...
class GitLabFeature:
    ...
    def group_export_limitations(self, criteria: str) -> Dict[str, List[str]]:
    ...
    def project_export_limitations(self, criteria: str) -> Dict[str, List[str]]:
        ...
        try:
            if project.triggers.list(get_all=False):
                result['Pipeline triggers'] = [
                    trigger.description for trigger in project.triggers.list(get_all=True)
                ]
        except GitlabListError:
            pass
        ...
        try:
            if self.__gitlab.http_get(f'/projects/{project.id}/hooks', get_all=True):
                result['Webhooks'] = [
                    f'{hook["name"] if hook["name"] else hook["url"]}' # type: ignore[index]
                    for hook in self.__gitlab.http_get(f'/projects/{project.id}/hooks',
                                                       get_all=True)
                ]
    ...
    def project_get_branch_default(self, criteria: str) -> str:
    ...
    def project_set_branch_default(self, criteria: str, default_branch: str) -> None:
    ...
    # Subgroup create
    def subgroup_create(self, parent: str, path: str, name: str) -> None:
