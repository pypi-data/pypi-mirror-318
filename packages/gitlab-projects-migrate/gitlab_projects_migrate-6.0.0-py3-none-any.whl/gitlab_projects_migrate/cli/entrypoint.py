...
class Entrypoint:
    ...
    def cli(
        ...
        # Handle group recursively
            ...
            if output_gitlab and not options.exclude_subgroups or not output_exists:
    ...
    def group(
        ...
        if export_limitations:
            ...
            for limitation, items in export_limitations.items():
                print(f'{Colors.BOLD}     - Limited feature export: '
                      f'{Colors.RED}{limitation}'
                      + (f'{Colors.CYAN} ({", ".join(items)})' if items else '') + \
                      f'{Colors.RESET}')
            if not Entrypoint.confirm(
                    '',
                    input_group.full_path,
    ...
    def project(
        ...
        if branch_default:
            output_gitlab.project_set_branch_default(
                output_project.path_with_namespace,
                branch_default,
            )
            print(f'{Colors.BOLD}     - Set default branch: '
                  f'{Colors.CYAN}{branch_default}'
                  f'{Colors.RESET}')
            Platform.flush()
    ...
    def subgroup(
        ...
        if migration:
            ...
            if output_gitlab and input_subpath in [
                    Namespaces.subpath(
            ...
            if output_gitlab or options.archive_exports:
                ...
                with NamedTemporaryFile(
                    ...
                    if output_gitlab:
                        print(f'{Colors.BOLD}   - Importing to: '
                              f'{Colors.GREEN}'
                              f'{output_group.full_path}{output_namespace}/{output_path}'
                              f'{Colors.RESET}')
                        Platform.flush()
                        output_gitlab.group_import(
                            file_export.name,
