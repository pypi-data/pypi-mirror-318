...
def _template_data_generator(sub: str, action: Mapping[str, Any], desktop_object: DesktopObject,
                             attr_perms: Mapping[str, Sequence[Permission] | None], len_fixtures=1):
...
def _set_collection_template(action: Mapping[str, Any], collection_doc: Mapping[str, Any],
    ...
    if 'cj-template' in targets:
        template = {'data': [d for d in _template_data_generator(sub, action, desktop_object, attr_perms, len_desktop_objects)],
...
async def _expected_one_form(
    ...
    template_data = [d for d in _template_data_generator(context.sub, action, obj, attr_perms)]
