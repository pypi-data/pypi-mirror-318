...
class CJ(Representor):
    ...
    async def formats(self, request: Request,
        ...
        def cj_generator() -> Generator:
            for w in (wstl_obj if not isinstance(wstl_obj, Mapping) else [wstl_obj]):
                yield self.__format(request, w, link_callback, include_data=include_data)
    ...
    @staticmethod
    def __format(request: Request, wstl_obj: Mapping[str, Any],
...
def _get_items(request: Request, wstl_obj: dict[str, Any], tvars: uritemplate.variable.VariableValueDict,
               link_callback: Callable[[int, Link], None] | None = None, include_data = True) -> Generator[dict[str, Any], None, None]:
...
def _get_template(sub: str, wstl_actions: list[Mapping[str, Any]], tvars: uritemplate.variable.VariableValueDict,
    ...
    if attribute_permissions and len(data or []) == 1:
        ...
        empty_obj = desktop_object_type_for_name(data[0]['type'])()
        empty_obj.owner = sub
        attribute_permissions_ = {attr: [perm.name for perm in empty_obj.custom_attribute_permissions(attr, sub) or [Permission.VIEWER, Permission.EDITOR]] for attr in empty_obj.get_attributes()}
    else:
