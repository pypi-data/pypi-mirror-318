...
def parse_jsonpath(manifest, extend=None):
    ...
    def tokenize(tList):
        ...
        try :
            ...
            return([False, f"Error field in xls {tList[i]}"])
                ...
                if len(strValues) > 2:
                    if '==' in strValues[1]:
                        ...
                        if 'Texas' in TexasValueList[0]:
                            TexasValue = ' '.join(TexasValueList[:-1]) + "(" + TexasValueList[-1] + ")"
    ...
    def jsonpath_to_dict(manifestItem):
        ...
        if not jsonPath.startswith("$."):
            return({'error':f"Given JSON_PATH : {jsonPath}  is not starting with $.xxx"})
        ...
        try:
            ...
            return({"error": f"Invalid JSONPATH: {jsonPath}"})
