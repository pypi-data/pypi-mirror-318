...
class AudioInference(FireworksClient):
    ...
    async def transcribe_stream_async(
        ...
        Union[TranscriptionResponse, TranscriptionVerboseResponse, str], None
    ]:
        """
        Transcribe a real time audio into text using ASR (audio speech recognition) via a WebSocket connection.
        See the OpenAPI spec (https://docs.fireworks.ai/api-reference/audio-transcriptions)
        for the most up-to-date description of the supported parameters

        Transcribe audio in real-time using a WebSocket connection.

        Parameters:
        - audio_generator (AsyncGenerator[bytes, None]): An async generator that yields audio data chunks.
        - model (str, optional): The ASR model name to call. Defaults to `self.model`.
        - vad_model (str, optional): The VAD model name to call. Defaults to `self.vad_model`.
        - alignment_model (str, optional): The alignment model name to call. Defaults to `self.alignment_model`.
        - prompt (str, optional): The input prompt with which to prime transcription.
        - response_format (str): The format in which to return the response. Can be one of "json", "text", "srt", "verbose_json", or "vtt". If not present, defaults to "json".
        - temperature (float): Sampling temperature to use when decoding text tokens during transcription. Defaults to 0.0.
        - preprocessing (str, optional): The preprocessing to apply.
        - language (str, optional): The target languages for transcription. The set of supported target languages can be found at https://tinyurl.com/bdz3y63b.
        - timestamp_granularities (List[str], optional): The timestamp granularities to populate for this transcription. `response_format` must be set "verbose_json" to use timestamp granularities. Either or both of these options are supported: "word", or "segment". If not present, defaults to "segment".

        Yields:
        - TranscriptionVerboseResponse: An object with transcription.

        Raises:
        - RuntimeError: If there is an error in the audio transcription process.
        """
        params = TranscriptionWebSocketRequest(
            model=model or self.model,
            vad_model=vad_model or self.vad_model,
            alignment_model=alignment_model or self.alignment_model,
            prompt=prompt,
        ...
        async with httpx.AsyncClient(
            ...
            **self.client_kwargs,
            ...
            ws_url = f"ws{self.base_url[4:]}/v1/audio/transcriptions/ws"
            async with aconnect_ws(ws_url, client) as ws:
                print(f"Connected to {ws_url}")
                await ws.send_json(params.model_dump())
                print(f"Sent params to {ws_url}")
                ...
                async for audio_chunk in audio_generator:
                    ...
                    try:
                        ...
                        if "error" in as_json:
                            self._raise_for(as_json["status"], lambda: as_json["error"])
                    except json.JSONDecodeError:
                        pass

                    if response_format in [None, "json"]:
                        yield TranscriptionResponse(**json.loads(response))
                    elif response_format == "verbose_json":
                        yield TranscriptionVerboseResponse(**json.loads(response))
                    else:
                        yield response
