...
class BotPaymentsDisabled(BadRequest):
    """This method can only be run by a bot. Please enable bot payments in botfather before calling this method."""
...
class ButtonDataInvalid(BadRequest):
    """The button callback data of one or more of the buttons you provided is invalid"""
...
class StarrefPermilleTooLow(BadRequest):
    """The ``commission_permille`` parameter must be between [`starref_min_commission_permille*` client configuration parameter](https://core.telegram.org/api/config) and [`starref_max_commission_permille*` client configuration parameter](https://core.telegram.org/api/config)"""
