...
from eoq3.value import BOL, U32, I64, STR, NON, LST
from eoq3.command import Cmp, Get, Crt, Scc, EVT_TYPES
from eoq3.query import Qry, His, Cls, Pth
from eoq3.concepts import CONCEPTS, MXMDB, MXELEMENT, M2MODEL, M2PACKAGE, M1OBJECT
...
class ActionManager:
    ...
    def __init__(self, config:Config=EOQ_DEFAULT_CONFIG):
        ...
        self.domain = None
        self.sessionId = None
        self.actionsM2Model = None
        self.actionsM1Model = None
        self.actionsContainer = None
        self.serializer = TextSerializer()
            
    def Connect(self, domain:Domain, sessionId:str):
        self.domain = domain
        ...
        #.Upd(His(STR('job')),LST([STR('id'),STR('action'),STR('outputEvtKey'),STR('inputEvtKey'),STR('status'),STR('controlFlag')]),LST([His(STR('jobid')),His(STR('action')),His(STR('outputEvtKey')),His(STR('inputEvtKey')),STR('READY'),STR('RUN')]),I64(0),mute=True)\
    ...
    def Close(self):
        pass #keep the actions model in the MDB
