...
from eoq3.value import I32, F64, DAT, STR, LST, EncVal
from eoq3.query import Obj, Qry, Cls, Pth, His, Slf, IDX_MODES
...
def JobWorkerProcess(actiondir:str, jobObjStr:str, actionname:str, arguments:list, sessionId:str, outputEvtKey:str, inputEvtKey:str, cmdTxQueue:Queue, cmdRxQueue:Queue, processLessMode:bool):
    #restore job
    ...
    #connect to domain
    domain = MultiprocessingQueueDomainClient(cmdTxQueue, cmdRxQueue, name="MultiprocessingQueueDomainClient(job: %s, action:%s)"%(jobObj,actionname))
    stdoutRedirector = SyncWriteRedirector(sys.stdout, domain, sessionId, outputEvtKey, 0.0, processLessMode)
    ...
    if(actionFunction):
        ...
        try:
            ...
            if( (isinstance(value,list) and not str == type(value)) or\
...
class ActionHandler():
    ...
    def __init__(self, basedir:str='actions', handlername:str='eoq3actionhandlerpy', config:Config=EOQ_DEFAULT_CONFIG):
        ...
        self.name = handlername #the name that idendifies the actions belonging to this handler
        self.config = config
        # initialize internal vars
        self.logger = GetLoggerInstance(config)
        self.domain = None
        self.sessionId = None
        self.actionsM1Model = None
        self.actionsContainer:Obj = None #the model element containing the actions
        self.actionHandler:Obj = None #the model element for this action handler
        self._actionRegistry = {} #name -> (action,handlerfunction)
        self.myJobs = {}
        self.evtQueue = deque()
    ...
    def Connect(self, domain:Domain, sessionId:str=None,)->None:
        self.domain = domain 
        self.sessionId = sessionId #the default session id the handler uses
    ...
    def __ConnectToDomain(self):
        #check if the the actions meta-model and container exists
        try:
    ...
    def __OnDomainEvent(self, evts:Evt, context:str, source:Observable):
    ...
    def __HandleNewJob(self,jobObj:Obj)->None:
        ...
        if(actionObj in self._actionRegistry and 'READY' == status and 'RUN' == controlFlag):
            action = self._actionRegistry[actionObj]
    ...
    def __AbortJob(self,job:Job)->None:
        ...
        if(job.HasStarted()):
            if(self.config.processLessMode):
                pass #processes cannot be killed. Can not do anything here.
            else:
                #kill the process
    ...
    def __ScanForActions(self)->list:
        actions = []
    ...
    def __UpdateActionRegister(self,actions):
        ...
        for i in range(len(actions)):
            self._actionRegistry[res[i]] = actions[i]
    ...
    def __JobMonitorThread(self, job:Job):
        ...
        if(self.config.processLessMode): #in debug mode no process is created, but the server executed in the same Process
            proc = Thread(name="JobWorker (Thread)", target=JobWorkerProcess, args=(self._basedir, jobObjStr,\
                                                       job.action.name, job.arguments,\
                                                       self.sessionId, job.outputEvtKey, job.inputEvtKey,\
                                                       cmdRxHostQueue, cmdTxHostQueue, True, )) 
        else:
            proc = Process(name="JobWorker (Process)", target=JobWorkerProcess, args=(self._basedir, jobObjStr,\
                                                       job.action.name, job.arguments,\
                                                       self.sessionId, job.outputEvtKey, job.inputEvtKey,\
                                                       cmdRxHostQueue, cmdTxHostQueue, False, )) 
        sTime = timer()
