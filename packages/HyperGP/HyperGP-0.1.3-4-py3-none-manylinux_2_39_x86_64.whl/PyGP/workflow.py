...
from PyGP.src import device as set_device, query_device
import multiprocessing
import copy
from PyGP.src import check_gpu
...
class GpOptimizer(BaseStruct, __Mods):
    ...
    def status_init(self, **kwargs):
        ...
        Examples:
            ...
            >>> pop_size = 1000
            >>> pset = PyGP.PrimitiveSet(input_arity=1,  primitive_set=[('add', PyGP.tensor.add, 2),('sub', PyGP.tensor.sub, 2)])
            >>> pop = PyGP.Population(parallel=False)
    ...
    def iter_component(self, *args):
        ...
        Args:
            args: The ``PyGP.states.ParaStates`` module is needed to register each component.
        
        Examples:
            >>> optimizer.iter_component(
            >>>     ParaStates(func=PyGP.RandTrCrv(), source=["p_list", "p_list"], to=["p_list", "p_list"],
            ...            mask=set_prmask(pop_size)),
            >>>     ParaStates(func=shuffle, source=["p_list"], to=["p_list"],
            ...             mask=[1]),
            >>>     ParaStates(func=PyGP.RandTrMut(), source=["p_list", ProgBuildStates(pset=pset, depth_rg=[2, 3], len_limit=100), True], to=["p_list"],
            ...             mask=[set_armask(pop_size), 1, 1]),
            >>>     ParaStates(func=PyGP.executor, source=["p_list", "input", "pset"], to=["output", None],
            ...             mask=[1, 1, 1]),
            >>>     ParaStates(func=PyGP.evaluation, source=["output", "output_tensor"], to=["fit_list"],
            ...             mask=[1, 1])
            )
        """
