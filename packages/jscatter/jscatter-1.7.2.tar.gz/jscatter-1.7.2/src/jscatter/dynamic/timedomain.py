...
def simpleDiffusion(t, gamma=None, q=None, D=None, w=0, beta=1, type='lognorm'):
    r"""
    ...
    w : float
        Relative standard deviation of the diffusion coefficient distribution.
        In absolute units :math:`\sigma=w*q^2*D`.
        For a typical DLS experiment `w≈0.25` just from instrument noise (e.g. noise ≈ 1e-3 Zetasizer Nano, Malvern)
    ...
    if w == 0:
        ...
        if type[0] == 't':
            a = (0 - gamma) / (w * gamma)
            b = np.inf
            result = formel.pDA(_diff, w * gamma, parname='gamma', type='truncnorm', gamma=gamma, t=t, nGauss=30, a=a, b=b)
        else:
            result = formel.pDA(_diff, w*gamma, parname='gamma', type='lognorm', gamma=gamma, t=t, nGauss=30)
...
def doubleDiffusion(t, q=None, gamma1=None, D1=None, w1=0,
                               gamma2=None, D2=None, w2=0, frac=0.5, beta=1, type='lognorm'):
    r"""
    ...
    """
    result1 = simpleDiffusion(t, gamma=gamma1, q=q, D=D1, w=w1, beta=1, type=type)
    result2 = simpleDiffusion(t, gamma=gamma2, q=q, D=D2, w=w2, beta=1, type=type)
