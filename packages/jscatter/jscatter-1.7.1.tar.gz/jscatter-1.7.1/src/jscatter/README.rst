...
**But using Windows nativly or with WSL ?**
Some parts (Fortran compiled things in *bio*) are currently not working on Windows.
But e.g. cloudscattering does for a comparison.
...
use 'spawn' method and Linux can use 'fork'. Fork is much faster.
...
My advice, if methods from cloudscattering (some formfactors, lattice sf and bio) are used Install Jscatter via WSL.
Benefits from 'fork' and Fortran.
