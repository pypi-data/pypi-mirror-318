...
class CarunaPlusPrices:
    def __init__(self):
        self.timestamp = datetime.now()


class CarunaPlusMarketPrices(CarunaPlusPrices):
...
class CarunaPlusExchangePrices(CarunaPlusPrices):
    def __init__(self):
        super().__init__()
...
class CarunaPlusPriceClient:
    ...
    _carunaplus_market_price_prices: CarunaPlusMarketPrices = None
    _carunaplus_exchange_prices: CarunaPlusExchangePrices = None
        

    def _are_market_price_prices_valid(self):
        return self._is_carunaplus_prices_valid(self._carunaplus_market_price_prices)


    def _are_exchange_prices_valid(self):
        return self._is_carunaplus_prices_valid(self._carunaplus_exchange_prices)

    
    def _is_carunaplus_prices_valid(self, carunaplus_prices: CarunaPlusPrices):
        """If the latest price scrape has happened within the last hour, then use cache"""

        now = datetime.now()
        if carunaplus_prices is None:
            return False
        were_market_prices_scraped_within_hour = now-timedelta(hours=1) <= carunaplus_prices.timestamp <= now
    ...
    def get_exchange_prices(self) -> CarunaPlusExchangePrices:
        """Get the margin price for the Exchange Electricity contract"""
        if self._are_exchange_prices_valid():
            return self._carunaplus_exchange_prices

        self._carunaplus_exchange_prices = CarunaPlusExchangePrices()
        return self._carunaplus_exchange_prices