...
def _get_fully_qualified_schema(m: dict):
    """Returns expected schema string given dbt model response"""

    return m["database"] + "." + m["schema"]


def _fetch_model_info_from_dbt_cloud_run_id(dbt_cloud_run_id: str) -> Tuple[Path, dict]:
    ...
    model_schema_info = {
        m["uniqueId"]: _get_fully_qualified_schema(m) for m in models_in_run
    }
    ...
    prod_model_schema_info = {
        m["uniqueId"]: _get_fully_qualified_schema(m) for m in prod_model_locations
    }
...
def handle_dbt_args(
    ...
    if dbt_cloud_run_id:
        ...
        return _fetch_model_info_from_dbt_cloud_run_id(dbt_cloud_run_id)
