...
def track_interaction(widget_name, event_type):
    print(f"Interaction with {widget_name}: {event_type}")
...
def set_window_title(_root, _title):
    _root.title(_title)
    track_interaction("window", "title set")

def set_window_icon(_root, _icon_path):
    _root.iconbitmap(_icon_path)
    track_interaction("window", "icon set")

def minimize_window(_root):
    _root.iconify()
    track_interaction("window", "minimized")

def maximize_window(_root):
    _root.state('zoomed')
    track_interaction("window", "maximized")

def destroy_window(_root):
    _root.destroy()
    track_interaction("window", "destroyed")

def center_window(_root, width=300, height=200):
    _root.geometry(f"{width}x{height}+{(_root.winfo_screenwidth()//2)-(width//2)}+{(_root.winfo_screenheight()//2)-(height//2)}")
    track_interaction("window", "centered")

def set_window_bg_color(_root, color):
    _root.configure(bg=color)
    track_interaction("window", f"background color set to {color}")

def set_window_always_on_top(_root):
    _root.attributes("-topmost", True)
    track_interaction("window", "always on top set")

def remove_window_always_on_top(_root):
    _root.attributes("-topmost", False)
    track_interaction("window", "always on top removed")

def set_window_opacity(_root, opacity):
    _root.attributes("-alpha", opacity)
    track_interaction("window", f"opacity set to {opacity}")

def hide_window(_root):
    _root.withdraw()
    track_interaction("window", "hidden")

def show_window(_root):
    _root.deiconify()
    track_interaction("window", "shown")

def set_window_fixed_size(_root):
    _root.resizable(False, False)
    track_interaction("window", "fixed size set")

def enable_window_resizing(_root):
    _root.resizable(True, True)
    track_interaction("window", "resizing enabled")

def set_window_bg_image(_root, image_path):
    img = tk.PhotoImage(file=image_path)
    label = tk.Label(_root, image=img)
    label.place(relwidth=1, relheight=1)
    label.image = img  # Store the reference to prevent garbage collection
    track_interaction("window", f"background image set from {image_path}")

def change_window_icon(_root, icon_path):
    _root.iconbitmap(icon_path)
    track_interaction("window", f"icon changed to {icon_path}")

def create_label(_root, _text):
    label = tk.Label(_root, text=_text)
    label.pack()
    label.bind("<Button-1>", lambda event: track_interaction("label", "clicked"))
    track_interaction("label", "created")
    return label

def create_button(_root, _text, _command):
    button = tk.Button(_root, text=_text, command=lambda: [track_interaction("button", "clicked"), _command()])
    button.pack()
    track_interaction("button", "created")
    return button

def create_entry(_root):
    entry = tk.Entry(_root)
    entry.pack()
    entry.bind("<FocusIn>", lambda event: track_interaction("entry", "focused"))
    entry.bind("<FocusOut>", lambda event: track_interaction("entry", "unfocused"))
    track_interaction("entry", "created")
    return entry

def create_text_widget(_root, _width=30, _height=10):
    text_widget = tk.Text(_root, width=_width, height=_height)
    text_widget.pack()
    text_widget.bind("<KeyRelease>", lambda event: track_interaction("text widget", f"key released: {event.keysym}"))
    track_interaction("text widget", "created")
    return text_widget

def create_checkbox(_root, _text, _command):
    checkbox = tk.Checkbutton(_root, text=_text, command=lambda: [track_interaction("checkbox", "clicked"), _command()])
    checkbox.pack()
    track_interaction("checkbox", "created")
    return checkbox

def create_radio_buttons(_root, _options, _command):
    var = tk.StringVar()
    for option in _options:
        radio_button = tk.Radiobutton(_root, text=option, variable=var, value=option, command=lambda: [track_interaction("radio button", "selected"), _command()])
        radio_button.pack()
    track_interaction("radio buttons", "created")
    return var

def create_dropdown(_root, _options, _command):
    var = tk.StringVar()
    dropdown = tk.OptionMenu(_root, var, * _options, command=lambda _: [track_interaction("dropdown", "selected"), _command()])
    dropdown.pack()
    track_interaction("dropdown", "created")
    return var
...
def create_listbox(_root, _items, _command):
    ...
    listbox.pack()
    listbox.bind("<ButtonRelease-1>", lambda event: track_interaction("listbox", "item selected"))
    track_interaction("listbox", "created")
    return listbox

def create_canvas(_root, _width=400, _height=300):
    canvas = tk.Canvas(_root, width=_width, height=_height)
    canvas.pack()
    track_interaction("canvas", "created")
    return canvas

def create_progress_bar(_root):
    progress_bar = tk.Progressbar(_root, length=200, mode='indeterminate')
    progress_bar.pack()
    track_interaction("progress bar", "created")
    return progress_bar

def create_scrollbar(_root, _widget):
    scrollbar = tk.Scrollbar(_root, orient=tk.VERTICAL, command=_widget.yview)
    _widget.config(yscrollcommand=scrollbar.set)
    scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
    track_interaction("scrollbar", "created")
    return scrollbar

def create_frame(_root):
    frame = tk.Frame(_root)
    frame.pack()
    track_interaction("frame", "created")
    return frame

def create_menu_bar(_root):
    menu_bar = tk.Menu(_root)
    _root.config(menu=menu_bar)
    track_interaction("menu bar", "created")
    return menu_bar
