...
class ProteinStructure:
    ...
    def length(self):
        ...
        :return: int: 原子数量
        """
        return min(len(self.atom_control), len(self.atom_treatment))
    ...
    def calculate_rmsd(self):
        ...
        if len(self.atom_control) != len(self.atom_treatment):
            print("Warning: Atom counts are not equal. Ensure lengths are aligned before RMSD calculation.")
            return None
        diff = self.atom_control - self.atom_treatment
        rmsd = np.sqrt(np.mean(np.sum(diff ** 2, axis=1)))
        self.rmsd = rmsd
        # print(f"RMSD between matched structures: {rmsd:.4f}")
        return rmsd
